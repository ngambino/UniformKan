...
&&
  \interval \otimes D_1
  \ar@{.>}[rr]^-{\cod(\pi_1)}
  \ar[dd]^(0.35){\interval \otimes \cod(\sigma_1)}
  \ar[dr]^{\interval \otimes \cod(u_{\zeta})}
&&
  D_1
  \ar[dd]^(0.35){\cod(\sigma_1)}
  \ar[dr]^{\cod(u_{\zeta})}
\\&
  D_2
...
\\
  \interval \otimes D_1
  \ar[rr]^(0.7){I \otimes \cod(\sigma_1)}
  \ar@{-->}[uurrrr] % bug: doesn't work?
...
\bibliography{uniform-kan-bibliography}
