...
Y \otimes U \ar@/_2pc/[drr]_{V \otimes g} \ar[r] & (Y \otimes U) +_{X \otimes U} (X \otimes V) \ar[dr]^-(.35){f \hatotimes g}  & \\ 
 & & Y \otimes V} 
 \]
This operation extends to a bifunctor  $\catE^{\to} \times \catE^{\to} \to \catE^{\to}$, which equips the arrow category of $\catE$ with a symmetric monoidal structure, with unit the canonical map $\hatunit \co 0 \to \unit$.  If the symmetric
...
\begin{proof} For $(f, \phi) \in \liftr{\cal{J}}$, we have
\[ 
\liftr{F}(f, \phi) = (f, \phi_F) \, , \quad \liftr{G}(f, \phi) = (f, \phi_G) 
\]
But the functions $\phi_F$ and $\phi_G$ coincide since, for
every $i \in \cal{I}$, we have that $\sigma_i \co v_{Fi}  \Rightarrow v_{Gi}$ is the identity square on $u_i
\co A_i \to B_i$. 
By the naturality condition for $\phi$, applied to the diagram 
\[
\xymatrix{
...
the class $\cal{I}_1$ is small, while other classes have better closure properties. 
...
The next lemma is a special case of \cref{prod-exp-general}, but we state it explicitly for later reference.
