...
Let $p \co X \to Y$ be a right map for $\kcylinv \otimes \id_{\calE} \co \catE \to \catE^\to$ where $k \in \braces{0, 1}$.
...
Consider a morphism $(s, t) \co p \to q$ of uniform $\cal{I}$-fibrations $p \co X \to Y$ and $q \co U \to V$:
\[
\xymatrix{
  X \ar[r]^s \ar[d]_{p} & U \ar[d]^{q} \\
  Y \ar[r]_t & V
...
\rlap{.}}
\]
Let $\sigma_1 \co v_{j_1} \to u_i$ and $\sigma_2 \co v_{j_2} \to u_i$ with $j_1, j_2 \co \cal{J}$ denote the base changes of $i$ over along $p_1$ and $p_2$, respectively:
\[
\xymatrix{
  C_1
  \ar[rr]
