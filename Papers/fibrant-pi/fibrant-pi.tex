...
Section~\ref{sec:strhe} introduces strong homotopy equivalences, which play an important role in the technical development in
...
The aim of this section is to establish some general facts regarding categories of right maps that will be useful in the remainder of the paper. Most of these facts are expected counterparts of well-known statements for classes of weakly orthogonal classes in the non-algebraic setting. In particular, we will apply these results to categories of trivial uniform Kan fibrations and categories of uniform Kan fibrations, since both of these kinds of categories are defined as categories of right maps.
...
The terminology introduced in the next example is intended to suggest the analogy with the theory
developed by Cisinski in~\cite{cisinski-asterisque}, where he studied extensively model structures in which the cofibrations are the monomorphisms. 



\begin{example}[Uniform trivial fibrations] \label{exa-triv-kan-fib} Let $\catE$ be a presheaf category and write $\cal{M}$
for the subcategory of $\cal{E}^\to$ consisting of decidable monomorphisms, \ie mo\-no\-mor\-phisms whose 
components are functions with decidable image,  and pullback squares. The right $\cal{M}$-maps play a particularly important
role in our development, and therefore we shall introduce a special terminology for them: we will call them 
\emph{uniform  trivial fibrations}. We define the category $\mathsf{TrivFib}$  of
uniform trivial fibrations and their morphisms by letting
\[
 \mathsf{TrivFib} \defeq \liftr{\cal{M}} \rlap{.}
\]
Note that  the compatibility condition
for a uniform trivial fibration $(f, \phi) \co X \to Y$ involves diagrams of the form
\[
\xymatrix{
...
\end{align*}
is defined as follows. For $(f, \phi) \in \liftr{\cal{J}}$, we let $\liftr{F}(f,\phi) \defeq (f, \phi_F)$, where $\phi_F(i, s, t) \defeq \phi(Fi, s, t)$.
We will use this definition in the proof of~\cref{thm:orth-nat}. As shown in~\cite[Proposition~3.8]{garner:small-object-argument}, analogously to the way in which standard orthogonality operations form a Galois connection, the orthogonality functors form an adjunction, 
...
\end{align*}
respectively. Let us also note that this part of construction works for any bipointed category.
\end{remark}
...
\subsection*{Orthogonality and adjunctions} Next, we discuss the interaction between the orthogonality functors and adjunctions. Let us fix  an adjunction
\[
\xymatrix@C+1em{
  \cal{E}
  \ar@<5pt>[r]^{F}
...
\cref{lift-of-adjunction} implies that orthogonality functors commute with left composition with adjoints, as the
next corollary makes precise.
...
\subsection*{Orthogonality and Kan extensions} We now establish some general facts about the interaction between orthogonality functors and left Kan extensions, which will be applied in \cref{sec:unif} to show that uniform  (trivial) Kan fibrations are the right maps of a natural
...
Note that in the next auxiliary result we make the additional assumption of working within a presheaf category. 


\begin{lemma} \label{left-kan-extension-of-representables} Assume that $\cal{E}$ is the category of 
presheaves over a small category $\cat{C}$.
Let $\cal{J}$ be a full subcategory of $\catE_{\cart}^\to$ closed under pullbacks along morphisms with 
codomain a representable presheaf.
...
Assume that $\cal{E}$ is the category of 
presheaves over a small category $\cat{C}$.
...
First, let $\cal{I}_\otimes \defeq \cal{I} + \cal{I}$. Then, define  $u_\otimes \co \cal{I}_\otimes \to \catE^\to$ via the coproduct diagram
\begin{equation*}
\xymatrix@C+2em{
...
\rlap{.}}
\end{equation*}
Note that, even if $u \co \cal{I} \to \catE^\to$ is an inclusion, $u_\otimes \co \cal{I}_\otimes \to \catE^\to$ is not. With these definitions
...
When working with presheaf categories, the category~$\cal{M}$ of decidable monomorphisms and pullback squares
and the inclusion $u \co \cal{M} \hookrightarrow \cal{E}^\to$ are of particular importance, {cf.}\,\cref{exa-triv-kan-fib}.
In this setting, a uniform~$\cal{M}$-fibration, \ie a right~$\cal{M}_\otimes$-map, will be called a \emph{uniform
fibration}, without further specification. We defin the category $\mathsf{Fib}$ of uniform  fibrations and their morphisms
by letting 
\[
...
\begin{example}[Uniform Kan fibrations] A uniform fibration in $\SSet$, will be called a \emph{uniform Kan
fibration}. More explicitly, a uniform Kan fibration $(p, \phi) \co X \to Y$ consists of a simplicial map
...
\end{gather*}
\noten{Please change the above diagram to make use of the appropriate set-theoretic notation}
...
We wish to show that categories of uniform (trivial) fibrations in a presheaf category are the categories
of right maps of a natural weak factorisation system. In the classical setting of simplicial sets, the existence of weak factorisation systems having trivial Kan fibrations and Kan fibrations as their right classes is proved using Quillen's small object argument, applied after having identified a suitable class of generating left maps.
...
Our treatment will be analogous: first, we will  isolate suitable small categories of generating left maps, and then obtain the
required natural weak factorisation system by applying Garner's small object argument~\cite{garner:small-object-argument}.
The key step is contained in the next result. For its statement recall from \cref{exa-triv-kan-fib} that, in a presheaf category
$\cal{E}$, we defined
 $\mathsf{TrivFib} = \liftr{\cal{M}}$, where $\cal{M}$
is the subcategory of $\cal{E}^\to$ consisting of decidable monomorphisms and pullback squares.


\begin{lemma} \label{small-gen-triv-kan} Let $\catE$ be a presheaf category. There is an isomorphism 
$\mathsf{TrivFib} = \liftr{ ( \cal{N}) }$, where $\cal{N}$ is the full subcategory of $\cal{M}$ spanned by the mononorphisms whose codomain is 
a representable presheaf.
\end{lemma}

\begin{proof} The claim follows from \cref{awfs-on-arrows-into-representables}.
\end{proof}

In particular, \cref{small-gen-triv-kan} says  that, for a map $f \co X \to Y$ in a presheaf category $\cal{E} = \mathrm{Psh}(\catC)$,  to give a natural choice of fillers for all diagrams with an arbitrary monomorphism on the right-hand side is the same as to give a choice of fillers for all diagrams of the form
\[
\xymatrix{
A \ar[r] \ar[d] & X \ar[d]^f \\
\yon(x) \ar[r] & Y \rlap{,} }
\]
where $\yon(x)$ the Yoneda embedding of some $x \in \catC$. The reduction to this type of diagrams relies essentially
on the good behaviour of the orthogonality operations with respect to colimits, as in~\cref{awfs-on-arrows-into-representables}.


\medskip

Let us point out that we have that $\mathsf{TrivFib} = \liftr{\cal{S}}$ for every full subcategory $\cal{S} \subseteq \cal{M}$ 
containing~$\cal{N}$, since~$\cal{N}$ is the restriction to maps into representables of $\cal{M}$. In particular, one could consider for
$\cal{S}$ the full subcategory of $\cal{M}$ spaned by decidable monomorphisms with codomain a finite product of representables or, in the case of simplicial sets, decidable monomorphisms with codomain a finite and finite-dimensional simplicial set. These subcategories have better closure properties than $\cal{N}$, but $\cal{N}$ has the  key advantage of being a small category in a wide
class of  presheaf categories, namely the categories of presheaves over an elegant Reedy category~\cite{bergner-rezk-elegant}.
We exploit the smallness of the category $\cal{N}$ defined in \cref{small-gen-triv-kan} to establish our next result. 

\begin{theorem}
Let $\catE$ be a category of elegant Reedy presheaves.
There exists a natural weak factorisation system $(\mathsf{L}, \mathsf{R})$ on $\cal{E}$ such that the category of $\mathsf{R}$-algebras is the category of uniform trivial fibrations.
...
\begin{proof}
Since $\catE$ is a category of elegant Reedy presheaves, the full category $\cal{N} \subseteq \cal{M}$ spanned by monomorphisms with codomain a representable presheaf is small.
Now, since the inclusion~$\cal{N} \hookrightarrow \catE^\to$ preserves $\omega$-filtered colimits, it is possible to apply Garner's small object argument to obtain an algebraic weak factorisation system $(\mathsf{L}, \mathsf{R})$.
The fact that the category of $\mathsf{R}$-algebras is the category of uniform trivial fibrations follows from \cref{small-gen-triv-kan}.
\end{proof}

\begin{theorem}
Let $\catE$ be a category of elegant Reedy presheaves.
There exists a natural weak factorisation system $(\mathsf{L}, \mathsf{R})$ such that the category of $\mathsf{R}$-algebras is the category of uniform fibrations.
In particular, there is a functorial factorisation of maps of simplicial sets which sends a map $f \co X \to Y$ to a diagram of the form
\[
\xymatrix{
X \ar[rr]^f \ar[dr]_{i_f} & & Y \\
 & P_f \ar[ur]_{p_f} }
 \]
where $p_f$ admits the structure of a uniform fibration and $i_f$ admits the structure of a $\mathsf{L}$-coalgebra.
\end{theorem}

\begin{proof}
By Garner's small object argument, it is sufficient to find  a functor $v \co \cal{J} \rightarrow \catE^\to$ such that the
category~$\cal{J}$ is small and the category of uniform Kan fibrations is isomorphic to~$\liftr{\cal{J}}$. By \cref{prod-exp-general} and \cref{small-gen-triv-kan},
such a category is given by $u_\otimes \co \cal{N}_\otimes \rightarrow \catE^\to$, where $u \co \cal{N} \to \cal{E}^\to$ is the 
evident inclusion.
\end{proof}



%\section{Relating Kan fibrations and uniform Kan fibrations}
%\label{sec:kanfuk}

\subsection*{Relating Kan fibrations and uniform Kan fibrations}

\cnote{Please proofread this subsection.}

The aim of this subsection is to compare the standard notion of a (trivial) Kan fibration with the notion of a uniform (trivial) Kan fibration: we will show that under classical reasoning, a map $p \co X \to Y$ of simplicial sets is a (trivial) Kan fibration if and only if it can be equipped with the structure of a uniform (trivial) Kan fibration.
...
In fact, we will prove this equivalence in the more general setting of presheaves over an arbitrary elegant Reedy category.
This covers \eg cubical sets as well.
Let us first review some terminology.
\notec{The introduction of Reedy categories should probably be moved earlier.}

A \emph{Reedy category} $(\R, \deg, \Rp, \Rm)$ is a category $\R$ with a degree function $\deg \co \obj(\R) \to \alpha$ where $\alpha$ is some ordinal and wide subcategories $\Rp, \Rm$ as follows.
The maps of $\Rp$ are monotone with respect to $\deg$ and are called cofaces.
The maps of $\Rm$ are antimonotone with respect to $\deg$ and are called codegeneracies.
Every map in $\R$ is assumed to factor uniquely as a codegeneracy followed by a coface.
Recall that this necessarily implies that $\R$ is skeletal.

Recall from~\cite{bergner-rezk-elegant} that $\R$ is called \emph{elegant} if every element in any presheaf on $\R$ is a degeneracy in a unique way.
For every degree $n < \alpha$, we have \emph{skeleton} comonad $\Sk_n$ and a \emph{coskeleton} monad $\Cosk_n$ on $\Rhat$ forming an adjunction $\Sk_n \dashv \Cosk_n$.
The action of $\Sk_n$ is to keep only the non-degenerate elements of degree strictly less than $n$ (note the index shift contrary to the usual terminology \notec{Is this a problem?}).
Given $A \co \R$, the \emph{boundary} of $A$ is defined as $\partial A = \Sk_{\deg(A)}(\yon A)$.
The counit of $\Sk$ induces a \emph{boundary inclusion} monomorphism $i^A \co \partial A \to \yon A$.

For the remainder of this section, we fix a decidable elegant Reedy category $\R$ and consider presheaves $\calE = \Rhat$.
We will first establish the classical equivalence of trivial fibrations and uniform trivial fibrations.

\begin{definition}
A \emph{trivial fibration} is a right $\cal{U}_0$-map where $\cal{U}_0$ is the discrete subcategory of $\Rhat^{\to}$ consisting of boundary inclusions $\partial A \to \yon A$. 
\end{definition}

Assuming the axiom of choice, this definition coincides with the classical definition of (what we call) a trivial fibration \notec{Notation...} admitting fillers for lifting problems against boundary inclusions (this will be the only use of choice, the remainder of the classical development in this subsection relies on excluded middle only). 
Relating these to uniform trivial fibrations will proceed via the following intermediate notion.

\begin{definition}
A \emph{regular trivial fibration} is a right $\cal{U}_1$-map where $\cal{U}_1$ is the following subcategory of $\Rhat^{\to}$: objects are boundary inlusions $\partial A \to \yon A$ and identities on $\yon A$; non-identity morphisms are commutative squares
\[
\xymatrix{
  \partial A_1
  \ar[r]
...
\]
with the bottom map $d$ in $\Rm$ and not an identity.
\end{definition}

Unravelling the definition slightly, a regular trivial fibration is a map equipped with a filler for each boundary inclusion lifting problems such that degenerate lifting problems have degenerate fillers.

\begin{proposition}[ZFC] \label{trivial-fibration-to-regular}
Let $p \co X \to Y$ be a trivial fibration.
Then $p$ can be given the structure of a regular trivial fibration.
\end{proposition}
...
\end{equation}
We need to choose a filler for it such that for every non-trivial codegeneracy $d \co A \to B$ and factoring of~\eqref{trivial-fibration-to-regular:0} as below,
...
We do this by case distinction (using the law of excluded middle) on whether a factoring~\eqref{trivial-fibration-to-regular:1} exists at all.
...
It only remains to justify that the latter choice is independent of the particular factoring.
This is where the elegancy assumption on $\R$ is used.
...
Let $d_i \co A \to B_i$ be a non-trivial codegeneracy for $i \co \braces{1, 2}$.
Then the span $(d_1, d_2)$ has a pushout $(e_1, e_2)$ with $e_i \co B_i \to C$ a codegeneracy for $i \co \braces{1, 2}$.
...
\begin{remark}
The construction of \cref{trivial-fibration-to-regular} does \emph{not} extend to functor $\liftr{\cal{U}_0} \to \liftr{\cal{U}_1}$ of categories over $\catE^{\to}$.
\end{remark}

We will now relate regular trivial fibrations with uniform trivial fibrations.
For this, we need to know how to go from fillers for boundary inclusions to fillers for monomorphisms.

Let $i \co U \to V$ be a monomorphism in $\Rhat$.
A \emph{cellular presentation} of $i$ is a cocontinuous ordinal-indexed diagram $F \co \gamma \to \Rhat \backslash U$ with colimit $i$
\[
\xymatrix{
  U = F 0
  \ar[r]^-{F s_0}
...
Since $\R$ is elegant, we have $i$ admitting a \emph{canonical cellular presentation} indexed by $\alpha$ where the $\beta$-th successor step adds the non-degenerate representables of degree $\beta$ in $V$ not contained in $U$.
...
\begin{proposition}[ZFC] \label{regular-trivial-fibration-to-uniform}
Let $p \co X \to Y$ be a regular trivial fibration.
Then $p$ can be given the structure of a uniform trivial fibration.
\end{proposition}
...
Importantly, standard Reedy reasoning \notec{Too vague?} shows that this filler is independent of the particular cellular presentation chosen.
...
Recall that $e_{\beta+1}' b$ is by construction the filler for the above lifting problem from $i^B$ provided by the given right lifting structure of $p$.
...
Recall that the base change of $i^A$ along itself is an isomorphism since $i^A$ is mono.
It would follow that the base change of $i^A$ along $\yon(d)$ is an isomorphism as well.
...
But then we can make use of $\cal{U}_1$-coherence in the following comparison of lifting problems:
\[
\xymatrix{
  \partial B
  \ar[r]
...
\begin{theorem}[ZFC] \label{thm:ac-kan-is-uniform} \hfill
\begin{enumerate}[(i)]
...
By the non-algebraic counterpart of \cref{prod-exp-general}, it follows that $\hatexp(\overline{\delta}^k, p)$ is a trivial fibration for $k \co \braces{0, 1}$, and hence a uniform trivial fibration by part (i).
...
The aim of this section is to develop some general results on homotopies in the general setting of a category $\catE$ 
with pullbacks and pushouts, equipped with a functorial cylinder $(\interval \otimes (-), \lcyl, \rcyl, \ccyl)$.
In particular, we introduce the notion of strong homotopy equivalence and provide a characterization for it, which will be useful in our study of the Frobenius condition for uniform fibrations in \cref{sec:frocuf}. We begin by recalling a basic definition.
...
We have a dual construction of a category $\cal{S}_0(\cal{I})$ of strong right homotopy equivalences relative to $u \co \cal{I} \to \catE^\to$.
  ...
  Our next goal is to give an alternative characterisation of strong left or right homotopy equivalences. For this, we need
  some definitions.
...
\to \cal{E}^\to$. As we will see in~\cref{lift-dependent-product}, analogously to the situation for Lawvere's original formulation of the Frobenius condition, there is an equivalent formulation of our Frobenius condition that involves pushforward, rather than pullback, functors. The second aim of this section is to introduce a counterpart of the well-known Beck-Chevalley conditions in our setting.
...
\]
satisying the evident coherence conditions with respect to $\phi$.
\end{enumerate}
\end{proposition}
...
As we will see in \cref{sec:becccu}, cartesian squares between uniform $\cal{I}$-fibrations satisfy the Beck-Chevalley condition under very mild assumptions on $u \co \cal{I} \to \cal{E}^\to$. In particular, cartesian squares between uniform fibrations in presheaf categories satisfy the Beck-Chevalley condition.
...
It should be pointed out that the assumption of connections is used only in the definition of the functor $M$,
and not of $L$, in \cref{thm:she-to-retract-closure}.
The second key step to prove \cref{thm:frobenius-fibrations} is the following lemma, whose technical proof
...
and let $p \co X \to Y$ be a uniform $\cal{I}$-fibration.  Assume that the pullback along $p$ lifts to a functor
\[
\xymatrix@C+2em{
  \cal{I}_{/Y}
  \ar[r]^{p^*}
...
Let $\catE$ be a category of presheaves equipped with a functorial cylinder $( \interval \otimes (-), \rcyl, \lcyl, \ccyl)$ with connections, and recall that we write $u \co \cal{M} \hookrightarrow \catE^\to$ for the inclusion of the subcategory of decidable monomorphisms and pullback squares.
...
\]
In particular, the pushforward of a uniform fibrations along a uniform fibration is again a uniform fibration.  \end{example}

\begin{example}[The Frobenius property for uniform Kan fibrations] As a special case of \cref{frobenius-uniform-presheaf}, we obtain that 
in $\SSet$ the pushforward of a uniform Kan fibrations along a uniform Kan fibration is again a uniform Kan fibration. We consider this a constructive counterpart of the 
result that the pushforward along a Kan fibration preserves Kan fibrations~\cite{voevodsky-simplicial-model}, which cannot
be proved constructively~\cite{coquand-non-constructivity-kan}. Obviously, since exponentiation is a special case of pushforward, this result shows also that the exponential of two uniform Kan complexes is again a Kan complex. 
\end{example} 
...
\noten{Some preliminary discussion needs to be added}
...
to slices of $\cal{I}$ and $\cal{S}_k(\cal{I})$. \noten{We know this for slice of $\cal{I}$. But what about the other claim?}
...
\noten{Add discussion of examples}
...
\begin{proof}[Proof of \cref{strong-h-equiv-base-change-along-fibration}] \noten{Proof needs to be checked. Notation needs to be made consistent with statement}
