...
\item $\hatexp(\intervall, f)$ and $\hatexp(\rcyl, f)$ admit the structure of right $\cal{I}$-maps.
\end{enumerate} 
\end{corollary}
...
Given a functor $u \co \cal{I} \to \catE^{\to}$, we define the category $\cat{S}_\ell(\cal{I})$ of strong left homotopy equivalences in $\catE$ relative to $\cal{I}$ as
follows.
\begin{description}
...
\item[Morphisms] a morphism from $(i,g, h, k)$ to $(i', g', h', k')$ consists of a map $m \co i \to i'$ in $\cat{I}$ such that, writing 
\[
\xymatrix{
A_i \ar[d]_{u_i}  \ar[r]^s & A_{i'} \ar[d]^{u_{i'}} \\
B_i \ar[r]_t & B_{i'}}
\]
...
\xymatrix{
  \interval \otimes A
  \ar[r]^{h}
  \ar[d]_{\interval \otimes s}
&
  A
  \ar[d]^{\interval \otimes s}
\\
  \interval \otimes A'
  \ar[r]_{h'}
&
...
\xymatrix{
  \interval \otimes B
  \ar[r]^{k}
  \ar[d]_{\interval \otimes t}
&
  B
  \ar[d]^{\interval \otimes t}
\\
  \interval \otimes B'
  \ar[r]^{k'}
&
...
\end{mathpar}
\end{description}
Note that there is an obvious forgetful functor $\cat{S}_\ell(\cal{I}) \to \cal{I}$. We write $\cal{S}_\ell(u) : \cat{S}_\ell(\cal{I}) \to \catE^{\to}$ be its composition with $u \co \cat{I} \to \catE^{\to}$.


\begin{remark}
\label{strong-h-equiv-as-section-algebraic}
Following the proof of \cref{strong-h-equiv-as-section}, the category $\cat{S}_\ell(\cal{I})$ can isomorphicly be described at the level of the arrow category $\catE^{\to}$ as follows.  An object is a pair $(i, \rho)$ consisting of $i \in \cal{I}$ and a retraction $\rho$ to $\thetal \hatotimes u_i$, as in the diagram
\[
\xymatrix@C+1em{
u_i 
...
\]
A morphism from $(i, \rho)$ to $(i', \rho')$ consists of $\tau \co i \to i'$ that coheres with $\rho$ and $\rho'$ as below:
\[
\xymatrix{
  \rcyl \hatotimes u_i 
  \ar[r]^-{\rho}
...
\]
With this description, the functor $\cal{S}_\ell(u)$ sends $(i, \rho)$ to $i$.
\end{remark}

\begin{remark}
We have a dual construction of a category $\cat{S}_r(\cal{I})$ of strong right homotopy equivalences relative to some functor $u : \cal{I} \to \catE^{\to}$.
\end{remark}
...
the details explicit. Suppose we are given an object $(i, \rho) \in \cat{S}_\ell(\cal{I})$, lying over $u_i \co A_i \to B_i$.
...
\]
For morphisms, suppose we have $\tau \co (i, \rho) \to (i', \rho')$ in $\cat{S}_l(\cal{I})$.
...
Suppose we are given a morphism $\tau \co i \to i'$ in $\cal{I}$, lying over map $\rcyl \hatotimes u_m$ of arrows.
...
Let $p : Y \to X$ be a $(\lcyl \otimes \id_{\calE})$-right map.
...
Suppose we are given $i : \cat{I}_{/X}$ such that $u_i$ is a strong left homotopy equivalence.
...
\]
Let $\sigma : \cal{I}(i') \to u_i$ denote the base change of $i$ along $p$:
\[
\xymatrix{
  A'
  \ar[r]
...
\[
\xymatrix{
  \rcyl \hatotimes \cal{I}(i_1)
  \ar[r]^-{\rho_1}
  \ar[d]^{\rcyl \hatotimes u_\tau}
&
  \cal{I}(i_1)
  \ar[d]^{u_\tau}
\\
  \rcyl \hatotimes \cal{I}(i_2)
  \ar[r]^-{\rho_2}
&
  \cal{I}(i_2)
}
...
\]
Let $\sigma_1 : \cal{I}(i_1') \to u_i$ and $\sigma_2 : \cal{I}(i_2') \to u_i$ denote the base changes of $i$ along $p_1$ and $p_2$, respectively:
\[
\xymatrix{
  A_1'
  \ar[rr]
...
\]
Recall that we have a canonical morphism $\phi_{u_i}^{\to} : \cal{I}(i_1') \to \cal{I}(i_2')$ over $Y_2$ as indicated in the diagram.
...
Our goal is to check that $\phi_i'$ lifts to a morphism in $\cat{S}(\cal{I})_{/Y_2}$, \ie to verify that $\phi_{u_i}^{\to}$ coheres with as follows:
\[
\xymatrix{
  \rcyl \hatotimes \cal{I}(i_1')
  \ar[r]^-{\rho_1'}
...
&&
  ...
  \ar@{.>}[rr]^-{\rho_1'}
  \ar[dd]^(0.3){\rcyl \hatotimes \sigma_1}
  \ar[dr]^{\rcyl \hatotimes \cal{I}(\phi_i')}
&&
  \cal{I}(i_1')
  \ar[dd]^(0.3){\sigma_1}
  \ar[dr]^{\cal{I}(\phi_i')}
