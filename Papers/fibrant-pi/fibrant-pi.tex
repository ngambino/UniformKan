...
Let us first define the action of $G$ on objects.
...
\]
Let $\tau : \cal{I}(i') \to \cal{I}(i)$ denote the base change of $i$ along $p$:
\[
\xymatrix{
  A'
  \ar[r]
...
\]
Since $\tau$ is a cartesian square, it suffices to solve this problem when projected to the codomains:
\[
\xymatrix@C+2em{
  B'
...
&
  \interval \otimes B'
  \ar@{.>}[r]^{\cod(\rho')}
  \ar[dd]^(0.7){\interval \otimes \cod(\tau)}
  \ar@{-->}@/_1em/[drr]
&
  B'
  \ar[dd]^(0.8){\cod(\tau)}
...
The square $\cod(\tau)$ over $p$ is cartesian by construction.
