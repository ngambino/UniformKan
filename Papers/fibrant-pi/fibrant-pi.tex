...
Through this section, let $\catE$ be a category with finite colimits.
In our running examples, which we will now briefly introduce, this will be a presheaf category.

Our first example will be the category $\catE = \SSet$ of simplicial sets.
Recall that simplicial sets are defined as presheaves $\SSet = \Psh(\Delta)$ over the simplex category $\Delta$.
As any presheaf category, we have $\SSet$ Cartesian closed.

Our second example will be the category $\catE = \CSet$ of cubical sets as studied in~\cite{coquand-variation}.
Cubical sets are defined as presheafs $\CSet = \Psh(\Box)$ over a cube category $\Box$ that encodes the specific variant of cubical sets under consideration.
...
An interval object with contractions is given by $\Delta^1$ with endpoint inclusions $\lcyl = h_1^1 : \braces{0} \to \Delta^1$ and $\rcyl = h_0^1 : \braces{1} \to \Delta^1$.
...
An interval object with contractions is given by $\Box^1$ with endpoint inclusions $\lcyl = \braces{0} \to \Box^1$ and $\rcyl : \braces{1} \to \Box^1$ (formally given by the maps from a singleton set to the free de Morgan algebra on an empty set that pick false and true, respectively).
...
Connection operations $c^k : \Box^1 \otimes \Box^1 \simeq \Box^2 \to \Box^1$ for $k \in \braces{0, 1}$ are induced by the conjunction and disjunction operations of de Morgan algebras.
...
To illustrate the connection to the classical Kan filling condition for cubes, let $i^n : \partial \Box^n \to \Box^n$ for $n \in \mathbb{N}$ denote the boundary inclusion for the representable $\Box^n$, \ie its maximal non-trivial subobject.
...
If $u : \cal{D} \to \catE^\to$ lifts through $\cal{S}_k \to \catE^\to$, then so does $\hateval(u, -) : \cal{D}^\to \to \catE^\to$.
\end{lemma}
...
Since $\otimes_{\cal{D}} \co [\cal{D}, \catE] \times \cal{D} \to \catE$ and $\otimes_\catE : [\catE, \catE]' \times \catE \to \catE$ preserve pushouts in their first and second argument, respectively, it also follows that we have an isomorphism
...
For this section, fix a functor $u \co \cal{I} \to \catE^\to$.

\subsection*{The Frobenius condition}

The first aim of this section is to introduce the Frobenius condition for a map $f \co X \to Y$ with respect to $u$
...
For the remainder of this section, we fix a category $\catE$ with finite colimits equipped with a functorial cylinder functor $(\interval \otimes (-), \rcyl \otimes (-), \lcyl \otimes (-))$ with effective connections as well as a functor $u \co \cal{I} \to \catE^\to$.
\notec{Except for the appendix lemmata}
