...
\begin{abstract}
We develop further the theory of algebraic weak factorisation systems and apply it to 
solve to the problem of proving constructively a counterpart of the classical fact that,
in the category of simplicial sets, the pushforward functors along Kan fibrations preserve Kan fibration. 
In order to do this, we introduce the notion of a uniform Kan fibration in simplicial sets, but our
general approach subsumes also the existing work on cubical sets.
\end{abstract}
...
This paper contributes to the ongoing efforts to define a constructive version of Voevodsky's simplicial model of univalent foundations~\cite{voevodsky-simplicial-model} and prove his canonicity conjecture
on the univalence axiom. We will focus on the question of proving a constructive counterpart of the
well-known statement that, in the category of simplicial sets, the pushforward functor (\ie the right adjoints to 
the pullback functor) along a Kan fibrations preserves Kan fibrations. This result plays an important role in
the development of the simplicial model of univalent foundations in that it ensures that the axioms for $\Pi$-types can be interpreted correctly~\cite[Lemma 2.3.1]{voevodsky-simplicial-model}. 
However, as shown in~\cite{coquand-non-constructivity-kan}, it cannot be proved constructively without changes.
Because of this, Bezem, Coquand and Huber decided to switch from simplicial sets to cubical sets and started to develop a cubical model of univalent foundations~\cite{coquand-cubical-sets}. In that setting, working with the notion of a \emph{uniform Kan fibration}, they have shown that it is possible to prove a counterpart of the fact the pushforward functor along a Kan fibrations preserves Kan fibrations. 


These results may give the impression that there is something intrinsically non-constructive in simplicial homotopy theory. The work presented in this paper  shows that this impression is false and the possibility of eveloping a constructive version of Voevodsky's model remaining within in the setting of simplicial sets remains open. Indeed,
we will introduce a notion of uniform Kan fibration in simplicial sets and prove that the pushforward functor along a 
uniform Kan fibrations preserves uniform Kan fibrations. We will also show that, working within ZFC, the standard
notion of a Kan fibration is equivalent to that of a uniform Kan fibration, thereby proving that our result is classically equivalent to the standard one. Our approach is inspired by some aspects of the development of cubical sets and
by the theory of natural weak factorisation systems~\cite{grandis-tholen-nwfs} and algebraic weak factorisation systems~\cite{garner:small-object-argument}. In fact, we will work at a level of generality that allows us to treat
the simplicial and the cubical setting simoultaneously. 


\medskip


In order to motivate present our results more precisely, let us briefly recall the classical proof of the fact that the pushforward functor along a Kan fibrations preserves Kan fibrations. First of all, applying 
a general fact regarding the interaction of orthogonality classes of maps and adjoint functors, 
one observes that the desired statement is equivalent to showing that 
pullbacks along Kan fibrations preserves trivial cofibrations. One then proves that pullbacks along Kan 
fibrations preserves trivial cofibrations combining the fact that the model structure on simplicial sets is right proper 
(\ie that pullbacks along Kan fibrations preserve weak equivalences) with the fact that pullbacks preserve 
cofibrations (since these are the monomorphisms). 
...
Let $(\catE, \otimes, \unit, \sigma)$ be a symmetric monoidal category,. 
Note that we write $\unit$ for the unit of the tensor product. The initial and terminal objects of $\catE$, when assumed to exist, will be denoted by $\initial$ and $\terminal$, respectively. Let us assume also that $\catE$ is finitely cocomplete. Recall from~\cite[Section 4]{riehl-verity:reedy} that, given $f \co X \to Y$ and $g \co U \to V$, their \emph{pushout product} is the arrow $f \hatotimes g$ fitting in the following pushout diagram:
\[
\xymatrix@R=1.2cm{
...
\subsection{Left and right maps} Given a set of morphisms $\cal{C}$ of $\catE$, we 
...
Note that the naturality condition becomes vacouos when $\cal{C}$ is a discrete category. Even in this
case, however, in order to have a right $\cal{C}$-map it is necessary to define a function $\phi$ providing 
...
image. A right $\cal{M}$-map will be called a \emph{uniform trivial Kan fibration}. 
In \cref{trivial-kan-fibrations} we unfold this definition and study uniform trivial Kan fibrations.
\end{example}
...
\subsection{Intervals and fibrations} By definition an \emph{interval} in $\catE$ is a tuple $\interval = (\interval, \intervall, \intervalr, \varepsilon)$ consisting of  an object $\interval \in \catE$, 
...
\unit \ar[r]^\intervall \ar[dr]_{\id_\unit} & \interval \ar[d]^-{\varepsilon} & \unit \ar[dl]^{\id_\unit} \ar[l]_{\intervalr}  \\
 & \unit & }
 \]
Let us consider a fixed interval $\interval = (\interval, \intervall, \intervalr, \varepsilon)$. Given a functor $u \co \cal{C} \to \catE^\to$, we wish to define a category~$\cal{C}_\interval$ and a functor $u_\interval \co \cal{C}_\interval \to \catE^\to$. First of all, let $\cal{C}_\interval$ be the coproduct
 \[
...
\subsection{Orthogonality functors} 
...
\begin{definition} A \emph{uniform trivial Kan fibration}  is a right $\cal{M}$-map,
...
\begin{definition} A \emph{uniform Kan fibration} is a right $\cal{M}_{\Delta_1}$-map, \ie 
...
\section{Pullback and pushforward of orthogonal maps}
