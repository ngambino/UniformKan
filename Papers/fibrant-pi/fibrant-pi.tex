...
The notation $\interval \otimes (-)$ adopted here is suggestive of the fact that, in many examples, a functorial cylinder is defined using a monoidal structure and an interval object.
...
Recall that a \emph{homotopy from $f$ to $g$}, denoted $h \co f \sim g$, is a morphism $\phi \co \interval \otimes X \to Y$ such that the following diagrams commute:
\begin{equation}
\label{equ:homotopy}
\begin{gathered}
\xymatrix@C=1.2cm{
...
The notion of a \emph{strong deformation retract} is obtained by requiring also that the homotopy $\phi$ is trivial.
For our purposes, it will be convenient to introduce the following generalisation of strong deformation retracts.
...
We use the notion of a strong homotopy equivalence to introduce a special class of functorial cylinders that will play an important role in our development.
...
\]
\item Assume further that the functorial cylinder has connections.
