...
When applied to simplicial sets, this result gives a constructive counterpart of one of the key facts underpinning Voevodsky's simplicial model of univalent foundaitons, while in cubical sets it subsumes some of the existing work on cubical models of type theory.
\end{abstract}
...
In this paper we introduce and study the notion of a uniform fibration in the general setting of categories with a functorial cylinder, taking further ideas in the two lines of research mentioned above.
...
On the other hand, by working without assuming the law of excluded middle or the axiom of choice, we contribute to the definition of a constructive version of the simplicial model of univalent foundations~\cite{voevodsky-simplicial-model}, which is also the goal of work on cubical sets mentioned above.
Such a definition is of great interest since it would improve the relative consistency result in~\cite[Theorem~3.4.3]{voevodsky-simplicial-model} and help to establish the homotopy canonicity conjecture for the univalence axiom~\cite[Conjecture 1]{voevodsky:uf}.

As an application of our results, we prove constructively that in the category of simplicial sets the pushforward along a uniform Kan fibration preserves uniform Kan fibrations.
...
It should be pointed out that this result is far from being a simple application of the axiom of choice: the function that is required for having a uniform fibration needs to satisfy an appropriate naturality condition.
...
We then show that, under the crucial assumption of connections, uniform fibrations satisfy the Frobenius condition (\cref{uniform-fibrations-uniform-frobenius}).
As a consequence, we obtain that the pushforward, \ie the right adjoint to pullback, along a uniform fibration preseves uniform fibrations (\cref{uniform-fibrations-frobenius-pushforward}).
...
The aim of this section is to introduce the main notions that will be used in our development.
Throughout this section, let $\catE$ be a fixed category with finite colimits.

Our motivating examples are the presheaf categories of simplicial sets $\SSet$ and of cubical sets studied in~\cite{coquand-variation}, which we now review.
...
Note that the presence of symmetries precludes $\Box$ being a Reedy category.
\notec{
Because it is has non-trivial isomorphisms.
Something far more non-trivial is true, though: the presence of symmetries and connections precludes $\Box$ from being a generalized Reedy category in one of the possible senses on the market (Berger-Moerdijk, Cisinski).
This can be seen by considering the morphism $\Box^{\braces{a, b}} \to \Box^{\braces{x, y}}$ that sends $x$ to $a \wedge b$ and $y$ to $a \vee b$.
This is not isomorphism since you can only determine $x$ and $y$ up to ordering from information about $a$ and $b$.
However, it also does not factor through $\Box^1$.
}

We stress that the only feature of the cube category $\Box$ relevant to our development are connections and symmetries (although we speculate a two-sided version should carry over to classical cube categories without symmetry \notec{which would make $\Box$ into a generalized Reedy category (even a strict Reedy category if we remove involutions)}).
...
Second, if the functorial cylinder connections, as we assume for our main results in \cref{sec:frocuf} and onwards, then the maps in $\cal{I}_\otimes$ can still be shown to be strong homotopy equivalences in a uniform way, \notec{not true} giving rise to a functor $\cal{I}_\otimes \to \cal{S}_0(\cal{I}) + \cal{S}_1(\cal{I})$ over $\catE^\to$ in the terminology of \cref{sec:remshe} if we assume $\cal{I}$ to be closed under Leibniz product not just with endpoint inclusions, but also with $[\lcyl, \rcyl]$.
...
\rlap{.}}
\]
where the map on the left is the Leibniz product of one of the horn inclusion $h_{1-k}^1 \co \braces{k} \to \Delta^1$ mentioned in \cref{exa:cyl-in-sset} with a decidable monomorphism $i \co A \to B$.
...
In particular, our approach works for Boolean instead of de Morgan algebras as well (worth mentioning?).
The problem seems to be with Coquand's representation of $L$-systems, which does not keep track of joins and meets.
It seems they fixed this with their more recent ``partial elements'', but I just cannot follow their notation.
I guess those are just decidable subobject, \ie what we have.
}
...
The goal of $u \hatotimes_{\cal{D}} (-) \co \cal{D}^\to \to \catE^\to$ lifting through $\cal{S}_k \to \catE^\to$ is equivalent to a retraction to the
map~$\thetak \hatotimes_\catE (u \hatotimes_{\cal{D}} (-))$.
...
Note that the our presheaves are valued in the category of sets, while the presheaves over an elegant Reedy category considered in~\cite{shulman:reedy} are valued in simplicial sets.
\notec{This feels way too random a comment.}
...
\begin{proof}
Part (i) is an application of the axiom of choice\footnote{This will be the only use of choice in this section, the remainder of the classical development in this subsection relies only on the law of the excluded middle.}.
...
\rlap{.}}
\]
If $v_2$ satisfies the uniform Frobenius condition with respect to a functor $u \co \cat{I} \to \catE^\to$, then also $v_1$.
\end{proposition}
...
\notec{For some reason, the actual usage of something having the (uniform) Frobenius property with respect to something else seems a bit reverse.}
...
\rlap{.}}
\]
\notec{This is a repetition of \cref{uniform-fibrations-frobenius-pushforward}.}
...
\rlap{.}}
\end{equation}
First of all, inspection of the proof of~\cref{strong-h-equiv-frobenius} reveals that we indeed have such a natural isomorphism at the level of right maps for strong homotopy equivalences:
\[
\xymatrix{
...
The next question thus becomes: if we do want to have 2-functoriality of pushforward, what should an alternate notion of uniform $\cal{I}$-fibrations be instead?
The easy answer, as per the discussion above, would be to define $\Fib[\cal{I}]' \defeq \liftr{(\cal{S}_0(\cal{I}) + \cal{S}_1(\cal{I}))}$ as right maps for strong homotopy equivalences.
However, there is a better solution going back to the discussion in \cref{retraction-for-connections} and incorporating Coquand \etals idea of taking composition instead of filling as the primitive operation.
\notec{Find appropriate reference.}

For this, we first have to remark that it is possible to extend in a rather straightforward fashion the theory of \cref{sec:ortf} from lifts of arrows against arrows to lifts of \emph{squares} against \emph{squares}, in the sense that a \emph{lifting problem} of a left square $L$ against a right square $R$ given as per the dashed arrows below 
\[
\xymatrix{
  \bullet
  \ar[r]
...
\rlap{,}}
\]
with a \emph{filler} consisting of a diagonal map for the composite square cohering as indicated by the dotted arrow.
Although it is convenient to develop the theory at this level of generality, we will only be needing an intermediate generalization of lifts of \emph{squares} against \emph{arrows}.
Recall that $\Fib[\cal{I}]$ was defined as the right maps for the categories of arrows $\kcyl \hatotimes u$ with $k \in \braces{0, 1}$.
We now define an alternate category $\Fib[\cal{I}]''$ of uniform $\cal{I}$-fibrations as the right maps for the categories of \emph{squares} $\thetak \hatotimes u$ with $k \in \braces{0, 1}$, once again brining to prominence the fundamental square~\eqref{the-trivial-square}.
In the terminology of ?? \notec{insert reference}, the old category $\Fib[\cal{I}]$ corresponds to maps with a Kan filling structure, while the new category $\Fib[\cal{I}]''$ corresponds to maps with a Kan composition structure.

It is easy to see from the characterization \cref{strong-h-equiv-as-section} of strong homotopy equivalences that the map $L_k$ from $\cal{S}_k(u)$ to $\overline{\kcyl \hatotimes u}$ from \cref{lem:from-strong-hequiv} factors as $L_k = \overline{(L_k'')}_l \cc L_k'$ where $L_k'$ goes from $\cal{S}_k(u)$ to $\overline{\thetak \hatotimes u}_l$ and $L_k''$ is the canonical map from $\thetak \hatotimes u$ to the square $\kcyl \hatotimes u$ constant in horizontal direction.
Here, we have silently used a notion of \emph{left retract closure} $\overline{(-)}_l$ for squares that generalizes in an asymmetric, but evident way the retract closure construction of \cref{sec:ortf}.
The binary comparison in \cref{relating-strong-hequiv-and-uniform-fib} thus turns into a ternary comparison as follows:
\begin{equation} \label{relating-strong-hequiv-and-uniform-fib-and-uniform-composition-fib}
\begin{gathered}
\xymatrix@C+2em{
&
  \Fib[\cal{I}]''
  \ar[dl]_{\liftr{(L_0'' + L_1'')}}
  \ar[dd]|!{[dl];[dr]}{\hole}
\\
  \Fib[\cal{I}]'
  \ar[rr]^(0.35){\liftr{(M_0 + M_1)}}
  \ar[dr]
&&
  \Fib[\cal{I}]
  \ar[ul]_{\liftr{(L_0' + L_1')}}
  \ar[dl]
\\&
  \catE^\to
}
\end{gathered}
\end{equation}
As before, only the lower three faces commute here by default.
However, under the assumption that the retraction $\rho$ to $\thetak \hatotimes \kcyl \hatotimes (-)$ provided by effective connections as per \cref{retraction-for-connections} is also a retraction to $\kcyl \hatotimes \thetak \hatotimes (-)$ (this is the case if we have connections, crucially using commutativity of the right half of diagram~\eqref{connections:1}, which was not needed for effective connections), we can state directly a natural transformation from the unit $\eta_{\thetak \hatotimes u} \co \thetak \hatotimes u \to \overline{(\thetak \hatotimes u)}_l$ of the left retract closure monad to the composition $L_k' M_k L_k''$.
Using the analogue of the adjunction~\eqref{garner-adjunction} for lifts of squares against arrows, it then follows that the top triangle in~\eqref{relating-strong-hequiv-and-uniform-fib-and-uniform-composition-fib} commutes when starting from $\Fib[\cal{I}]''$.
Thus, the category $\Fib[\cal{I}]''$ is distinguished from $\Fib[\cal{I}]$ and $\Fib[\cal{I}]'$ in that it is a retract of the other categories, giving credence to the point of view of seeing it as the most primitive of the three notions.

We finish by remarking that it is possible to give a proof of the uniform Frobenius condition for $\Fib[\cal{I}]''$ similar to our current version for $\Fib[\cal{I}]'$ once the generalization of the theory of \cref{sec:ortf} to lifts of squares against squares is in place.
% The following is not quite true: note that using the retract situation, our existing proof of the uniform Frobenius property for $\Fib[\cal{I}']$ transfers to $\Fib[\cal{I}]''$ such that 2-functoriality of lifts of pushforward is preserves.
\notec{To do: comparison with Coquand's proof fits in here as well.}
\end{remark}
