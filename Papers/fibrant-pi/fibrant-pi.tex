...
We define a \emph{uniform trivial Kan fibration} \noten{Should these be called uniform trivial Cisinski fibrations?} to be a right $\cal{M}$-map. Note that  the compatibility condition
...
\noten{Add that ``cofibrations'' are closed under pullback assuming $\cal{I}$ and $\cal{J}$ as above}
...
Recall from \cref{exa-triv-kan-fib} that a uniform trivial Kan fibration is a right $\cal{M}$-map, where 
$\cal{M} \hookrightarrow \cal{E}^\to$ is the subcategory of consisting of decidable monomorphisms and
pullback squares. A \emph{uniform Kan fibration} is a uniform $\cal{M}$-fibration.
We write $\mathsf{KanFib}$ for the category of uniform Kan fibrations and their morphisms, \ie $\mathsf{KanFib} = \Fib{\cal{M}}$.
\end{example}

Below, we illustrate the notion of a uniform Kan fibrations in simplicial sets and cubical sets.
...
We will show in \cref{sec:unif} that, assuming the axiom of choice, every Kan fibration in the usual sense admits the structure of a uniform Kan fibration.
\end{example}
...
We wish to show that uniform trivial Kan fibrations and uniform Kan fibrations in $\cal{E}$ are the right classes of a natural weak factorisation system. In particular, we will apply Garner's small object argument after having isolated suitable small categories of generating left maps.
...
In particular, we introduce the notion of strong homotopy equivalence and provide a characterization for it, which will be useful in our study of Frobenius condition for uniform fibrations in \cref{sec:frocuf}. We begin by recalling a basic definition.
