...
\title[Kan fibrations]{Kan fibrations, algebraic weak factorisation systems \\ and right properness}
...
This paper contributes to the ongoing efforts to obtain a new version of Voevodsky's simplicial model of univalent foundations~\cite{voevodsky-simplicial-model} that is  definable in a constructive setting, \ie without making use of the law of excluded middle or the axiom of choice. We focus our attention on the key fact that is necessary to define the interpretation of $\Pi$-types in the simplicial model, namely that pushforward (\ie the right adjoint to pullback) along a Kan fibration preserves Kan fibrations. Remarkably, as shown in~\cite{coquand-non-constructivity-kan},  this result cannot be proved constructively.
In order to address this issue, Bezem, Coquand and Huber are developing a new model of 
univalent foundation using cubical, rather than simplicial, sets~\cite{coquand-cubical-sets,coquand-variation,huber-thesis}. In particular, they introduced the notion of a uniform Kan fibration in the category of cubical sets and shown that 
the pushforward functor along a uniform Kan fibrations preserves uniform Kan fibrations. Because of these results and the
subsequent focus by many
authors on cubical sets or its equivalents~\cite{awodey-cubical,pitts-cubical-nominal,swan-awfs}, one may get the impression that it is necessary to switch from simplicial sets to cubical sets in order to address constructivity issues.


Our aim here is to show that this is not the case: we introduce the notion of a uniform Kan fibration in simplicial sets and give a constructive proof that the pushforward functor along a uniform Kan fibrations preserves uniform Kan fibrations. Furthermore, we show that,  assuming the axiom of choice, a map of simplicial sets is a uniform Kan fibration if and only if it is a Kan fibration in the usual sense, thus showing that our result is classically equivalent to the standard one. Our approach is based on the theory of natural weak factorisation systems~\cite{grandis-tholen-nwfs} and algebraic weak factorisation systems~\cite{garner:small-object-argument}. In fact, we will work at a level of generality that allows us to obtain as a special case also the corresponding statement for cubical sets proved in~\cite{coquand-variation} (but not that in~\cite{coquand-cubical-sets}, which falls outside our axiomatic approach).

In order to discuss the content of the paper in more detail, let us briefly recall a classical proof of the fact that the pushforward functor along a Kan fibration preserves Kan fibrations~\cite[Lemma~2.3.1]{voevodsky-simplicial-model}. For this, recall that the category of simplicial sets admits a model structure in which the class of fibrations  consists of the Kan fibrations. Hence, there is a  weak factorisation system in which the left class consists  of the trivial cofibrations, \ie the weak homotopy equivalences that are monomorphisms, and the right class consists of the Kan fibrations. By a standard fact about the interaction between weak orthogonality classes and adjoint functors, for a Kan fibration~$p \co X \to Y$, the 
 ...
 the pullback functor $p^*$  preserves trivial cofibrations. Since pullbacks always preserve monomorphisms, 
  the latter statement follows from the fact that the model structure on simplicial sets is right proper, \ie that pullbacks along Kan fibrations preserve weak equivalences. This fact, in turn, can be established either using the right properness of the model structure on topological spaces in which
  the fibrations are the Serre fibrations, as done in~\cite[Theorem~13.1.13]{hirschhorn-model-localizations} or purely combinatorially, via the theory of minimal fibrations, as done in~\cite[Theorem~1.7.1]{joyal-tierney-notes}. But, as shown in~\cite{coquand-non-constructivity-kan}, the use of classical logic is unavoidable in either case. 


We take this as an indication  that the standard notion of a Kan fibration is not suitable for developing simplicial homotopy theory in a constructive setting. A well-known strategy to address this kind of issue is to isolate a new notion that is classically equivalent to the standard one, but allows a constructive development of the theory. We will introduce a candidate for this role, to which we
...
\end{equation}
where the map $h^k_1 \hattimes i$ is obtained by the so-called pushout product of a \emph{decidable monomorphism}
$i \co A \to B$ and the horn inclusion $h^k_1 \co 1 \to \Delta_1$. This definition has two distinct important aspects. On the one hand, following~\cite{grandis-tholen-nwfs,garner:small-object-argument}, we work with a fully
algebraic version of the notion of a fibration, where diagonal fillers for appropriate diagrams is given as part of the structure of a fibration, rather than 
being a mere property. This allows one to require also a naturality property for diagonal fillers, which plays an important role in defining an interpretation
of type theory. The second aspect is the choice of the maps on the left-hand side of the diagram in~\eqref{diag-kan}
...
\section{Two algebraic weak factorisation systems}
