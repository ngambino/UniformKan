...
\notec{We follow Garner~\cite{garner:small-object-argument} (or even earlier?) in the following definition.}`
...
\]
\notec{I think $\phi(j, s, t) \cc b$ would be more readable.}
...
We define a \emph{uniform trivial Kan fibration} to be a right $\cal{M}$-map, \ie \notec{Is this elaboration really needed? It is just a duplication of the above.} a map $f \co X \to Y$ equipped with a function $\phi$ that assigns to every decidable monomorphism $i \co A \to B$ and commuting square
\[
\xymatrix{
...
\notec{Will people get angry about the use of Kan?}
\end{example}
...
\notec{Insert reference (\eg Kamps and Porter, Abstract Homotopy and Simple Homotopy Theory, or Williamson's thesis)}

\begin{definition}
A \emph{functorial cylinder} $(\interval \otimes (-), \lcyl, \rcyl, \ccyl)$ in $\calE$ is an endofunctor $\interval \otimes (-) \co \catE \to \catE$ equipped with natural transformation $\lcyl \co \Id_\catE \to \interval \otimes (-)$, $\rcyl \co \Id_\catE \to \interval \otimes (-)$, called the \emph{right and left endopoint inclusions} \notec{is this terminology standard?}, respectively, and $\ccyl \co \interval \otimes (-) \to \Id_\catE$, such that the following diagram commute \notec{Does it not suffice to say that $\lcyl$ and $\rcyl$ are sections to $\ccyl$?}:
\[
\xymatrix{
...
An \emph{interval object} $(\interval, \ell, r, c)$ \notec{change notation?} in $\calE$ is an object $\interval \co \catE$ equipped with maps $\ell, r \co \top \to \intervall$ called \emph{endpoint inclusions} and a common retraction $c \co \intervall \to \top$ to both $\ell$ and $r$.
...
An interval object is given by $\Delta^1$ \notec{agree on notation} with endpoint inclusions $\braces{0} \to \Delta^1$ and $\braces{1} \to \Delta^1$.
...
\end{enumerate}
\notec{Do we really have to repeat all that stuff? It kind of suggests that the paper is full of long technical definitions whereas many definitions are instances of previously defined things (``A uniform $\cal{I}$-fibration (morphism) is a right $\cal{J}$-map (morphism)''). The details are once again spelled out anyway in the examples below.}
\end{definition}
...
\end{gather*}
\notec{Use point-set notation here to make diagrams less repetitive? E.g., $\braces{k} \times A \hookrightarrow \Delta^1 \times A$. Would get rid of explicit pushout.}
...
However, they are included indirectly since they are retracts of one-dimensional horns by Leibniz product with certain subobjects of representables~\cite{joyal-quaderns} \notec{more explicit: with themselves} (see also \cite[Proposition 2.1.2.6]{lurie:htt}).
Indeed \notec{I don't think it makes sense to say ``Indeed'' here.}, we will show in \cref{sec:kanfuk} that, assuming the axiom of choice, every Kan fibration in the usual sense admits the structure of a uniform Kan fibration.
\end{example}
...
\notec{None of these facts are very likely to be original. For instance, [...] appears/is mentioned in Garner, Riehl, etc.}
...
As shown in~\cite[Proposition~3.8]{garner:small-object-argument}, analogously to the way in which standard orthogonality operations determine a Galois connection between the poset of subset \notec{Subclass? But then ``poset'' sounds strange.} of arrows in~$\catE$ and its opposite, the orthogonality functors form an adjunction
...
\notec{Does this sentence apply to the following proposition as well?}
...
\notec{This is a description of $\liftr{\cal{I}}/X$. How about contrast the $\phi$ part for $\liftr{(\cal{I}/X)}$?}
\end{proof}

Let us also observe that the retract closure commutes with slicing and coslicing. \notec{This should be at least a proper remark.}
...
\notec{Can we not fix this adjunction (separately from the subclasses $\cal{I}$ and $\cal{J}$ for this subsection? It would avoid the repetition in the following proposition.}
...
\]
\end{enumerate}
\notec{I guess you don't like different arrow styles.}
\end{proposition}
...
\rlap{,}}
\]
\notec{which has now been repeated for the third time} there are canonical isomorphisms
\begin{align*}
...
\]
Assume that $\cal{E}$ has pushouts and $\cal{F}$ has pullbacks.
\notec{...to here, we have exact duplication of the start of the subsection.}
Let
\[
\begin{aligned}
  \widehat{F} &\co \cal{K}^\to \times \cal{E}^\to \to \cal{F}^\to
...
Let us now return to consider the case of \notec{three levels of indirection} a category $\cal{E}$ equipped with a functorial cylinder $(\interval \otimes (-), \lcyl, \rcyl, \ccyl)$.
If $I \otimes (-)$ has a right adjoint, we obtain a \emph{functorial cocylinder} $([I, -], \overline{\delta}^0, \overline{\delta}^1, \overline{\epsilon})$ \notec{Fix notation}, \ie a functorial cylinder in the dual category $\catE$. \notec{cite reference (as in first section).}
...
\]
\end{definition}
\notec{I don't like the bracketing. Either choose $\liftl{( \liftr{\cal{I}})}/X$ or $\liftl{(\liftr{(\cal{I}/X)})}$.}
...
\notec{I disagree with the presentation here. The combinatorial proof of right properness in Joyal goes the other way round, reducing right properness to Frobenius.}
However, by the independence result in~\cite{coquand-non-constructivity-kan}, these arguments must use classical reasoning. \notec{...when the classical Kan condition is naively taken as the constructive definition of Kan fibrations.}
...
\notec{This should be a remark, not a lemma. If there is a need for a lemma/proposition, it should be for the corollary that pullback lifts to slices of $\liftr{\cal{I}}$.}

\begin{lemma} \label{slicing-2-functorial}
Let $u \co \cal{I} \to \catE^\to$ be a functor and $f \co X \to Y$ a map in $\catE$.
Then, \notec{I think commas after ``Then'' and ``So'' suggest or very informal casual style.}
\begin{enumerate}[(i)]
...
\notec{
I want to have something like the following definition:
\begin{definition}
Let $u \co \cal{I} \to \catE^\to$ and $v : \cal{J} \to \catE^\to$ be functors.
We say that $v$ satisfies the ??-condition with respect to $u$ if for every object $j \co \cal{J}$, we have that $v_j$ satisfies the Frobenius condition with respect to $u$, and for every morphism $\tau \co j \to j'$ in $\cal{J}$, we have that $v_\tau$ satisfies the Beck-Chevalley condition with respect to $u$.
In detail,
\begin{enumerate}[(i)]
\item
for every object $j \co \cal{J}$, pullback along $v_j \co X \to Y$ lifts to a functor
\[
\xymatrix@C=1.5cm{
  \cal{I}/Y
  \ar[r]^{v_j^*}
...
\]
\item
for every morphism $\tau \co j_1 \to j_2$ in $\cal{J}$, 
\[
\xymatrix{
  X_1
  \ar[d]_{s}
