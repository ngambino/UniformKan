...
\title[NWFS with the Frobenius property]{Natural weak factorisation systems \\
with the Frobenius property}
...
This paper has two main  goals. The first is to develop further the theory of natural weak
factorisation systems~\cite{grandis-tholen-nwfs} by isolating hypotheses
under which Garner's small object argument~\cite{garner:small-object-argument} produces a 
natural weak factorisation system that satisfies what we call the Frobenius property, \ie in which
 the pullback along a right map preserves left maps\footnote{This terminology is inspired by 
the analogy with Lawvere's Frobenius reciprocity condition~\cite{lawvere-1969}.}. 
Examples of such natural weak factorisation systems are given by those consisting of
the trivial cofibrations and the fibrations in a model category that  is right proper and in which cofibrations 
are preserved by pullbacks (\eg when they are given by monomorphisms). These
weak factorisation systems are also closely connected to the axioms for
identity types in Martin-L\"of type theories~\cite[Section~4]{gambino-garner:idtypewfs}. Indeed, our second goal is to apply these results and contribute to the ongoing efforts to obtain a 
constructive version of Voevodsky's simplicial model of univalent foundations~\cite{voevodsky-simplicial-model},
\ie definable without making use of the law of excluded middle or the axiom of choice. In particular, we prove a constructive counterpart of the result that the pushforward along a Kan fibration preserves Kan fibrations, which is fundamental to interpreting correctly type-theoretic dependent products in the simplicial model.
...
In order to treat homogeneously  simplicial sets and of cubical sets (only those 
in~\cite{coquand-variation}, but not that in~\cite{coquand-cubical-sets}), we develop
our theory at the level of generality of categories of presheaves over elegant Reedy categories,
...
with algebraic notions of weak factorisation systems and in a constructive setting.
 
In this context, we introduce the notion of a uniform trivial fibration and of a uniform fibration. 
...
As we will explain below, the algebraic approach is essential in order to be able to 
apply Garner's small object argument and obtain weak factorisation systems in which
uniform trivial fibrations and uniform fibrations are the right classes. 

In analogy with the way trivial fibrations of simplicial sets are defined in classical homotopy theory,
uniform trivial fibrations are defined here as maps equipped with a function that provides
fillers for all lifting problems against decidable monomorphisms. Uniform fibrations, instead, are defined as maps equipped with a function that provides fillers for all lifting  problems againsts the maps obtained by taking the pushout product of  a decidable monomorphisms with an interval endpoint inclusion. 
...
\section{Fibrations vs. uniform fibrations}
...
\section{The Frobenius property via homotopy equivalences} 
