...
If $\interval \otimes (-)$ has a right adjoint, we obtain a \emph{functorial cocylinder} $(\exp(\interval, -), \exp(\bar{\delta}^0, -), \exp(\bar{\delta}^1, -))$ \notec{Fix notation}, \ie a functorial cylinder in the opposite category $\catE^{\op}$~\cite{kamps-porter:homotopy}.
...
Translating to our language, a filling problem for an \emph{L-system} is a lifting problem of the form $\kcyl \hatotimes f \to p$.
...
Let $v_1 \co \cal{J}_1 \to \catE^\to$ and $v_2 \co \cal{J}_2 \to \catE^\to$ be functors related by a functor $\cal{J}_1 \to \cal{J}_2$ over $\calE^\to$.
...
Let $u_1 \co \cal{I}_1 \to \catE^\to$ and $u_2 \co \cal{I}_2 \to \catE^\to$ be functors related by functors $\cal{I}_1 \to \liftl{(\liftr{\cal{I}_2})}$ and $\cal{I}_2 \to \liftl{(\liftr{\cal{I}_1})}$ over $\catE^\to$.
...
\section{The Frobenius and Beck-Chevalley conditions for strong homotopy equivalences}

For this section, we fix a category $\catE$ with finite colimits equipped with a functorial cylinder functor $(\interval \otimes (-), \rcyl \otimes (-), \lcyl \otimes (-))$ with effective connections as well as a functor $u \co \cal{I} \to \catE^\to$.
...
The proof of the main result in this section, \cref{uniform-fibrations-uniform-frobenius}, relies on two key steps, one followin
g from our earlier results on strong homotopy equivalences and one on right orhogonality classes.

For the remainder of this section, let $\catE$ be a category with finite colimits equipped with a functorial cylinder functor $(\interval \otimes (-), \rcyl \otimes (-), \lcyl \otimes (-))$ with effective connections.
We fix a functor $u \co \cal{I} \to \catE^\to$ that is closed closed with respect to Leibniz product with endpoint inclusions.

Broadly speaking, the next result should be read as saying that if pullback along a fibration preserves cofibrations, then it preserves trivial cofibrations.

\begin{theorem} \label{uniform-fibrations-uniform-frobenius}
If $\liftr{(\cal{I}_{\otimes})}$ satisfies the uniform Frobenius property with respect to $\cal{I}$, then also with respect to $\cal{I}_{\otimes}$.
\end{theorem}

\begin{proof}
Use \cref{strong-h-equiv-uniform-frobenius-fibrations} and translate it from strong homotopy equivalences relative to $\cal{I}$ to uniform $\cal{I}$-fibrations using \cref{uniform-frobenius-change-u} with \cref{lem:from-strong-hequiv,lem:to-strong-hequiv}.
\begin{comment}
...
If $(s, t)$ satisfies the Beck-Chevalley condition with respect to $u \co \cal{I} \to \catE^\to$, then the canonical natural transformation $\psi \co t^* q_* \to p_* s^*$ lifts to a natural transformation
\[
\xymatrix@C+2em{
  \Fib[\cal{I}]_{/U}
  \ar[r]^{q_*}
...
As another special case of \cref{frobenius-uniform-presheaf}, we obtain that in $\CSet$ the pushforward of a uniform Kan fibrations along a uniform Kan fibration is again a uniform Kan fibration.
Insufficiency of the classical notion of Kan fibration for a constructive derivation of this statement should follow by an argument analogous to~\cite{coquand-non-constructivity-kan}: the first step there restricts attention to dimensions zero and one where the cubical model minus inversions and the simplicial model coincide.
\end{example}

\notec{To do: remark discussing 2-functoriality and the three possibilities of defning uniform $\cal{I}$-fibrations as right maps for filling, composition, or strong homotopy equivalences. Comparison with Coquand's proof fits in here as well.}
...
\]
by $v_{//X} \co \cal{J}_{//Y} \to \catE^\to$.
Note that the canonical map
\[
  (\liftl{(\liftr{\cal{I}})})_{/X} \to \cal{J}_{//Y}
\]
of functors over $\catE^\to$ is not in general invertible, which we highlight by using the double slash subscript notation.
The assumption now is that pullback along $p$ lifts to a functor $p^* \co \cal{I}_{/Y} \to \cal{J}_{/X}$.
Since we have a canonical map
\[
\cal{S}_k(\cal{J}_{//X}) \to \liftl{(\liftr{\cal{S}_k(\cal{I}_{/X})})}
,\]
the goal will follow if we can show that it lifts further to a functor $p^* \co \cal{S}_k(\cal{I}_{/Y}) \to \cal{S}_k(\cal{J}_{/X})$ as follows:
\[
\xymatrix@C+2em{
  \cal{S}_k(\cal{I}_{/Y})
  \ar@{.>}[r]^{p^*}
...
We define separately the action of the lift $p^* \co \cal{S}_k(\cal{I}_{/Y}) \to \cal{S}_k(\cal{J}_{//Y})$ on objects and on morphisms.
...
\rlap{.}}
\]
Let $\sigma \co (v_{//X})_{j} \to u_i$ denote the pullback of $i$ along $p$ with $j \co \cal{J}_{//Y}$:
\[
\xymatrix{
  C
  \ar[r]
...
\rlap{.}}
\]
We want to make $(v_{//X})_j$ into a strong left homotopy equivalence.
This means to find a retraction $\pi$ to $\thetak \hatotimes (v_{//X})_j$ as follows:
\[
\xymatrix@C+2em{
  v_{j}
  \ar[r]^-{\thetak \hatotimes (v_{//X})_j}
  \ar@{=}[dr]
&
  \kcyl \hatotimes v_{j}
  \ar@{.>}[d]^{\pi}
...
\[
\begin{aligned}
  \sigma_1 &\co (v_{//X})_{j_1} \to u_{i_1}
\, ,\\
  \sigma_2 &\co (v_{//X})_{j_2} \to u_{i_2}
\end{aligned}
\]
...
We want to show that $\upsilon$ in addition forms a morphism of strong left homotopy equivalences $\upsilon \co ((v_{//X})_{j_1}, \pi_1) \to ((v_{//X})_{j_2}, \pi_2)$.
For this, we have to verify commutativity of the following diagram:
\[
\xymatrix@C+2em{
  \kcyl \hatotimes (v_{//X}){j_1}
  \ar[r]^-{\pi_1}
...
\xymatrix@C+2em{
  (v_{//X})_{j_1}
  \ar[rr]^-{\thetak \hatotimes (v_{//X})_{j_1}}
  \ar[dd]_{\sigma_1}
  \ar[dr]^{u_{\upsilon}}
&&
  \kcyl \hatotimes (v_{//X})_{j_1}
  \ar@{.>}[rr]^-{\pi_1}
...
Let $\sigma_1 \co (v_{//X_1})_{j_1} \to u_i$ and $\sigma_2 \co (v_{//X_2})_{j_2} \to u_i$ with $j_1 \co \cal{J}_{//Y_1}$ and $j_2 \co \cal{J}_{//Y_2}$ denote the base changes of $i$ over along $p_1$ and $p_2$, respectively:
\[
\xymatrix{
  C_1
  \ar[rr]
  \ar[dd]_{(v_{//X_1})_{j_1}}
  \pullback{dr}
  \ar@{.>}[dr]
&&
  A
...
\rlap{.}}
\]
Recall that we have a canonical morphism $\phi_{u_i}^\to \co (v_{//X_1})_{j_1} \to (v_{//X_2})_{j_2}$ over $Y_2$ as indicated in the diagram.
By assumption, this lifts to a morphism $\upsilon \defeq \phi_j \co j_1 \to j_2$ in $\cal{J}_{//Y_2}$.
...
Our goal is to check that $\phi_j$ lifts to a morphism in $\cal{S}(\cal{J}_{//Y_2})$, \ie to verify that $u_{\upsilon}$ coheres as follows:
\[
\xymatrix@C+2em{
  \rcyl \hatotimes (v_{//X_1})_{j_1}
  \ar[r]^-{\pi_1}
...
\xymatrix@C+2em{
  (v_{//X_1})_{j_1}
  \ar[rr]^-{\thetak \hatotimes (v_{//X_1})_{j_1}}
  \ar[dd]_{\sigma_1}
  \ar[dr]^{u_\upsilon}
&&
  \rcyl \hatotimes (v_{//X_1})_{j_1}
...
\\&
  (v_{//X_2})_{j_2}
  \ar[rr]^-(0.3){\thetak \hatotimes (v_{//X_2})_{j_2}}
  \ar[dd]_(0.3){\sigma_2}
&&
  \rcyl \hatotimes (v_{//X_2})_{j_2}
  \ar@{.>}[rr]^-(0.3){\pi_2}
