...
The terminology introduced in the next example is intended to suggest the analogy with some aspects of the theory
...
\begin{example}[Uniform trivial Cisinski fibrations] \label{exa-triv-kan-fib}
...
We define a \emph{uniform trivial Cisinski fibration} to be a right $\cal{M}$-map. Note that  the compatibility condition
...
In \cref{sec:unif}, we expand on this example and introduce the notion of a Cisinski fibration.
\end{example}

\begin{example}[Uniform trivial Kan fibrations] A right $\cal{M}$-map in $\SSet$ will be called a 
\emph{uniform trivial Kan fibration}. 
\end{example} 
...
\begin{example}[Uniform Cisinski fibrations] 
Let $\catE$ be a presheaf category equipped with a functorial cylinder $(\interval \otimes (-), \lcyl, \rcyl, \ccyl)$.
Recall from \cref{exa-triv-kan-fib} that a uniform trivial Cisinski fibration is a right $\cal{M}$-map, where 
$\cal{M} \hookrightarrow \cal{E}^\to$ is the subcategory of consisting of decidable monomorphisms and
pullback squares. A \emph{uniform Cisinski fibration} is a uniform $\cal{M}$-fibration.
We write $\mathsf{CisFib}$ for the category of uniform Cisinski fibrations and their morphisms, \ie $\mathsf{CisFib} = \Fib{\cal{M}}$.
\end{example}

Below, we illustrate the notion of a uniform Cisinski fibrations in simplicial sets and cubical sets.

\begin{example}[Uniform Kan fibrations]
In the special case of the category of simplicial sets, a uniform $\cal{M}$-fibration will be called a \emph{uniform Kan
fibration}. This notion of a uniform Kan fibration involves diagrams of the form
\[
\xymatrix{
...
We wish to show that categories of uniform (trivial) Cisinski fibrations in a presheaf category are the categories
...
The key lemma for our results is the following result.

\begin{lemma} \label{small-gen-triv-kan} Let $\catE$ be a presheaf category. There is an isomorphism
\[
\mathsf{TrivCisFib} = \liftr{  \cal{M}_{\textup{rep}} }
\]
where 
\[
 \cal{M}_{\textup{rep}}  = \braces{i \co A \rightarrow \yon(c) \mid i \text{ is a decidable monomorphism} } \rlap{.}
\]
\end{lemma}
...
Note that the category of uniform trivial Cisinski fibrations is isomorphic also to the right orthogonality categories of 
\begin{align*}
...
\end{align*}
Indeed, for every full subcategory $\cal{S} \subseteq \cal{M}$ containing $\cal{M}_\textup{rep}$ we have that $\liftr{\cal{M}} = \liftr{\cal{S}}$, since $\cal{M}_\textup{rep}$ is the restriction to maps into representables of $\cal{M}$. The classes $\cal{M}'$ and $\cal{M}'' $ have
better closure properties than $\cal{M}_\textup{rep}$, but the latter has the  key advantage of being a small category in a wide
...
We exploit this fact to establish our next result. 
...
Now, since the inclusion $\cal{M}_{\textup{rep}} \hookrightarrow \catE^\to$ preserves $\omega$-filtered colimits, it is possible to apply Garner's small object argument to obtain an algebraic weak factorisation system $(\mathsf{L}, \mathsf{R})$.
...
\begin{proof}
By Garner's small object argument, it is sufficient to find  a functor $v \co \cal{J} \rightarrow \catE^\to$ such that $\cal{J}$ is small and the category of uniform Kan fibrations is isomorphic to~$\liftr{\cal{J}}$. By \cref{prod-exp-general} and \cref{small-gen-triv-kan},
such a category is obtained from  the inclusion $u \co \cal{M}_{\textup{rep}} \to \cal{E}^\to$ by taking Leibniz products with respect to
endpoint inclusions.
\end{proof}

\noten{The last proof needs further polishing. Revised this section up to here.}
...
\noten{Some discussion also that exponentiation of uniform Kan complexes is a uniform Kan complex. Discuss what were the key ideas involved in the proof}
...
\end{gathered}
\end{equation}
satisfying coherence with respect to $\phi$. \noten{This is the first time this idea is mentioned. It should be explained in general when we define Beck-Chevalley.}
