...
For this section, fix a category $\catE$. Given a set of morphisms $I$ in $\catE$, we define $\liftr{I}$ to be the class of morphisms of $\catE$ that have 
the right lifting property with respect to all the morphisms in~$I$. Dually, we write $\liftl{I}$ for the class of morphisms of $\catE$ that have the left lifting
property with respect to all the morphisms of $I$. These operations determine a Galois connection $\liftr{\brarghole} \dashv \liftl{\brarghole}$ between the poset of subsets of arrows in $\catE$ and its opposite. Furthermore, these operations enjoy several properties that are useful in developing homotopical algebra.
...
\end{gathered}
\end{equation}
Here, instead of starting from a subset of $\catE^\to$, we consider a category $\cat{A}$, to be thought of an indexing category, and a functor $U \co \cat{A} \to \catE^\to$, which assigns an arrow $U a$ in $\catE$ to each index $a \in \cat{A}$. After reviewing this adjunction, we establish some analogues of well-known properties of the orthogonality relation in the extended setting, which will be useful in the following.
 
 
 \begin{definition} Let $U \co \cat{A} \to \catE^\to$ be a functor. A \emph{right lifting structure} on a map $g \co C \to D$ with respect to $U$ is 
