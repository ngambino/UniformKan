...
\notec{Fix: this definition presupposes the Leibniz construction.}
...
Formally, for $k \in \braces{0, 1}$, the functor $\kcyl \hatotimes (-) \co \catE^\to \to \catE^\to$ is required to lift through the forgetful functor $\cal{S}_k \to \catE^\to$.
\end{definition}
...
Let $(\interval \otimes (-), \lcyl \otimes (-), \rcyl \otimes (-))$ be a functorial cylinder with contractions $\epsilon \times (-)$ and connections $h^k \otimes (-)$ for $k \in \braces{0, 1}$.
If $\interval \otimes (-)$ preserves finite colimits, then the functorial cylinder has effective connections.
\end{proposition}

\begin{proof}
\notec{This proof is horrible. Note that if you start from an interval object with effective connections (nicely shown to follow from contractions and connections) and use the remark from Section~4 that strong homotopy equivalences are closed under Leibniz tensor with arbitrary maps, then you get this proof.}
We will only show that $\lcyl \hatotimes (-) \co \catE^\to \to \catE^\to$ lifts through $\cal{S}_0 \to \catE^\to$.
Given $u \co X \to Y$, we need to make
\[
  f \defeq \lcyl \hatotimes u \co \interval \otimes X +_X Y \to \interval \otimes Y
\]
into a strong homotopy equivalence.
The homotopy inverse
\[
  g \co \interval \otimes Y \to \lcyl \hatotimes u \co \interval \otimes X +_X Y
\]
will be $g = \inr \cc (\epsilon \otimes Y)$.
Noting that $I \otimes (-)$ preserves pushouts, the homotopy
\[
  \phi \co \interval \otimes \interval \otimes X +_{\interval \otimes X} \interval \otimes Y \to \interval \otimes X +_X Y
\]
is defined functorially as $\phi = h^0 \otimes X +_{\epsilon \otimes X} \epsilon \otimes Y$ using the right part of~\eqref{connections:0}.
Its signature $\phi : g \cc f ~ \id$ follows from the left parts of~\eqref{connections:0} and~\eqref{connections:1} and the laws for contractions.
The homotopy
\[
  \psi : \interval \otimes \interval \otimes Y \to \interval \otimes Y
\]
is defined as $\psi = h^0 \otimes Y$.
Its signature $\psi : f \cc g ~ \id$ follows from the left parts of~\eqref{connections:0} and~\eqref{connections:1}, respectively.
Finally, the strength $f \cc \phi = \psi \cc f$ follows from the right part of~\eqref{connections:0}.

The above assignment of a strong left homotopy equivalence is functorial in $u$ since we are dealing with natural transformations only.
\end{proof}
...
If the functorial cylinder is induced by tensoring with an interval object in a symmetric monoidal category, then \cref{strong-h-equiv-as-section} shows that strong left or right homotopy equivalences are closed under Leibniz tensor with arbitrary maps.
\end{remark}
...
\]
This means that an object of $\cal{S}_k(\cal{I})$ consists of an object $i \co \cal{I}$ together with data $(g, \phi, \psi)$ making~$u_i$ into a strong (left or right) homotopy equivalence.
...
Under this identification, an object of $\cal{S}_k(\cal{I})$ consists of an object $i \co \cal{I}$ together with a retraction to $\thetak \hatotimes u_i$.
...
We want to show that $\tau'$ in addition forms a morphism of strong left homotopy equivalences $\tau' \co (i_1', \rho_1') \to (i_2', \rho_2')$.
