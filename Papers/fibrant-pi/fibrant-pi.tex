...
In the remainder of this section, we extend some useful facts about the usual notion of orthogonality to its algebraic counterpart. 
...
We now consider the effect of adjoint functors on orthogonality. In the standard setting, it is well known that 
...
We now consider slicing. In the classical setting it is well-known that slicing and taking orthogonality classes commute with each other. In order to provide a counterpart of this fact, we need some auxiliary definitions. Given a functor $U : \cat{A} \to \catE^{\to}$ and $X \in \catE$.  we define the category $\cal{A}/X$
and a functor $U/X \co \cal{A}/X \to (\cal{E}/X)^\to$ as follows. The category $\cat{A}/X$ has as objects pairs consisting of an object $a \in \cal{A}$ and a commutative triangle of the form
\[
\xymatrix{
...
\]
which also admits an explicit description dual to the one given above for $\catA_{/X}$. With these definitions in place, we can now state the counterpart in our setting of the familiar commutation between slicing and orthogonality operations. 
...
\begin{enumerate}[(i)]
\item The functor $\liftr{\brarghole}$ commutes (up to natural isomorphism) with slicing.
\item The functor $\liftl{\brarghole}$ commutes (up to natural isomorphism) with coslicing.
\end{enumerate}
\end{lemma}
