...
We begin by recalling a basic definition from~\cite{kamps-porter:homotopy}.

\begin{definition}
A \emph{functorial cylinder} $(\interval \otimes (-), \lcyl, \rcyl)$ in $\calE$ is an endofunctor $\interval \otimes (-) \co \catE \to \catE$ equipped with natural transformation 
\[
\lcyl \otimes (-) \co \Id_\catE \to \interval \otimes (-) \, , \quad \rcyl \otimes (-) \co \Id_\catE \to \interval \otimes (-) \rlap{,}
\]
called the \emph{right and left endopoint inclusions}, respectively.
\end{definition}
...
\subsection*{Homotopies} % \label{def:homotopy}
...
\begin{definition} 
 A map $f \co X \to Y$ is called a \emph{left homotopy equivalence} if there exist $g \co Y \to X$ and homotopies $\phi \co \id_X \sim g \cc f$ and $\psi \co \id_Y \sim f \cc g$.
...
Such a left (or right) homotopy equivalence is said to be \emph{strong} if the diagram
\[
\xymatrix{
...
\begin{remark} In a functorial cylinder with connections as in \cref{def:cyl-with-con}, the components of $\rcyl$ and $\lcyl$ 
 are a strong right and left homotopy equivalences, respectively.
 \end{remark}
...
In particular, we provide a characterization of strong homotopy equivalences, which will be useful in our study of the Frobenius
and Beck-Chevalley conditions for uniform fibrations in \cref{sec:frocuf}.
