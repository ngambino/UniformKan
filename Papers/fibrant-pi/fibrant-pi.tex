...
\label{sec:right-maps-and-fibrations}
...
\id_\catE  \ar[r]^-{\lcyl} \ar@{=}[dr] & \interval \otimes (-) \ar[d]^-(.4){\ccyl} & \id_\catE \ar@{=}[dl] \ar[l]_-{\rcyl}  \\
 ...
 We use it in combination with the Leibniz construction to define a functor $v \co \cal{J} \to \catE^\to$ that
...
\begin{example}[Uniform Kan fibration] Let $\catE$ be a presheaf category equipped
...
As shown in~\cite{garner:small-object-argument}, analogously to the way in which standard orthogonality operations determine a Galois connection between the poset of subsets of arrows in~$\catE$ and its opposite, the orthogonality functors form an adjunction 
\begin{equation}
...
We can apply the results obtained above to relate precisely right maps and uniform fibrations. 


\begin{proposition} \label{prod-exp-general}
Let $\cal{E}$ be a finitely complete and cocomplete category equipped with a functorial cylinder $(\interval \otimes (-), \rcyl, \lcyl, \ccyl)$,
and let $u \co \cal{I} \to \cal{E}^\to$ be a functor.  For every map $p \co X \to Y$ in $\cal{E}$ the following are equivalent: 
\begin{enumerate}[(i)]
...
\to \cal{E}^\to$. As we will see, our results in \cref{sec:orthogonal-functors} show that when the pullback functor $f^*$ has a right adjoint 
...
As we will see in \cref{sec:beck-chevalley-fibrations}, cartesian squares between uniform fibrations satisfy
...
of Frobenius condition for uniform fibrations in \cref{sec:frobenius-uniform-fibration}. We begin by recalling a basic
definition. 
...
X \ar[r]_{f} & Y}
\]
\item  A map $f \co X \to Y$ is called a \emph{right homotopy equivalence} if there exist $g \co Y \to X$ and homotopies $h \co g \cc f
 \sim \id_X $ and $k \co
 f \cc g \sim \id_Y$. Such a right homotopy equivalence is said to be \emph{strong} if the
following diagram commutes [CHANGE]
\[
\xymatrix{
\interval \otimes X \ar[r]^{\interval \otimes f } \ar[d]_{h} & \interval \otimes Y \ar[d]^{k} \\
X \ar[r]_{f} & Y}
...
\begin{definition} A functorial cylinder $(\interval \otimes (-), \rcyl, \lcyl, \ccyl)$ is said to \emph{have connections} if $\kcyl$ are strong left homotopy equivalence. ADD.
\end{definition}

By \cref{strong-h-equiv-as-section} a functorial cylinder has connections if and only if  for $f \co X \to Y$, we have 
\begin{equation}
...
\ie a retraction to $\thetak \hatotimes (\kcyl \hatotimes u_i)$ by the assumption that~$\cal{I}$ is
closed with respect to Leibniz product with the endpoint inclusions. But such a retraction is provided by taking 
...
Let $\catE$ be a category equipped with a functorial cylinder $( \interval \otimes (-), \rcyl, \lcyl, \ccyl)$ with connections,
and $u \co \cal{I} \to \catE^\to$ a functor that is closed with respect to Leibniz product with endpoint inclusions. 
For every uniform $\cal{I}$-fibration $p \co Y \to X$, if pullback along $p$ lifts to a functor
\[
\xymatrix@C+2em{
  \cal{I}_{/X}
  \ar[r]^{F}
...
always preserve monomorphisms, it follows that for every uniform Kan fibration $p \co Y \to X$,
...
\[
\xymatrix@C+2em{
\mathsf{KanFib}_{/Y} \ar[r] \ar[d]_{} &  \mathsf{KanFib}_{/X} \ar[d] \\ 
(\cal{E}_{/Y})^\to \ar[r]_{p_*} & (\cal{E}_{/X})^\to
}
\]
...
\label{sec:beck-chevalley-fibrations}
