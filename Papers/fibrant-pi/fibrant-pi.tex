...
Second, if we assume generalized connections in the sense of $(1+n+1)$-ary operations that behaves as connections for fixed middle $n$ arguments, then the maps in $\cal{I}_\otimes'$ can still be shown to be strong homotopy equivalences in a uniform way.
This gives rise to a functor $\cal{I}_\otimes' \to \cal{S}_0(\cal{I}) + \cal{S}_1(\cal{I})$ over $\catE^\to$ in the terminology of \cref{sec:remshe} if we assume $\cal{I}$ to be closed under Leibniz product not just with endpoint inclusions, but also with $[\lcyl, \rcyl]$ (this is the case if $[\lcyl, \rcyl]$ is a decidable monomorphism and the maps in $\cal{I}$ are the decidable monomorphisms in a presheaf category as in \cref{sec:unifpc}).
...
An easy answer, as per the discussion above, would be to define $\Fib[\cal{I}]' \defeq \liftr{(\cal{S}_0(\cal{I}) + \cal{S}_1(\cal{I}))}$ as right maps for strong homotopy equivalences.
In contrast to $\Fib[\cal{I}]$, however, it does not seem that $\Fib[\cal{I}]'$-admits a description as the category of algebras in a natural weak factorization system.
\notec{To do: comparison with Coquand's proof needed; what reference to use?}
\end{remark}
