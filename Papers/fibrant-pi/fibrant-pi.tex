...
$\bot \co 0_\catE \Rightarrow \Id_\catE$ be the natural transformation with the unique maps $\bot_X \co 0 \to X$
as components. Observe that for every $f \co X \to Y$, we have an isomorphism
\begin{equation}
\label{equ:bot-hatotimes-f}
\bot \hatotimes f \iso f 
\end{equation}
For $k \in \{ 0, 1 \}$, we then have a commutative square of natural transformations
\[
\xymatrix@C+2em{
0_\catE \ar[r]^{\bot} \ar[d]_{\bot} & \id_\catE \ar[d]^{\rcyl} \\
\id_\catE \ar[r]_{\lcyl} & \interval \otimes (-)}
\]
This diagram determines two maps in the arrow category of $[\catE, \catE]$, 
\begin{equation}
\label{equ:thetas}
...
maps in~\eqref{equ:thetas} give us maps 
 \[
...
X \ar[r]_{f} & Y }
\end{equation}
Secondly, requiring that the two horizontal composites are a section to $\theta \hatotimes f$ means that the diagrams
\begin{equation}
...
\[
L(i, \rho) \defeq (i, u_i, \thetal \hatotimes u_i, \rho) \, .
\]
Observe that this definition makes the diagram in~\eqref{she-to-retract-closure} commute. 

\medskip

In order to define the second functor,  we make two furher assumptions. The first is that  $\rcyl$ is a strong left homotopy equivalence.
[THIS NEEDS TO BE FIXED]
Our second additional assumption is that the functor $\rcyl \otimes (-) \co \catE^\to \to \catE^\to$ lifts 
to $\cal{I}$, in the sense that we have a diagram of the form
\[
\xymatrix@C+2em{
\cal{I}  \ar[d]_u \ar[r]^{\rcyl \hatotimes (-)} & \cal{I} \ar[d]^{u} \\
\cal{E}^\to \ar[r]_{\rcyl \hatotimes (-)} & \cal{E}^\to}
\]
This means that for $i \in \cal{I}$, we have $\rcyl \hatotimes i \in \cal{I}$ such that
\begin{equation}
\label{equ:I-closed-under-tensor}
u_{\rcyl \hatotimes i} = \rcyl \hatotimes u_i \, .
\end{equation}
...
\xymatrix@C+2em{
  \rcyl_X
  \ar[r]^-{\thetal \hatotimes \rcyl_X}
  \ar[dr]_{\id}
&
  \rcyl \hatotimes \rcyl_X \ar[d]^{\rho_X}
\\&
  \rcyl_X
}
\]
...
with a retraction to $\thetal \hatotimes (\rcyl \hatotimes u_i)$. But such a retraction is obtained as follows [THIS NEEDS TO BE FIXED]
...
\]
where we used the equation in~\eqref{equ:I-closed-under-tensor}. We can then let $M(i) \defeq (i, \rho \hatotimes i)$.
...
\begin{proof} The claim is a consequence of \cref{pitchfork-leibniz-adjunction}.
\end{proof}
