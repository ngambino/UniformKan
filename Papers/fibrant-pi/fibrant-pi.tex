...
\section{Right maps and uniform fibrations} 
...
In order to introduce the notion of a uniform $\cal{I}$-fibration, let us now assume that $\catE$ has a functorial cylinder $(\interval \otimes (-), \lcyl, \rcyl, \ccyl)$, \ie an endofunctor $\interval \otimes (-) \co \catE \to \catE$ equipped with natural transformation $\lcyl, \rcyl$ and $\ccyl$ having components
of the form
\[
...
X  \ar[r]^{\lcyl_X} \ar[dr]_{\id_X} & \interval \otimes X \ar[d]^-(.4){\varepsilon_X} & X \ar[dl]^{\id_X} \ar[l]_-{\rcyl_X}  \\
 ...
 It is also convenient to introduce a special case of the Leibniz construction. Given
...
by letting $(\kcyl \otimes u)_i \defeq \kcyl  \hatotimes u_i$,  where $\kcyl \hatotimes u_i$ is given by the construction above. 
 ...
 We define a category~$\cal{J}$ and a functor $v \co \cal{J} \to \catE^\to$. 
...
with a functorial cylinder $(\interval \otimes (-), \rcyl, \lcyl, \ccyl)$. 
...
a functor $u \co \cal{I} \to \catE^\to$ to its right orthogonal $\liftr{u} \co \liftr{\cal{I}} \to \catE^\to$
...
\end{gathered}
\end{equation}
For details, see~\cite{garner:small-object-argument}. 
...
\begin{remark} A cylinder has connections if and only if  for $f \co X \to Y$, we have 
\begin{equation}
...
the main result in this section, \cref{thm:frobenius-fibrations} relies on results on strong homotopy equivalences. We begin 
with a simple lemma.
...
\cref{strong-h-equiv-as-section-algebraic} can be applied to construct auxiliary functors:
\begin{equation}
...
\xycenter{
 ...
 & \ \catE^\to & }
 \end{equation}
For the functor in~\eqref{she-to-retract-closure}, we we describe its action on objects, leaving the definition of the definition 
of the action on arrows to the
...
In order to define the functor in~\eqref{horn-times-gen-to-she}, let us assume  that we have a functorial cylinder with connections
and that $\cal{I}$  is closed with respect to Leibniz product with the endpoint inclusions, in the sense 
that the  functor $\kcyl \hatotimes (-) \co \catE^\to \to \catE^\to$ lifts 
to $\cal{I}$, thus giving us  a diagram of the form
\[
\xymatrix@C+2em{
...
\begin{example}[Frobenius property for uniform Kan fibrations] Let $\catE$ be a category of presheaves 
...
\begin{example}[Pushforward of uniform Kan fibrations along uniform Kan fibrations] For every uniform Kan
fibration $p \co Y \to X$, pushforward along $p$ lifts to a functor 
\[
\xymatrix@C+2em{
