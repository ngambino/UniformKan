...
\begin{example} \label{pitchfork-leibniz-most-general-example}
...
\end{gathered}
\end{equation}
Using \cref{generalized-uniform-frob-nicer}, this amounts to the generalized functorial Frobenius condition for $(\liftl{(\liftr{\cal{I}})}, \liftr{\cal{I}}, \liftl{(\liftr{\cal{I}})})$.
...
We begin by illustrating how the adjunction in~\eqref{equ:leibniz-prod-exp}  fits into the setting of \cref{pitchfork-leibniz-most-general-example}.
For this, recall that we have the adjunction in~\eqref{equ:cyl-exp-adj} and write $\bar{\delta}^k \co \exp(I, -) \to \Id$ for the mate of $\delta^k \co \Id \to I \otimes (-)$, for $k \in \braces{0 \, , 1 }$, so that $\alpha \defeq (\delta^k, \bar{\delta}^k)$ is a map of adjunctions
from $\Id \dashv \Id$ to $I \otimes \dashv \exp(I, -)$. Inspecting~\eqref{delta-otimes-definition}, we see that the left adjoint 
$\delta^k \hatotimes (-)$ in~\eqref{equ:leibniz-prod-exp} arises by applying the Leibniz construction to $\hateval_L(\alpha, -)$.
Similarly, inspecting~\eqref{hatexp-delta-definition}, we see that $\hatexp(\bar{\delta}^k, -)$ arises by applying the the dual Leibniz construction to $\hateval_R(\alpha, -)$.  As described in \cref{pitchfork-leibniz-most-general-example}, this allows us to apply \cref{lift-of-leibniz-adjunction,lift-of-leibniz-adjunction-pullback-form} in this situation.
...
The statement then follows from \cref{lift-of-leibniz-adjunction-pullback-form}.
\end{proof}
...
\rlap{.}}
\]
The goal now follows from the outer square by applying \cref{lift-of-leibniz-adjunction} as instantiated in 
the proof of~\cref{prod-exp-general}. 
\end{proof}
