...
\end{enumerate}
Condition~(ii) means that $\interval \otimes i : \interval \otimes A \to \interval \otimes B$ is in $\cal{M}$ given $i : A \to B$ in $\cal{M}$ and that $\interval \otimes (-)$ preserves pullback squares.
Condition~(iii) means that $\kcyl \otimes C : C \to \interval \otimes C$ is in $\cal{M}$ for any $C \in \catE$ and that the naturality squares of $\kcyl \otimes (-)$ are pullbacks ($k \in \braces{0, 1}$).
...
It it natural to consider for $\cal{M}$ the full subcatgory of $\catE_\cart^\to$ of either all monomorphisms or all decidable monomorphisms \footnote{
Assuming the law of excluded middle, all monomorphisms are decidable.
We expect that the restriction to decidable monomorphisms is important to treat constructively with further aspects of the theory such as universes (\cf \cite{coquand-face}).
...
For any of these two choices, conditions~(i) to~(iv) are inherited from the setting $\calE = \Set$.
Below, we illustrate the notion of a uniform fibration in the special cases of simplicial sets and cubical sets
\end{example}

\begin{example}[Uniform fibrations in simplicial sets]
A uniform fibration in $\SSet$ with $\cal{M}$ consisting of all decidable monomorphisms will be called a \emph{uniform Kan fibration}.
...
A uniform fibration in $\CSet$ with $\cal{M}$ consisting of all decidable monomorphisms will be called a \emph{uniform Kan fibration}.
...
\footnote{Without symmetries, one should consider two-sided Leibniz tensors such as $i^a \hatotimes \lcyl \hatotimes i^b : \sqcup_a^{a+1+b} \to \Box^{a+1+b}$.}

Let us point out that what we call a uniform Kan fibration in cubical sets does not seem to correspond to the notion of uniform Kan fibration defined in~\cite{coquand-face,coquand-variation}: the notions of $L$-system and partial elements used therein capture only a proper subset $\cal{M}'$ of decidable monomorphisms.
Since the presence of symmetries precludes the cube category being Reedy, this may turn out to be a natural restriction.
Fortunately, this choice $\cal{M}'$ also satisfies the properties~(i) to~(iv) of \cref{uniform-fib-presheaf-cat} and is thus covered by our setting.
\end{example}
...
\end{aligned}
\]
respectively, where $\cal{M}$ is a full subcategory of $\catE_\cart^\to$ of monomorphisms and pullback squares.
