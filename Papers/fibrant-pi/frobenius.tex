...
To illustrate the relationship with the standard Kan filling condition for cubes, let $i^n \co \partial \Box^n \to \Box^n$, for $n \in \mathbb{N}$, denote the boundary inclusion for of the $n$-cube $\Box^n$ \notec{Do these form a generating set for cofibrations?}, which is given by the $n$-fold Leibniz product of~$[\lcyl, \rcyl] \co \braces{0\,, 1} \to \Box^1$. 
Then the maps
\[
...
\begin{proof} This is a just diagram-chasing argument, but we outline it since this gives an unfolding of the abstract proof of~\cref{thm:kcylf-is-she-alg},
...
\rlap{.}}
\]
But now $\kcylinv \otimes Y \iso \kcylinv \hatotimes \bot_Y$ \notec{Check indices}, where $\bot_Y \co 0 \to Y$ is the unique map from the initial object of~$\catE$ into~$Y$, which is a cofibration by the assumption that $(\Cof, \TrivFib)$ is suitable (condition~(S2) of \cref{thm:suitable-wfs}). By~\cref{thm:kcyl-of-cof-is-trivcof-non-alg}, we have that $\kcylinv \hatotimes \bot_Y \in \TrivCof$ and
...
For every $u \co \cal{I} \to \catE^\to$, we have pairs of functors as follows \notec{Add bracketing to expressions below to
help readers to parse them}
\[
\begin{aligned}
...
But for such lifting problems the order of slicing \notec{Do you mean the order of slicing and taking orthogonality?} makes no difference.
\end{proof}
...
require  to have also a comultiplication $\LL \to \LL \LL$ and a multiplication $\RR \RR \to \RR$, with components of the form
\begin{align*}
\xymatrix{
  X
  \ar@{=}[r]
...
$(f, h)$ \notec{I think $(f,h)$ is not quite correct here, as we have more data} to $h^*(f)$. 
...
However, under the stated assumption, the lift $\tilde{Q}$ can be reconstructed from the lift $\tilde{P}$ by first pulling back $v_j$ along $B \to Y$ \notec{Is it here that the assumption of having a Grothendieck fibration is used? Make the 
assumption explicit} to a map $v_{j'} \co B \to D$ with $j' \to j$ a map in $\cal{J}$ in the above situation and then setting $\tilde{Q}(i, j) \defeq \tilde{P}(i, j')$.
...
By definition, this means that the pullback functor $Q$ of \cref{sec:frobc} lifts as follows:
\[
\xymatrix@C+3em{
...
We refer to the elements of $\cal{I}$ as \emph{generating uniform cofibrations}.
...
\end{gathered}
\end{equation}
We refer to the elements of $\cal{I}_\otimes$ as \emph{generating uniform trivial cofibrations}.
...
We now give an equivalent characterization of uniform fibrations, which does not refer to generating uniform cofibrations.
...
It remains to separately for $k \in \braces{0\,,1}$ construct a pullback of categories as follows:
\[
\xymatrix@C+1em{
  \liftr{\cal{I}}
  \ar[d]_{\liftr{(\delta^k \hatotimes u)}}
  \ar[r]
  \pullback{dr}
&
  \liftr{\cal{I}}
...
\\
  \cal{E}^\to
  \ar[r]_{\hatexp(\bar{\delta}^k, -)}
&
  \cal{E}^\to
\rlap{.}}
\]
This is an instance of \cref{lift-of-leibniz-adjunction}, using the adjunction~\eqref{equ:leibniz-prod-exp}.
\end{proof}
...
The generalized functorial Frobenius condition for $(\Cof \times_{\cal{E}} \cal{S}, \Fib, \Cof \times_{\cal{E}} \cal{S})$ implies the generalized functorial Frobenius condition for $(\cal{I}_\otimes, \Fib, \TrivCof)$ using functoriality of generalized functorial Frobenius in the form of \cref{generalized-uniform-frob-functorial} with $F$ and $H$ the functors of~\eqref{thm:onedir}.
...
We discuss how, in the case of $\SSet$ and~$\CSet$, the existence of the awfs's $(\CC, \TF)$, having uniform trivial Kan fibrations as monad algebras, and $(\TC, \FF)$, having uniform Kan fibrations as monad algebras, and the Frobenius property for $(\TC, \FF)$ can can be proved constructively, \ie without using the law of excluded middle or the axiom of choice.
...
Since the functors $\delta^k \hatotimes (-)$ for $k \in \braces{0\,,1}$ do not lift to endofunctors on the double category of squares in $\cal{E}$, it is not clear how to construct a double categorical version of the generating uniform trivial cofibrations $\cal{I}_\otimes$ to generate an awfs $(\TC', \FF')$.
\end{remark}
