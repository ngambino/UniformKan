...
Over the past few years, there has been increasing interest in algebraic  weak factorisation system~\cite{bourke-garner-I,bourke-garner-II,garner:small-object-argument,grandis-tholen-nwfs}. They
...
Although algebraic weak factorisation systems were originally introduced to address issues in categorical homotopy theory, namely the 
...
colimits and lifting problems in standard weak factorisation systems~\cite[Chapter~12]{riehl-cat-homotopy}, 
 ...
 has shown that they have natural applications in Voevodsky's univalence foundations programme~\cite{voevodsky:uf}, which have 
...
systems. In particular, we obtain algebraic counterparts of some important results in Cisinski's theory of Quillen model structures on presheaf categories~\cite{cisinski-asterisque} and apply them  to solve an open problem regarding the simplicial model of univalent foundations~\cite{coquand-non-constructivity-kan,voevodsky-simplicial-model}.

We will be  interested in a property of weak factorisation systems that is known as the Frobenius condition~\cite[Definition~3.2.1]{garner:types-omega-groupoids}, since it is closely related to Lawvere's Frobenius reciprocity condition~\cite{lawvere-equality} (see \cite{clementino:frobenius} for an explanation of the connection). One of the reasons for the 
interest in the Frobenius condition, 
...
since it is analogous to the well-known Beck-Chevalley conditions~\cite{benabou-descente,lawvere-equality}.  We will show that it is possible to formulate a single condition, which we call the uniform Frobenius condition, that is equivalent to combination of the the Frobenius and Beck-Chevalley conditions.
...
equipped with a functorial cylinder. In that setting, it is possible to define general notions of uniform (trivial) fibration
...
Our first main result (\cref{thm:sset-cset-nwfs}) is that uniform trivial fibrations and uniform fibrations in presheaf categories 
...
this algebraic weak factorization system satisfies the Frobenius property. In particular, this implies
that pushforward along a uniform Kan fibration preserves uniform Kan fibrations. Hence, we solve the open
problem of proving a constructive counterpart of the fact, mentioned above, that pushforward along 
a  Kan fibration preseves  Kan fibrations, which cannot be proved constructively~\cite{coquand-non-constructivity-kan}.
It seems therefore natural to regard the independence result in~\cite{coquand-non-constructivity-kan} simply as an indication that the standard notion of a Kan fibration is not suitable for developing simplicial homotopy theory constructively, rather than as evidence that the simplicial setting is inherently non-constructive.
