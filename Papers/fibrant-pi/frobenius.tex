...
This condition, called the functorial Frobenius condition, was stated for a cloven wfs in~\cite{garner:topological-simplicial}, which is slightly a more general notion than 
that of an awfs. Let us fix some notation.
...
The next proposition shows in what sense the generalized uniform Frobenius condition is functorial in its parameters.
...
\rlap{.}}
\end{align*}
If $(\cal{I}_1, \cal{J}_1, \cal{K}_1)$ satisfies the generalized uniform Frobenius condition, then so does $(\cal{I}_2, \cal{J}_2, \cal{K}_2)$.
\qed
\end{proposition}

Next, we show that the generalized uniform Frobenius condition can be simplified under some assumptions.
...
We can now restate the functorial Frobenius condition of~\cref{functorial-frobenius} equivalently in terms of the generalized uniform Frobenius condition of~\cref{generalized-uniform-frob}.
...
\item $(\pcoalg{\LL}, \palg{\RR}, \liftl{(\liftr{\pcoalg{\LL}})})$ satisfies the generalized uniform Frobenius condition.
\end{enumerate}
\end{proposition}
...
Then the following are equivalent:
\begin{enumerate}[(i)]
\item $(\cal{I}, \cal{J}, \liftl{(\liftr{\cal{I}})})$ satisfies the generalized uniform Frobenius condition.
\item $(\liftl{(\liftr{\cal{I}})}, \cal{J}, \liftl{(\liftr{\cal{I}})})$ satisfies the generalized uniform Frobenius condition.
\end{enumerate}
\end{proposition}
...
We can finally give the desired characterization of the functorial Frobenius condition for an algebra{-}ically-free awfs.
...
\begin{enumerate}[(i)]
\item $(\LL, \RR)$ satisfies the functorial Frobenius condition,
\item $(\cal{I}, \liftr{\cal{I}}, \liftl{(\liftr{\cal{I}})})$ satisfies the uniform Frobenius condition.
\end{enumerate}
\end{theorem}
...
By \cref{double-pitchfork-in-generalized-uniform-frob}, this is equivalent to the generalized uniform Frobenius condition for $(\cal{I}, \liftr{\cal{I}}, \liftl{ (  \liftr{\cal{I}} ) }   )$.
\end{proof}
...
First, let $\mathbb{C}$ be a small category and fix $\cal{E} \defeq \Psh(\mathbb{C})$. We write $\yon \co \mathbb{C} \to \cal{E}$ for the Yoneda embedding. 
...
\begin{proof}
For $x \in \cat{C}$, define $K(x)$ to be the set of subobjects of~$\yon(x)$ that are elements of~$\cal{M}$. The factorisation 
...
Any element $i \co A \to B$ of $\cal{M}$ arises as base change of $1 \to K$ along a unique map $B \to K$, sending an element
 $b \co \yon(x) \to B$, for $x \in \cat{C}$, to the subobject of $\yon(x)$ given by the base change of~$i  \co A  \to B$ along $b$,
 which is in $K(x)$ since $i$ is in $\cal{M}$. 
