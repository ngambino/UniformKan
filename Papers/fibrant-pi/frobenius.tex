...
We define
\[
\cal{J} \defeq \braces{\delta^k \hatotimes i \mid  k \in \{ 0 \, , 1 \} \, , \;  i \in \cal{I} \,  }
\, .\]
...
We claim that $R$ is the class of fibrations, \ie that $\liftr{\cal{J}} = \liftr{\Cyl}$. We have
\begin{alignat*}{2} 
p \in  \liftr{\cal{J}} 
& \Leftrightarrow  (\forall k \in \{ 0 \, , 1 \})(\forall  u \in \cal{I}) \;  \kcyl \hatotimes u \pitchfork p  & \quad & \text{(by definition of $\cal{J}$)} \\
 & \Leftrightarrow (\forall k \in \{ 0 \, , 1 \})(\forall  u \in \cal{I}) \; u \pitchfork \hatexp(\bar{\kcyl}, p) & \quad & \text{(by adjointness)}   \\
 & \Leftrightarrow (\forall k \in \{ 0 \, , 1 \}) \; \hatexp(\bar{\kcyl}, p) \in \TrivFib  & \quad & \text{(since $\liftr{\cal{I}}= \TrivFib$)}  \\
 & \Leftrightarrow p \in \Fib \, , & \quad & \text{(by~\cref{thm:char-fib-nonalg}),}
 \end{alignat*}
 as required.
\end{proof} 
...
By condition~(S1) of \cref{thm:suitable-awfs}, it is cofibrantly generated by a small category of arrows $u \co \cal{I} \to \cal{E}^\to$.
We will construct a second awfs $(\TC, \FF)$, cofibrantly generated by a category of arrows $u_\otimes \co \cal{I}_\otimes \to \cal{E}^\to$ defined next.
...
\end{gathered}
\end{equation}
Since $\cal{I}_\otimes$ is small, it cofibrantly generates an awfs $(\TC, \FF)$ via Garner's algebraic small object argument~\cite{garner:small-object-argument} (see also~\cite[Proposition~16]{bourke-garner-I}).
Mirroring our notation for $(\CC, \TF)$, we define
\[
\TrivCof \defeq \pcoalg{\CC}
\, , \quad
\Fib \defeq \palg{\TF}
\]
and refer to the objects in $\TrivCof$ as \emph{uniform trivial cofibrations} and objects in $\Fib$ as \emph{uniform fibrations}.

Because $(\CC, \TF)$ and $(\TC, \FF)$ are algebraically free, note that we have maps over $\cal{E}^\to$
\begin{equation} \label{relating-notions-of-fibrations}
\begin{alignedat}{2123132}
\Fib &= \palg{\TF}
&\quad \leftrightarrow \quad&&
\pAlg{\TF} &\cong \liftr{\cal{I}_\otimes}
\rlap{,}\\
\TrivFib &= \palg{\FF}
&\quad \leftrightarrow \quad&&
\pAlg{\FF} &\cong \liftr{\cal{I}}
\end{alignedat}
\end{equation}
in both directions passing between pointed endofunctor and monad algebras for the monad of the respective awfs.
This will be used frequently in the remainder of the development.

We now characterize uniform fibrations directly in terms of uniform trivial fibrations, analogously to how we characterized fibrations in~\cref{thm:char-fib-nonalg}.
The following proposition uses the adjunction in~\eqref{equ:leibniz-prod-exp} and follows from the general results in \cref{sec:ortf}.

\begin{proposition} \label{prod-exp-general}
For a map $p \co X \to Y$ in $\catE$, the following are equivalent:
\begin{enumerate}[(i)]
\item $p$ is a uniform fibration,
\item $\hatexp(\bar{\delta}^0, p)$ and $\hatexp(\bar{\delta}^1, p)$ are uniform trivial fibrations.
\end{enumerate}
\end{proposition}
