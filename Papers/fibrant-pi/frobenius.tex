...
Algebraic weak factorisation systems~\cite{garner:small-object-argument,grandis-tholen-nwfs} are a variant of standard weak factorisation systems~\cite{bousfield-wfs}
that involve maps which do not just satisfy a  lifting property (as in the definition of a Kan fibration), but are equipped with additional structure, given by an explicit choice of solutions for lifting problems which is subject to a naturality condition.
...
to the work of Bourke and Garner~\cite{bourke-garner-I,bourke-garner-II}, and has found applications not only
higher-dimensional category theory~\cite{batanin-cisinski-weber,garner:globular-operator-awfs}, but also in Voevodsky's univalent foundations programme~\cite{voevodsky:uf} via the seminal work of  Bezem, Coquand, and Huber on uniform Kan fibration in categories of cubical sets~\cite{coquand-cubical-sets},
which has developed further in~\cite{awodey-cubical,cohen-et-al:cubicaltt,pitts-cubical-nominal,swan-awfs}.  

  
 Our general aim here is to develop further the theory of algebraic weak factorisation systems and provide some
 applications. In particular, we introduce the 
 notion of uniform (trivial) fibration in  a category with a functorial cylinder
and use it to construct algebraic weak factorisation systems on presheaf categories, obtaining 
 results that can be understood as algebraic counterparts of some important results of Cisinski's theory
of Quillen model structures on presheaf categories~\cite{cisinski-asterisque}.
As applications of our results, we  solve an open problem in univalent foundations, namely that of proving a constructive version of the
 fact that pushforward along Kan fibrations preserves Kan fibrations ~\cite{coquand-non-constructivity-kan,voevodsky-simplicial-model}, and obtain a new proof of the right 
 properness of the Quillen model structure for Kan complexes~\cite{quillen-homotopical}.
 This proof avoids entirely the use of topological realization~\cite{hovey-model-categories} and
...
both a version of the usual Frobenius condition for maps, but  also a counterpart of it for morphisms of maps (\ie commutative squares),  which can be understood as a form of the Beck-Chevalley condition~\cite{benabou-descente,lawvere-equality}.\footnote{In~\cite{garner:topological-simplicial},
...
isolate conditions on the generating category of an algebraically-free algebraic weak factorisation system that are necessary and sufficient for the algebraic weak factorisation system to satisfy the functorial Frobenius condition. 
In order to do so, we introduce what we call the uniform Frobenius condition.

The main result in this paper  (\cref{frobenius-uniform-presheaf}) establishes the existence of algebraic weak factorisation systems with the functorial Frobenius condition on presheaf categories
...
For our third main result, we return to consider the general setting categories equipped with a functorial cylinder with connections and show that, under mild assumptions, uniform fibrations satisfy the uniform Frobenius condition (\cref{uniform-fibrations-uniform-frobenius}). As a direct application, we obtain that the algebraic weak factorisation systems on presheaf categories with uniform fibrations as right maps constructed earlier in the paper satisfy the uniform Frobenius condition (\cref{frobenius-uniform-presheaf}).
...
preserves uniform Kan fibrations. There are at least two reasons for the interest in this result. 
First,  since our proof is constructive, \ie avoids the the use of the law of excluded middle and the axiom of choice, 
we solve the open problem of obtaining a constructive counterpart of the fact, mentioned above, that pushforward along a  Kan fibration preserves  Kan fibrations, which cannot be proved constructively~\cite{coquand-non-constructivity-kan}. Because of this, it seems therefore natural to regard the independence result in~\cite{coquand-non-constructivity-kan} simply as an indication that the standard notion of a Kan fibration is not suitable for developing simplicial homotopy theory constructively, rather than as evidence that the simplicial setting is inherently non-constructive. Secondly, it gives a new proof of the right properness of the Quillen model structure for Kan complexes.
...
\subsection*{Remarks} For brevity, we will not always discuss the special cases  of our results for algebraic
weak factorisation systems  for standard weak factorisation systems .
Such results can be readily stated  by replacing 
...
which we leave for future investigation.} For the results in~\cref{sec:non-alg}, where we show how the classical notion of a fibration is equivalent to the uniform one, the use of the axiom of choice inseems unavoidable.


\subsection*{Organization of the paper}
Section~\ref{sec:ortf} establishes basic facts about orthogonality functors.  In Section~\ref{sec:frobc}.
we introduce the Frobenius, Beck-Chevalley, and uniform Frobenius conditions and related them
...
\cref{exa:composition-pullback-lift} below will be used when the Beck-Chevalley conditions are introduced in~\cref{sec:frobc}.
...
We will not need  the evident generalization of \cref{lift-of-mates} to Leibniz constructions.
...
a characterization of the functorial Frobenius condition for algebraically-free algebraic weak factorisation systems.
...
to assemble all the ingredients of the uniform Frobenius condition into a single global functor, as we do in~\cref{uniform-frob-nice}, which seems the most natural way to phrase the condition in our setting.
...
In the context of standard weak factorisation systems, \cref{uniform-frobenius-functorial} implies that,
...
We now fix a suitable category of monomorphisms $\cal{M}$ and write~$u \co \cal{M} \hookrightarrow \catE^\to$ to denote the inclusion. In this context, it is useful to refer to a  right $\cal{M}$-map as a \emph{uniform trivial $\cal{M}$-fibration} and define the category of uniform trivial $\cal{M}$-fibrations  by letting
\[
...
\rlap{,}}
\]
where $i$ and $j$ are in $\cal{M}$ and the square on the left is a pullback. The category of uniform  $\cal{M}$-fibrations, 
which are defined by instantiating~\cref{def:I-fibration}, is given by letting
\[
...
We wish to show that every (trivial) fibration  can be equipped with the structure of a uniform (trivial) fibration. We begin by proving that every trivial fibration  can be equipped with the structure of a uniform trivial fibration. This will shown in two steps: first,  we prove that 
every trivial fibration admits the structure of a regular trivial fibration (\cref{trivial-fibration-to-regular}); secondly, we prove
...
This section and the next one are devoted to establish results on
the uniform Frobenius condition for uniform fibrations, leading to the
proof that the algebraic weak factorisation system on presheaf categories
...
In this section we show that the algerbraic weak factorization system in which the right maps
are the uniform fibrations (constructed in~\cref{thm:sset-cset-nwfs}) satisfies the functorial Frobenius property.
For this, we will use our characterisation of the functorial Frobenius property in algebraically
free algebraic weak factorisation systems, in~\cref{thm:frobenius-comparison}. 
...
Let us summarize the result above as a theorem, for reference.
...
We can apply the above results to uniform fibrations in presheaf categories,  studied in~\cref{sec:unif}.
...
We consider this a constructive counterpart of the result that the pushforward along a Kan fibration preserves Kan fibrations (a fact equivalent to right properness for the model structure, see also \cite[Lemma 4.3.1]{voevodsky-simplicial-model}), which cannot be proved constructively~\cite{coquand-non-constructivity-kan}.
...
of this statement should follow by an argument analogous to~\cite{coquand-non-constructivity-kan}.
\end{example}
...
Finally, let us remark that it is possible to prove right properness without taking the algebraic approach adopted in 
