...
results in two running examples. The first the is the usual category $\SSet$ of simplicial sets, defined as the category of 
...
the cube category $\Box$ with objects of the form $\Box^A$ with $|A| \in \mathbb{N}$ and morphisms~$\Box^A \to \Box^B$ given by functions from $B$ to the free de Morgan algebra on $A$. Thus, $\CSet$ has symmetries, diagonals, connections, and involutions. The category $\Box$ has a symmetric monoidal structure, which makes cubical sets into a symmetric monoidal closed category.\footnote{Note that the presence of symmetries precludes $\Box$ from being a Reedy category.} We stress that the only feature of the cube category $\Box$ relevant for the main results  are connections and symmetries.
...
Let us now assume that $\catE$ is equipped with a functorial cylinder $\interval\otimes (-) \co \catE \to \catE$ with contractions and connections, and that the functor $\interval\otimes (-) \co \catE \to \catE$ has a right adjoint, which we write~$\exp(I, -) \co \catE \to \catE$.  As a consequence, $\interval\otimes (-) \co \catE \to \catE$ preserves colimits. In order to define the last ingredient of the axiomatic setting in which we will work, namely the the notion of a suitable weak factorisation system, we need to introduce some notation.  For $k \in \{ 0, 1\}$ and a map $i \co A \to B$ in $\catE$, we define the \emph{Leibniz product} of~$\kcyl$ and~$i$ to be the map 
\[
...
When working with a suitable weak factorisation system, we refer to maps in $L$ as \emph{cofibrations} and to maps in $R$ as \emph{trivial fibrations} in order to help the intuition.
Indeed, a wide class of examples of suitable weak factorisation systems arises from Cisinski model structures~\cite{cisinski-asterisque}, \ie cofibrantly generated Quillen model structures on presheaf categories in which the cofibrations are the monomorphisms. In particular, for every presheaf categories over an elegant Reedy category~\cite{bergner-rezk-elegant}, we have a suitable weak factorisation system 
cofibrantly generated by the boundary inclusions.  

Let us now fix a suitable weak factorisation system $(L,R)$ on $\catE$. We wish to define a new weak factorisation system, to be thought of as consisting of trivial cofibrations and fibrations. For this, we begin by defining the class of maps, called \emph{cylinder inclusions}, that will determine the notion of a fibration. This class is defined by letting
\[
\mathsf{Cyl}  \defeq \{ \kcyl \hatotimes i \ | \ k \in \{ 0, 1 \} \, , \; i \in L \}
\]

\begin{definition} A \emph{fibration} is a map with the right lifting property with respect to all cylinder inclusions. 
\end{definition}

The next proposition is straightforward, but we state it explicitly it for future reference. 

\begin{proposition} \label{thm:wfstimes} There exists a weak factorisation system $(L_\otimes, R_\otimes)$ on $\cal{E}$ whose right 
maps are precisely the fibrations.
\end{proposition}

\begin{proof} Let $I$ be a generating set of maps for the weak factorisation system $(L, R)$, which we assumed to be cofibranly generated,
and define the set  $J \defeq \{ \delta^k \hatotimes i \ | \ i \in I \}$. 
This set cofibrantly generates a weak factorisation system $(L_\otimes, R_\otimes)$. We claim that $R_\otimes$ is precisely the class of
fibrations, \ie that $J^\pitchfork = \mathsf{Cyl}^\pitchfork$. For this, it suffices to observe that, since $L$ is the saturation of~$I$, the class
$\mathsf{Cyl}$ and the set~$J$ have the same saturation, and therefore they have the same right orthogonal class.
\end{proof}

We refer to the maps in $L_\otimes$ as \emph{trivial cofibrations}. Note that, since $L$ is  closed under Leibniz products with the endpoint inclusions, we have that $L_\otimes \subseteq L$, which could be rephrased as saying that every trivial cofibration is a cofibration. 
...
Indeed, a fibration $p \co Y \to X$ is a map for which there exist diagonal fillers for diagrams of the form
\[
\xymatrix{
...
\rlap{,}}
\]
where the map on the left is the Leibniz product of the endopoint inclusion $h_k^1 \co \braces{k} \to \Delta^1$ with a monomorphism $i \co A \to B$. The equivalence between this notion and the standard notion of a Kan fibration is well-known and follows from the fact that an higher dimensional horn $h_i^n$ is a retract of the Leibniz product $\kcyl \hatotimes h_i^n$~\cite[Chap.~IV, Sec.~2]{gabriel-zisman:calculus-of-fractions}.
\end{example}

\begin{example}[Cubical sets]
In $\CSet$ we consider the weak factorisation system $(L, R)$, where $L$  consists of all monomorphisms.
...
Let us begin by recalling the Frobenius property for weak factorisation systems.
...
an $R$-map is again an $L$-map, \ie for every pullback diagram of the form
\[
\xymatrix{
 Y \ar[d]_j \ar[r]^q & X \ar[d]^i \\ 
B \ar[r]_p & A }
\]
where $p \in R$, if $i  \in L$ then $j \in L$.
\end{definition} 

As mentioned in the introduction, for a model structure~$(\cal{W}, \cal{F}, \cal{C})$ in which the cofibrations are the monomorphisms,
right properness, \ie that the pullback of a weak equivalence along a fibration is again a fibration, is equivalent to the Frobenius property of the weak factorisation system~$(\cal{W} \cap \cal{C}, \cal{F}$), \ie that the pullback of a trivial cofibrations along a fibration is again a trivial cofibration. In particular, the right properness of the model structure on $\SSet$ for Kan complexes is equivalent to the Frobenius property
of the weak factorisation system of trivial cofibrations and Kan fibrations, \ie that the pullback of a trivial cofibration along a Kan fibration
is again a trivial cofibration.

Our aim in this section is to show that the weak factorisation system $(L_\otimes, R_\otimes)$ of  \cref{thm:wfstimes} satisfies the Frobenius property. Our proof relies on the notion of a emph{strong homotopy equivalence}, which we introduce in~\cref{def:strhe}, and consists of three main steps. First, we will give a characterization of strong homotopy equivalences as retracts of certain canonical maps. Secondly, we show that the maps in $L$ which are strong homotopy
...
a fibration if and only if it has the right lifting property with respect to the strong homotopy equivalences in $L$. The third and final step of the proof will consist in showing that the pullback
of a $(S \cap L)$-map along a fibration is again an $(S \cap L)$-map, which can be  easily seen to imply the Frobenius condition for $(L_\otimes, R_\otimes)$.
...
Examples of strong $k$-oriented homotopy equivalences are given by strong deformation retracts, i.e. homotopy equivalences for which the homotopy~$\phi$ is trivial, \ie $\phi = \varepsilon_X$, where $\varepsilon \co I \otimes (-) \to \Id_\catE$ is the contraction of the functorial cylinder. 
...
Note that \cref{strong-h-equiv-as-section} implies that strong $k$-oriented homotopy equivalences are closed under retracts since functors, in this case $\delta^k \hatotimes (-)$, preserve retracts and sections are closed under retracts.
For our second step, we focus on the class $L \cap S$, \ie the class of maps in $L$ that are strong $k$-oriented homotopy equivalences,
for $k = 0, 1$. We begin by relating them with the maps in $L_\otimes$, as defined in~\eqref{thm:wfstimes}.

\begin{lemma}  \label{thm:main-sheretract}\hfill 
\begin{enumerate}[$(i)$] 
\item \label{lem:from-strong-hequiv}
$L \cap S \subseteq \overline{L_\otimes}$.
\item \label{lem:to-strong-hequiv}
$L_\otimes \subseteq L \cap S$.
\end{enumerate}
\end{lemma}

\begin{proof}
Part (i) follows from~\cref{strong-h-equiv-as-section-non-alg}. For part (ii), since $L$ is closed under Leibniz product with~$\kcyl$, we have $L_\otimes \subseteq L$.
...
A map is a fibration if and only if it has the right lifting property with respect to  $S \cap L$.
\end{proposition}
...
\end{gathered}
\end{equation}
where $p \in R_\otimes$, if $i  \in L_\otimes$ then $j \in L_\otimes$. For this, let us assume that
 $i \in L \cap S_k$ and show that  $j \in L \cap S_k$. Note that $i \in S_k$ implies that $\thetak \hatotimes i \co i \to \kcyl \hatotimes i$ has a retraction $\rho$, as follows:
\begin{align*}
\xymatrix@C+1em{
  i 
...
\rlap{.}}
\end{align*}
Also,  since $L$ is assumed to be closed under pullback, it suffices to show
only that $j \in S_k$.
...
\rlap{.}}
\]
We define the retraction $\tau$ so that the diagram below, where $\sigma \co j \to i$ is 
...
\rlap{.}}
\]
But we have such a filler since $\kcylinv \otimes Y \in L_\otimes$ and $p \in R_\otimes$. 
\end{proof}
