...
Fixing $k \in \braces{0, 1}$, we write $\cal{S}_k$ for the class of maps $f$ in $\cal{E}$ that admit data $(g, \phi, \psi)$ as above making $f$ into a strong $k$-oriented homotopy equivalence.
...
For an arrow $f \in \cal{E}^\to$, we have $f \in \cal{S}_k$ if and only if there is a retraction $\theta$ of the map $\thetak \hatotimes f$, as below:
\[
\xymatrix@C+2em{
  f
