...
Our first main result (\cref{thm:ac-kan-is-uniform}) shows in what sense the notion of a uniform fibration subsumes the classical notion of a fibration.
...
This result is not a simple application of the axiom of choice since the function that is required to exist in order to have a uniform fibration needs to satisfy an appropriate naturality condition.
...
Our second main result (\cref{thm:sset-cset-nwfs}) is that in presheaf categories satisfying a suitable presentability assumption (but not necessarily over an elegant Reedy category) uniform trivial fibrations and uniform fibrations are the right maps of two natural weak factorization systems.
...
This type of good interaction between colimits and lifting problems in the theory of natural weak factorization systems was already emphasized in~\cite{riehl-cat-homotopy}.
...
\footnote{
The results obtained in this article will hold analogously for the subcategory $\cal{M}'$ of all monomorphisms and pullback squares, or more generally for any full subcategory of $\cal{M}'$ that satisfies the premise of \cref{awfs-on-arrows-into-representables} and certain other closure properties.
However, we expect that the restriction to decidable monomorphisms will play an important role in future constructive developments.
}
...
In this subsection, the only one in which the law of excluded middle and the axiom of choice are assumed, we study the notion of a uniform (trivial) fibration in the context of presheaf categories over elegant Reedy presheaves, in the sense of \cite{bergner-rezk-elegant}, equipped with a functorial cylinder whose endofunctor has a right adjoint.
...
Our main result is that these categories can be also be defined as right orthogonal with respect to small categories over $\catE$.
...
Let us point out that by the above result we have $\TrivFib = \liftr{\cal{S}}$ for every full subcategory $\cal{S} \subseteq \cal{M}$ containing~$\cal{N}$ since~$\cal{N}$ is then also the restriction to maps into representables of $\cal{S}$.
...
\subsection*{Existence of natural weak factorization systems}

\cref{small-gen-triv-kan} allows us to apply Garner's small object argument~\cite{garner:small-object-argument} and obtain two natural weak factorization systems in which the right maps are the uniform trivial fibrations and the uniform fibrations, respectively.
...
In the above setting, we have that $\Fib[\cal{I}] = \liftr{(\cal{I}_\otimes)}$ satisfies the uniform Frobenius property with respect to $\cal{I}_{\otimes}$.
\end{theorem}
...
\rlap{.}}
\]
The dotted arrows were constructed by expanding the back and front face of this diagram to the total space of the codomain fibration and then appealing to the universal property of the Cartesian squares from $\cod(\sigma_1)$ and $\cod(\sigma_2)$ to $p$, respectively.
