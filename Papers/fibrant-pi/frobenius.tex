...
\section{Cylinders, homotopies and connections}
\label{sec:cylhuf}
...
We now introduce some important examples to which our results will apply.
...
Furthermore, instead of starting from a mere class of arrows in $\catE$ and defining its left or right orthogonal class, we consider a category~$\cal{I}$, to be thought of as an indexing category (but not assumed to be small), and a functor~$u \co \cal{I} \to \catE^\to$.
...
\rlap{.}}
\]
In the non-algebraic setting, it is well known that if we have classes of maps $\cal{I} \subseteq \catE^\to$ and~$\cal{J} \subseteq \cal{F}^\to$, then $F(\cal{I}) \subseteq \liftl{\cal{J}}$ if and only if $\liftr{\cal{I}} \subseteq G(\cal{J})$.
...
To illustrate the connection with the classical Kan filling condition for cubes, let $i^n \co \partial \Box^n \to \Box^n$ for $n \in \mathbb{N}$ denote the boundary inclusion for~$\Box^n$, given as the $n$-fold Leibniz tensor of $[\lcyl, \rcyl] \co \braces{0, 1} \to \Box^1$.%
...
\subsection*{Homotopies and strong homotopy equivalences}
...
These functors will in general not be inverses in either way.
\end{remark}



\subsection*{The uniform Frobenius condition for uniform fibrations}
\label{sec:frocuf}
