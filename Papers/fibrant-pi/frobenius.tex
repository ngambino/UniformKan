...
Then, a fibration in our sense is the same thing as a Kan fibration in the usual sense (proven in~\cite[Chap.~IV, Sec.~2]{gabriel-zisman:calculus-of-fractions}).
Explicitly, a fibration in our sense is a map with the right lifting property with respect to all maps of the form
\[
...
Note that this also induces lifts against any higher dimensional horn $h_i^n$ since $h_i^n$ arises as a retract of the Leibniz product $\kcyl \hatotimes h_i^n$.
\end{example}
...
Since the Leibniz product $\kcyl \hatotimes (-)$ preserves all operations involved in saturation, the class $\Cyl$ and the set~$J$ have the same saturation (since the Leibniz product $\kcyl \hatotimes (-)$ preserves all the operations involved in saturation); they therefore have the same right orthogonal class.
\end{proof}
...
For $k \in \braces{0, 1}$, a map $f \co X \to Y$ is called a \emph{strong $k$-oriented homotopy equivalence} if there is a map $g$ and homotopies $\phi$ and $\psi$ making $f$ into a
...
\begin{proof}
For part~(i), we have~$\Cyl \subseteq \Cof$ since cofibrations are closed under Leibniz product with~$\kcyl$ (condition (S4) in \cref{thm:suitable-wfs}).
...
Here, $\sigma \co j \to i$ is the pullback in~\eqref{non-alg-strong-h-equiv-uniform-base-change:0} and the horizontal composited should be identities:
\[
\xymatrix@C+2em{
j
...
\begin{remark}
When applied to the category of simplicial sets, \cref{thm:non-alg-frobenius} gives a new proof of right properness of the Quillen model structure for Kan complexes.
...
\footnote{Let us note also the possibility of a structural proof, part of a previous version of this paper and easily illustrated as follows in case the functorial cylinder comes from an interval object in a monoidal category.
Using reasoning as in \cref{thm:kcylf-is-she}, we have that $\delta^k$ is a strong $k$-oriented homotopy equivalences.
From the description of \cref{strong-h-equiv-as-section-non-alg}, strong $k$-oriented homotopy equivalences can be described as maps $f$ together with a retraction of $\theta^k \hatotimes f$ where $\theta^k \co \bot_1 \to \delta^k$ is the map in $\cal{E}^\to$ given by the square
\[
\xymatrix{
  0
  \ar[r]
...
\rlap{.}}
\]
The conclusion now follows from closure of retracts under functors, in this case pushout product with a map on the right, and associativity of pushout product.}
\end{proof}
