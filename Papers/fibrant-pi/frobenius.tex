...
Since the proof of these results for simplicial sets and cubical sets is constructive, \ie avoids the use of the law of excluded middle and the axiom of choice, we solve the open problem of obtaining a constructive version of the result asserting that, in simplicial sets, pushforward along Kan fibrations preserves of Kan fibrations. This problem was motivated by the question of whether the simplicial model of univalent foundations in~\cite{voevodsky-simplicial-model} can be developed constructively.  As shown in~\cite{coquand-non-constructivity-kan}, this is not possible since the result on pushforward along Kan fibrations,
which is essential to interpret the rules for $\Pi$-types, cannot proved constructively. This independence result has led the research 
community to shift its focus on models based on cubical sets and uniform Kan 
fibrations~\cite{awodey-cubical,coquand-cubical-sets,cohen-et-al:cubicaltt,pitts-cubical-nominal,swan-awfs}. Indeed,
in that setting it is possible to prove constructively that pushforward along a uniform Kan fibration preserves uniform Kan fibrations, leading
to the definition of a constructive model of univalent foundations~\cite{cohen-et-al:cubicaltt}. As our results show, the key aspect that makes 
it possible to obtain such a result is the idea of working with the algebraic notion of a uniform fibration, 
rather than that of focusing on cubical sets.
...
We then use this characterization to show in what sense the notion of a uniform (trivial) fibration subsumes the standard notion of a (trivial) fibration (\cref{unif-vs-non-unif}) In particular, we prove that, in a presheaf category, a map can be equipped with the structure of a uniform (trivial) fibration if and only if it is a (trivial) fibration. As we observe in~\cref{why-uniform}, however,
the algebraic notion of a fibration is essential for our results in~\cref{sec:alg-fib} to hold constructively. 
...
For $k \in \{ 0, 1\}$ and a map $i \co A \to B$ in $\catE$, we define the \emph{Leibniz product}~\cite{riehl-verity:reedy} of the endpoint inclusion~$\kcyl \otimes (-)$ and~$i$ to be the map
\[
...
The statement of~\cref{def:uniFrobcond} makes it clear that the generalized uniform Frobenius condition requires the specification of additional structure, rather than the mere satisfaction of a property.
...
\begin{enumerate}[(i)]
\item $(\LL, \RR)$ satisfies the functorial Frobenius condition,
\item $(\cal{I}, \liftr{\cal{I}}, \liftl{(\liftr{\cal{I}})}) = (\cal{I}, \pAlg{\RR}, \pcoalg{\LL})$ satisfies the generalized uniform Frobenius condition.
\end{enumerate}
\end{theorem}
...
\notec{No, they not --- our categories of generating cofibrations are different. I actually think that Coquand's category is more interesting. Perhaps we should default to theirs instead of all monos for cubical sets, so that it is more results how our setting relates to their setting?}
...
As suggested by our notation, the maps~$\thetak \hatotimes f \co f \to \kcyl \hatotimes f$ are really given as the action on objects of a functor $\thetak \hatotimes (-)$ as follows.
Let $\theta^k \co \bot_{\Id} \to \delta_k \otimes (-)$ denote the arrow in $[\cal{E}, \cal{E}^\to] = [\cal{E}, \cal{E}]^\to$ given by the following square:
\[
\xymatrix@C+1em{
  0
  \ar[r]^{\bot_{\Id}}
...
In fact, this is the action on objects of a lift of $\kcyl \otimes (-)$ to $\cal{S}_k$:
\[
\xymatrix@C+3em{
&
  \cal{S}_k
...
We will use that associativity of functor composition lifts to associativity of the corresponding Leibniz constructions if the involved functors preserve pushouts.
Starting with \cref{thm:endpoint-are-she-functorially}, a lift of $\delta^k$ to $\cal{S}_k$ is given by a retraction of $\theta^k \hatotimes (\delta^k \otimes (-)) = \theta^k \mathbin{\hat{\cc}} \delta^k$.
Our goal, a lift $\delta^k \hatotimes (-)$ to $\cal{S}_k$, our goal, is given by a retraction of $\theta^k \hatotimes (\delta^k \hatotimes (-)) = (\theta^k \mathbin{\hat{\cc}} \delta^k) \hatotimes (-)$.
But functors, in this case $m \mapsto m \hatotimes (-)$, preserve section-retraction pairs.
\end{proof}
...
As a step, we show that that strong homotopy equivalences and uniform fibrations satisfy the generalized uniform Frobenius condition, \ie the algebraic analogue of \cref{thm:non-alg-frobenius-she}.
...
\begin{proof}
First, we show that $(\Cof \times_{\cal{E}} \cal{S}, \Fib, \Cof \times_{\cal{E}} \cal{S})$ satisfies the generalized uniform Frobenius condition.
This follows from \cref{generalized-uniform-frob-product-u} since $(\Cof, \Fib, \Cof)$ satisfies the generalized uniform Frobenius condition by hypothesis~(S3) in the definition of a suitable awfs (\cref{thm:suitable-awfs}) and $(\cal{S}, \cal{S}, \Fib)$ satisfies the generalized Frobenius condition by \cref{technical}.

Let the awfs $(\mathsf{C_t}, \mathsf{F})$ be algebraically free on $\cal{I}_\otimes$ as in the proof of \cref{thm:sset-cset-nwfs}.
The generalized uniform Frobenius condition for $(\Cof \times_{\cal{E}} \cal{S}_k, \Fib, \Cof \times_{\cal{E}} \cal{S}_k)$ implies the generalized uniform Frobenius condition for $(\cal{I}_\otimes, \Fib, \TrivCof)$ using functoriality of generalized uniform Frobenius in the form of \cref{generalized-uniform-frob-functorial} with $F$ essentially the functor in part~\eqref{thm:onedir} of~\cref{thm:strong-hequiv} and $H$ the functor in part~\eqref{thm:twodir} of~\cref{thm:strong-hequiv}.
...
\footnote{This assumption not used in the instantiation of our general results to this setting, but only for the separate \cref{partial-map-classifier,unif-vs-non-unif}.z}
\end{enumerate}
...
of the existence of a subobject classifier), \cref{rem-lift-suitable} establishes the existence of the awfs of \cref{unif-triv-fib-sset}.
\notec{Different setting!}
\end{example}
