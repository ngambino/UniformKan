...
often defined using a monoidal structure and an interval object, as in \cref{exa:cyl-via-int} below.
...
\rlap{.}}
\end{gather*}
Note that, since $\cal{I}$ is assumed to be closed under Leibniz products with the endpoint inclusions, we have $\cal{I}_\otimes
\subseteq \cal{I}$. We then define an \emph{$\cal{I}$-fibration} to be a map with the right lifting property with respect to $\cal{I}_\otimes$
and we write $\Fib{\cal{I}}$ for the class of  $\cal{I}$-fibrations, 
\[
  \Fib{\cal{I}} \defeq \liftr{(\cal{I}_\otimes)} \rlap{.}
\]
We now illustrate the notion of a (trivial) $\cal{I}$-fibration in simplicial sets and cubical sets.
...
We briefly relate the notion of an $\cal{I}$-fibration with the notion of a naive fibration introduced by Cisinski~\cite{cisinski-asterisque}.
