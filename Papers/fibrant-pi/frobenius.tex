...
Algebraic weak factorisation systems~\cite{garner:small-object-argument,grandis-tholen-nwfs}
were originally introduced in order to improve on some aspects of standard weak factorisation systems~\cite{bousfield-wfs}, such as the unsatisfactory interaction between colimits and lifting problems, as discussed in~\cite[Chapter~12]{riehl-cat-homotopy}.  In recent years, there has been increasing interest in algebraic  weak factorisation systems (see~\cite{bourke-garner-I,bourke-garner-II}, for instance), also because the seminal work of  Bezem, Coquand, and Huber on uniform Kan fibration in categories of cubical sets~\cite{coquand-cubical-sets} 
 has shown that they have natural applications in Voevodsky's univalent foundations programme~\cite{voevodsky:uf}, which have been developed  in~\cite{awodey-cubical,cohen-et-al:cubicaltt,pitts-cubical-nominal,swan-awfs}.  The key aspect that
 differentiates algebraic from standard  weak factorisation systems is
 that the latter involve maps that merely satisfy a left (or right) lifting property,
as in the definition of a Kan fibration, while the former involve maps equipped with 
additional structure, consisting of an explicit choice of solutions for  left (or right) lifting problems
...
and use it to obtain  counterparts of some important results of Cisinski's theory of Quillen model structures on presheaf categories~\cite{cisinski-asterisque}. We  apply these results  to solve the open problem in univalent foundations, namely that of proving a constructive counterpart of the
...
\subsection*{Remark} In the hope of making the paper more widely appealing, we have
confined comments on constructivity issues to remarks and footnotes. However,
let us point out here for the readers interested in constructive aspects that, since our development is completely
algebraic, none of the results in this paper rely on the law of the excluded middle or the axiom of choice, apart  from~\cref{thm:sset-cset-nwfs} and those in~\cref{sec:non-alg}. For~\cref{thm:sset-cset-nwfs}, we argue separately that the result holds constructively for simplicial sets and
cubical sets in~\cref{thm:sset-cset-nwfs-constr}.\footnote{A constructive proof 
...
that we leave for future investigation.}
 The use of the axiom of choice in~\cref{sec:non-alg}, where we show how the classical notion of
a fibration is equivalent to the uniform one, seems unavoidable.
...
<<<<<<< HEAD
\begin{remark} \label{thm:sset-cset-nwfs-constr}
=======
