...
Thus,~$\CSet$ has symmetries, diagonals, connections, and involutions.
...
When $\cal{E}$ is a category of presheaves over an elegant Reedy category~\cite{bergner-rezk-elegant}, a map has the right lifting property with respect to monomorphisms if and only if it has it with respect to boundary inclusions, since every monomorphism is an $\omega$-composition of cobase changes of coproducts of boundary inclusions of simplices.
\end{example}
...
\rlap{.}}
\]
But now $\kcyl \otimes X \iso \kcyl \hatotimes \bot_X$, where $\bot_X \co 0 \to X$ is the unique map from the initial object of $\catE$ into $X$, which is a cofibration by the assumption that $(\Cof, \TrivFib)$ is suitable (condition~(S2) of \cref{thm:suitable-wfs}).
