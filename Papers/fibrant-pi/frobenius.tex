...
In the below diagram, the premise corresponds to a functor $Q_2$ making the inner square commute while the conclusion corresponds to a functor $Q_1$ making the lower trapezoid commute:
\[
\xymatrix{
&
...
\rlap{.}}
\]
We may construct $Q_1$ from $Q_2$ by pasting the inner square with the left triangle, given by $G$ and $H$, and the right triangle, given by $F$ and the multiplication of the monad $\liftl{(\liftr{(-)})}$.
\end{proof}
...
We finish by applying \cref{uniform-frobenius-functorial} with $F$ the unit of the monad $\liftl{(\liftr{(-)})}$ and $G$ the identity on $\cal{S}_0(\cal{I}) + \cal{S}_1(\cal{I})$.
\end{proof}
...
\begin{proof}
The result follows from \cref{strong-h-equiv-uniform-frobenius-fibrations} and \cref{uniform-frobenius-functorial} using for $F$ and $G$ the functors constructed in \cref{lem:from-strong-hequiv,lem:to-strong-hequiv}, respectively (\cf \cref{relating-strong-hequiv-and-uniform-fib}).
\end{proof}
