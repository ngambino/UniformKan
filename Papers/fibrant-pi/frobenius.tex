...
Since the functors $\delta^k \hatotimes (-)$ for $k \in \braces{0\,,1}$ do not lift to an endofunctor on the double category of squares in $\cal{E}$, it is not clear how to construct a double categorical version of the generating uniform trivial cofibrations $\cal{I}_\otimes$ to generate an awfs $(\TC', \FF')$.
\end{remark}
