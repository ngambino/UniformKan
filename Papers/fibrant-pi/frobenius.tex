...
of $\top \co 1 \to \Omega$ through $K$ follows since identities are contained in $\cal{M}$ by \cref{identities-in-M}. 

Any element $i \co A \to B$ of $\cal{M}$ arises as base change of $1 \to K$ along a unique map $B \to K$, sending an element
 $b \co y(c) \to B$, for $c \in \cat{C}$, to the subobject of $y(c)$ given by the base change of~$i  \co A  \to B$ along $b$,
 which is in $K(c)$ since $i$ is in $\cal{M}$. 
For the converse, it suffices to show that~$1 \to K$
is in $\cal{M}$, but this is equivalent to (M5).
\end{proof}
...
In this case, if a map has the right lifting property with respect to boundary inclusions then it is a trivial fibration, since
every monomorphism is an $\omega$-composition of cobase changes of coproducts of boundary inclusions of simplices.
By  \cref{unif-vs-non-unif}, such a map is then a uniform Kan fibration.
\end{example}
