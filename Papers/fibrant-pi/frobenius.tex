...
For example, for a class $J$ of maps in $\cal{E}$, we write $\liftr{J}$ for the class of maps having the right lifting property with respect to all maps in $J$.
As is well-known, we have inclusions
\begin{equation} \label{equ:inclus-orthog}
J \subseteq \liftl{(\liftr{J})}
\, , \quad
J \subseteq \liftr{(\liftl{J})}
\, .\end{equation}
...
We say that a wfs $(L, R)$ is \emph{free} on a class of maps $J$ (not necesssarily a set) if $R = \liftr{J}$.
If this is the case, then $L = \liftl{(\liftr{J})}$.
Of course, a wfs could be free on many different classes of maps.
By Quillen's small object argument~\cite{quillen-homotopical}, every set $J$ determines a wfs $(L, R)$ free on $J$.
...
Let $(L, R)$ be a wfs that is free on a class of maps $J$.
...
\begin{proof}
Let $J$ be a generating set for the suitable wfs $(\Cof, \TrivFib)$.
We define
\[
K \defeq \braces{\delta^k \hatotimes i \mid i \in J}
\, .\]
By the small object argument, there is a wfs $(L, R)$ cofibrantly generated by $J$, where $L$ is the saturation of $K$ (\ie its closure under identities, composition, retracts, pushouts and transfinite composition) and $R = \liftr{K}$.
We claim that $R$ is the class of fibrations, \ie that $\liftr{K} = \liftr{\Cyl}$.
Recall that~$\Cof$ is the saturation of $J$.
Since the Leibniz product $\kcyl \hatotimes (-)$ preserves all operations involved in saturation, the class $\Cyl$ and the set~$K$ have the same saturation; they therefore have the same right orthogonal class.
\end{proof}
...
thus writing just $\delta^k$ for the natural transformation $\delta^k \otimes (-)$.
...
Our goal is to show that $\delta^k \hatotimes (-)$ factors $\cal{S}_k \to \cal{E}^\to$. For this, we need to show
that the arrow 
\[
