...
As  applications, we obtain a new proof that the Quillen model structure for Kan complexes is right proper, avoiding
entirely the use of topological realization and minimal fibrations, and we solve an open problem in the study of univalent type theories, proving a constructive version 
of the preservation of  Kan fibrations by pushforward along Kan fibrations.  Our results also subsume and extend work by Coquand and others on cubical sets.
\end{abstract}
...
One reason for the interest in the Frobenius condition is that it is closely related to the right properness condition for a Quillen model structure, which requires that pullback along a fibration preserves weak equivalences.
...
In the category of simplicial sets, this amounts to the preservation of Kan fibrations by pushforward along Kan fibrations, a fact that plays an important role in the simplicial model of univalent foundations~\cite[Lemma~2.3.1]{voevodsky-simplicial-model}.

Our development begins by introducing a general notion of fibration, which subsumes that of a Kan fibration in simplicial and cubical sets.
...
The presence of this algebraic structure remedies some unsatisfactory aspects of wfs's (such as the failure of orthogonality classes to be closed under all colimits), something that has proved
to be useful for applications (see~\cite{batanin-cisinski-weber,garner:globular-operator-awfs,garner-homomorphisms} or~\cite{awodey-cubical,coquand-cubical-sets,cohen-et-al:cubicaltt,pitts-cubical-nominal,swan-awfs} for examples).
...
This problem was motivated by the strategy of proving that the extension of Martin-L\"of type theory with Voevodsky's univalence axiom is consistent relatively to Martin-L\"of type theory by developing the simplicial model of univalent foundations working within a constructive theory.
...
\subsection*{Remark}
In the hope of making the paper more concise, we confine comments on constructivity issues some remarks and footnotes.
...
The main core of the paper is then organized in two parts: the first part (\cref{sec:fib,sec:frob}) studies wfs's; the second part (\cref{sec:ortf,sec:frobc,sec:alg-fib,sec:alg-frob}) studies awfs's.
...
In the second part, we begin by establishing basic facts about orthogonality functors in~\cref{sec:ortf} and then establishing necessary and sufficient conditions for an algebraically-free awfs to satisfy the functorial Frobenius condition in \cref{sec:frobc}.
The rest of the second part then proceeds in parallel with the first one part.
\cref{sec:alg-fib} (parallel to \cref{sec:fib}) introduces uniform fibrations and establishes the existence of the associated awfs, while~\cref{sec:alg-frob} (parallel to \cref{sec:frob}) proves the uniform Frobenius property for uniform fibrations.
...
Examples of such categories abound; in particular, our assumptions are satisfied not only by presheaf categories, but also by arbitrary Grothendieck toposes.
...
Thus,~$\CSet$ has symmetries, diagonals, connections, and involutions. Note that the presence of symmetries precludes $\Box$ from being a Reedy category. We stress that the only feature of the category $\Box$ relevant for the main results are connections and symmetries.
...
As it is well-known, we have inclusions
...
We state it since it is used in the proof of~\cref{thm:non-alg-frobenius} and provides the non-algebraic counterpart of \cref{thm:frobenius-comparison}, which is the main result of \cref{sec:frobc}.
...
For $k \in \{ 0, 1\}$ and a map $i \co A \to B$ in $\catE$, we define the \emph{Leibniz product} of the endpoint inclusion~$\kcyl \otimes (-)$ and~$i$ to be the map
\[
...
When $\cal{E}$ is a category of presheaves over an elegant Reedy category~\cite{bergner-rezk-elegant}, 
a map has the right lifting property with respect to boundary inclusions then it is a trivial fibration, since
...
We define 
\[
J \defeq \{ \delta^k \hatotimes i \ | \ i \in I \} \rlap{.}
\]
...
First, in \cref{strong-h-equiv-as-section-non-alg}, we give a characterization of strong homotopy equivalences as certain retracts.
...
The third and final step of the proof, in~\cref{thm:non-alg-frobenius-she}, consists in showing that the pullback of a strong homotopy equivalence along a fibration is again an strong homotopy equivalence. The combination of these facts then implies easily the Frobenius condition for $(\TrivCof, \Fib)$ of \cref{thm:non-alg-frobenius}.
...
For part~(ii), recall that, since cofibrations are closed under Leibniz product with~$\kcyl$ (condition (S4) in \cref{thm:suitable-wfs}), we have~$\Cyl  \subseteq \Cof$.
...
\kcyl \hatotimes \bot_X$, where $\bot_X \co 0 \to X$ is the unique map from the initial object of $\catE$ into $X$, which is a cofibration by the assumption that $(\Cof, \TrivFib)$ is suitable (condition~(S2) of \cref{thm:suitable-wfs}).
...
The second part of the paper follows closely the first one, but in this section and the next we will need to recall and establish some facts about the algebraic setting that are useful for our development.
...
for which we now introduce some notation.
...
Then $\liftr{F} = \liftr{G}$ and $\liftl{F} = \liftl{G}$. \qed
\end{proposition}
...
In this section we obtain an analogue of~\cref{thm:frobenius-equivalence} for awfs's, \ie a necessary and sufficient condition for an algebraically free awfs to satisfy the functorial Frobenius condition, expressed purely in terms of a generating category.
...
We say that $(\cal{I}, \cal{J})$ satisfies the \emph{uniform Frobenius condition} if $(\cal{I}, \cal{J}, \liftl{(\liftr{\cal{I}})})$ satisfies the generalized uniform Frobenius condition.
\end{definition}
...
Let $u \co \cal{I} \to \cal{E}^\to$ be a functor, let $v \co \cal{J} \to \cal{E}^\to$ be a functor such that
...
\end{gathered}
\end{equation}
is a morphism of Grothendieck fibrations, and let $w \co \cal{K} \to \cal{E}^\to$ be a functor  that reflects isomorphisms.
Then the following are equivalent:
\begin{enumerate}[(i)]
...
We observe that both (i) and (ii) are equivalent to $(\pcoalg{\LL}, \palg{\RR}, \pcoalg{\LL})$ satisfying the generalized uniform Frobenius condition. For the equivalence with (i), 
use \cref{generalized-uniform-frob-nicer}, noting that $\pcoalg{\LL} \to \cal{E}^\to$ reflects isomorphism. For the equivalence with (ii), use \cref{generalized-uniform-frob-functorial}, noting that 
...
This rephrasing, which does not seem possible for the functorial Frobenius condition, will be essential to our characterization of algebraically-free awfs's satisfying the functorial Frobenius condition.
...
If $(\cal{I}_1, \cal{J}, \cal{K}_1,)$ and $(\cal{I}_2, \cal{J}_2, \cal{K}_2)$ satisfy the generalized uniform Frobenius property, then so do 
\[
 (\cal{I}_1 \times_{\cal{E}^\to} \cal{I}_2, \cal{J}, \cal{K}_1 \times_{\cal{E}^\to} \cal{K}_2) \, , \quad (\cal{I}_1 +_{\cal{E}^\to} \cal{I}_2, \cal{J}, \cal{K}_1 +_{\cal{E}^\to} \cal{K}_2) 
 \rlap{.}
 \]
\end{proposition}
...
This concludes our work on general orthogonality functors and the functorial Frobenius condition. In \cref{sec:alg-fib} and \cref{sec:alg-frob}, we develop the algebraic countepart
of the material in \cref{sec:fib} and \cref{sec:frob}.
...
\begin{example} \label{unif-triv-fib-sset} As we will see in~\cref{rem-lift-suitable},
...
As a step, we show that that strong homotopy equivalences and uniform fibrations satisfy the uniform Frobenius condition, \ie the algebraic analogue of \cref{thm:non-alg-frobenius-she}.
...
This follows from \cref{generalized-uniform-frob-product-u} since $(\Cof, \Cof, \Fib)$ satisfies the generalized uniform Frobenius condition by hypothesis~(S3) in the definition of a suitable awfs (\cref{thm:suitable-awfs}) and $(\cal{S}, \cal{S}, \Fib)$ satisfies the generalized Frobenius condition by \cref{technical}.
...
for every $Y$. In fact, as usual, only the base needs to be assumed uniform Kan.
...
First, we assume that~$\cal{E}$ is a presheaf category and that the functorial cylinder~$\interval \otimes (-) \co \cal{E} \to \cal{E}$ satisfies not only our standing assumptions of having contractions and connections and of possessing a right adjoint (see \cref{sec:bac}), but also the following two conditions:
\begin{enumerate}[({C}1)]
...
\rlap{,}}
\]
where $\yon(c)$ the Yoneda embedding of some $c \in \catC$.
...
Since assumptions (C1)-(C2) and (M1)-(M5) are fulfilled in both $\SSet$ and~$\CSet$, with $\cal{M}$ the category of all monomorphisms and pullback squares (with (M5) a consequence
of the existence of a subobject classifier), \cref{rem-lift-suitable} establishes the existence of the algebraic weak factorizations system of \cref{unif-triv-fib-sset}.
\end{remark}
...
For $\SSet$, this result may be considered as a constructive counterpart of the fact that the pushforward along a Kan fibration preserves Kan fibrations, which cannot be proved constructively
without changes~\cite{coquand-non-constructivity-kan}.
...
The rest of this section is devoted to give a characterization of uniform trivial fibrations in terms of the partial map classifier, suggested to us by Thierry Coquand and Andr\'e Joyal, and then use this characterization to relate non-algebraic and algebraic notions of (trivial) fibration. We begin with a simple lemma.
...
The \emph{$\cal{I}$-partial map classifier} $P_Y$ relative to $Y$ associated to~$\cal{I}$ is the polynomial endofunctor, in the sense of~\cite{gambino-kock}, on~$\cal{E}_{/Y}$ given by the data
\[
\xymatrix{
  Y
&
...
.
\end{align*}
Here, the penultimate step uses the pullback square~\eqref{partial-map-classifier:1} and the last step uses the Yoneda lemma and preservation of colimits by pullbacks.
\end{proof}
...
a (trivial) Kan fibration if and only if it is a (trivial) Kan fibration in the usual sense. 
\end{example}

\begin{remark} 
Since \cref{unif-vs-non-unif} is proved constructively, it is natural to wonder whether it is possible to develop constructively our theory in
the non-algebraic setting, at least for $\SSet$ or~$\CSet$. The key obstacle is that, while the special case of the algebraic 
result in \cref{rem-lift-suitable} admits a constructive proof in $\SSet$ and $\CSet$, as we explained in~\cref{rem:constructive-small-object},
its non-algebraic counterpart does not seem to provable constructively, even in the case when $\cal{M}$ consists of all monomorphisms
(\cf \cref{thm:generation-presheaf-cisinski} and \cref{rem:colimit-decomp}). And such a result is essential for the development, since it provides
the suitable (algebraic) wfs's on which the definition of uniform Kan fibration and the proof of the functorial Frobenius property are based.
\end{remark} 
