...
Explicitly, the above definition unfolds to making $f^* u_i \co f^* X_i \to f^* Y_i$ into a left $\liftr{\cal{I}}$-map for $i \in \cal{I}$ and $u_i \co X_i \to Y_i$ fitting in a diagram of form
\[
\xymatrix{
  X_i
  \ar[dr]
...
We say that a functor $v \co \cal{J} \to \catE^\to$ satisfies the \emph{uniform Frobenius condition} with respect to $u$ if:
\begin{enumerate}[(i)]
...
The uniform Frobenius condition of a functor $v \co \cal{J} \to \cal{E}^\to$ with respect to $u$ is equivalent to a lift $Q$ as below:
...
\end{gathered}
\end{equation}
Here $P$ denotes base change of arrows over objects, sending $(f, h)$ to $h^* f$.
\end{proposition}

\begin{proof}
Since the functor $\liftl{(\liftr{u})}$ is faithful, a lift $Q$ consists just of a lift of the action of $P$ on objects that is coherent with respect to the action of $P$ on morphisms.
Coherence with respect to morphisms in $\cal{I}_{/-} \times_{\cal{E}} \cal{J}$ separates into two parts:
\begin{enumerate}[(i)]
\item coherence with respect morphisms in $\cal{I}_{/D_j}$ for fixed $v_j \co C_j \to D_j$ with $j \in \cal{J}$,
\item coherence with respect to morphisms in $\cal{J}$.
\end{enumerate}
...
For future reference, we make explicit in the following two propositions the inherent functoriality of the uniform Frobenius condition in the parameters $u \co \cal{I} \to \cal{E}^\to$ and $v \co \cal{J} \to \cal{E}^\to$ evident in the phrasing given by \cref{uniform-frob-nice}.
...
\begin{proof}
Use contravariance of $u$ in the domain of $Q$ and covariance of $u$ in the codomain of $Q$ in \cref{uniform-frob-nice} together with the multiplication of the monad $\liftl{(\liftr{(-)})}$.
\end{proof}
...
The uniform Frobenius condition of $v \co \cal{J} \to \cal{E}^\to$ with respect to $u$ is equivalent to a lift $Q'$ as below:
