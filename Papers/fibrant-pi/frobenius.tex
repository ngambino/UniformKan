...
Another direction of research, which is motivated by Voevodsky's univalence foundations programme~\cite{voevodsky:uf} and began with work by Bezem, Coquand, and Huber~\cite{coquand-cubical-sets}, focuses on notions of uniform Kan fibration in categories of cubical sets~\cite{awodey-cubical,cohen-et-al:cubicaltt,huber-thesis,pitts-cubical-nominal,swan-awfs}.
...
On the one hand, the abstract approach that we adopt allows us to apply our results to both simplicial sets and to cubical sets with connections as defined in~\cite{cohen-et-al:cubicaltt}.
...
Analogous results have been obtained by Coquand and his collaborators for uniform fibrations in a category of cubical sets without connections~\cite{coquand-cubical-sets,huber-thesis} and in some categories of cubical sets with connections (\cf \cite{cohen-et-al:cubicaltt}).
...
When instantiated to the category of cubical sets with connections, our results give also a proof of the fact that pushforward along a uniform Kan fibration preserves uniform Kan fibrations, which avoids entirely combinatorial manipulations with cubical sets (\cf~\cite{huber-thesis}).
...
\footnote{
Even though we call it a condition, it actually consists of stipulated pieces of algebraic data.
We believe this misuse of language excusible in this case.
} for a map $f \co X \to Y$ with respect to a functor $u \co \cal{I} \to \catE^\to$ for a category $\cal{E}$ will pullbacks.
...
It suffices to lift the actions of $f^*$ on objects and morphisms without regard for further coherence.
The lift will then form a functor since $\liftl{(\liftr{u})}$ is faithful.
\end{remark}
...
In contrast to the Frobenius condition, the Beck-Chevalley condition is just a property, not stipulating additional structure.
This is because the functor $\liftl{(\liftr{u})} \co \liftl{(\liftr{\cal{I}})} \to \cal{E}^\to$ is faithful.
Note that it suffices to lift the components of $\phi$ without regard for further coherence.
They will then form a natural transformation, again because  $\liftl{(\liftr{u})}$ is faithful.
\end{remark}
...
Indeed, we these two conditions can be seen as the component for objects of $\catE^\to$ and the component for morphisms of $\catE^\to$ of a global condition.
...
Let $P \co \cal{E}^\to_{/-} \times_{\cal{E}} \cal{E}^\to \to \cal{E}_{/-}^\to$ denote base change of arrows over objects defined as $P(f, h) \defeq h^* f$.
The structure of the uniform Frobenius condition of a functor $v \co \cal{J} \to \cal{E}^\to$ with respect to $u \co \cal{I} \to \cal{E}^\to$ is equivalent to a lift of $P$ to a functor $Q$ as below:
...
\begin{proof}
First note that $\liftl{(\liftr{u})}$ is faithful.
Hence, the lift $Q$ is already characterized by lifts of objects and morphisms without regard for further coherence (\cf \cref{frobenius-no-coherence,beck-chevalley-no-coherence}).
The Frobenius and Beck-Chevalley conditions then correspond to letting the object of $\cal{J}$ and $\cal{I}$ be constant in the domain of $Q$, respectively.
\end{proof}
...
\rlap{.}}
\]
If $v_2$ satisfies the uniform Frobenius condition with respect to $u$, then so does $v_1$.
\end{proposition}

\begin{proof}
Use contravariance of $v$ in \cref{uniform-frob-nice}.
\end{proof}
...
\end{equation}
is a morphism of Grothendieck fibrations.
This is for example the case if $\cal{J}$ is a category of right maps or the category of algebras in an algebraic weak factorization system.
...
Let $P' \co \cal{E}^\to \times_{\cal{E}} \cal{E}^\to \to \cal{E}^\to$ denote the pullback functor given by $P'(g, h) \defeq h^* g$.
The structure of the uniform Frobenius condition of $v \co \cal{J} \to \cal{E}^\to$ with respect to $u \co \cal{I} \to \cal{E}^\to$ is equivalent to a lift of $P$ to a functor $Q'$ as below:
...
\rlap{.}}
\]
As a first step, under a mild assumption, we establish that if uniform $\cal{I}$-fibrations have the uniform Frobenius property with respect $\cal{I}$, then they have it also with respect to the category $\cal{S}_0(\cal{I}) + \cal{S}(\cal{I})_1$ of strong homotopy equivalences (viewed as a category over $\cal{E}^\to$ in the evident way).
By the close relationship between uniform $\cal{I}$-fibrations and the right maps for $\cal{S}_0(\cal{I}) + \cal{S}(\cal{I})_1$ (\cf \cref{relating-strong-hequiv-and-uniform-fib}) this will enable us to establish the uniform Frobenius condition for uniform $\cal{I}$-fibrations with respect to $\cal{I}_\otimes$, as desired.
...
Recall the definition~\eqref{rel-strong-h-equiv-def} of $\cal{S}_k(\cal{I})$ as a strict pullback of $\cal{S}_k \to \cal{E}$ and $\cal{I} \to \cal{E}$.
Using \cref{uniform-frobenius-product-u}, it will suffice to separately show that $\liftr{(\kcylinv \otimes \id_\catE)}$ satisfies the uniform Frobenius condition with respect to $\cal{S}_k$ and $\cal{I}$.
The former is \cref{strong-h-equiv-uniform-base-change}, while the latter is just the premise.
\end{proof}

For the rest of the section, we assume that the functor from $\cal{E}$ to $\cal{E}^\to$ sending $X$ to $\bot_X$ lifts through $u \co \cal{I} \to \cal{E}^\to$ so that the previous statement can be applied to uniform $\cal{I}$-fibrations.
...
Combining the cases $k = 0$ and $k = 1$, we have that if right $(\braces{\lcyl, \rcyl} \otimes \id_\catE)$-maps satisfy the uniform Frobenius property with respect to $\cal{I}$, then also with respect to $\cal{S}_0(\cal{I}) + \cal{S}_1(\cal{I})$.
...
By the definiton of $u_\otimes \co \cal{I}_\otimes \to \catE^\to$ as given in~\eqref{equ:u-tensor}, we hence have a functor over $\catE^\to$ from uniform $\cal{I}$-fibrations to right $(\braces{\lcyl, \rcyl} \otimes \id_\catE)$-maps.
...
Strong $k$-sided homotopy equivalences are uniformly stable under base change along maps in $\cal{J}$.

In detail, let $P' \co \cal{E}^\to \times_{\cal{E}} \cal{E}^\to$ denote the pullback functor $P'(g, h) \defeq h^* g$.
Then $P'$ lifts to $Q'$ as depicted below:
\begin{equation} \label{strong-h-equiv-uniform-base-change:goal}
\begin{gathered}
\xymatrix@C+2em{
  S_k \times_{\cal{E}} \cal{J}
  \ar@{.>}[r]^{Q'}
  \ar[d]
&
  S_k
...
In particular, we have that $\cal{J}$ has the uniform Frobenius property with respect to strong $k$-sided homotopy equivalences.
\end{lemma}

\begin{proof}
First note that the last sentence follows from the rest by \cref{uniform-frob-even-nicer} since $\cal{J}$ is a category of right maps, also using the unit of the monad $\liftl{(\liftr{-})}$.

For the lift of the action on objects in \eqref{strong-h-equiv-uniform-base-change:goal}, suppose we are given $v_j \co X \to Y$ with $j \in \cal{J}$ and $g \co B \to Y$ such that $g$ is a strong $k$-sided homotopy equivalence over $Y$.
This means we have a retraction $\rho$ to $\thetak \hatotimes g$ as follows:
\begin{align*}
\xymatrix@C+1em{
  g
...
\rlap{.}}
\end{equation}
We want to make $\bar{g}$ into a strong $k$-sided homotopy equivalence.
This means to find a retraction $\bar{g}$ to $\thetak \hatotimes \bar{g}$ as follows:
\[
\xymatrix@C+2em{
  \bar{g}
...
We want to show that $\bar{\tau}$ in addition forms a morphism of strong $k$-sided homotopy equivalences from $(\bar{g}, \bar{\rho})$ to $(\bar{g}', \bar{\rho}')$.
