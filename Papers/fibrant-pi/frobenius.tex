...
\begin{definition} \label{thm:suitable-wfs} We say that a weak factorisation system $(L, R)$ on $\catE$ is \emph{suitable} if the following conditions hold:
\begin{enumerate}[(i)]
...
introduced in~\cref{def:fibration}. For this, we first introduce suitable algebraic weak factorisation systems, in complete analogy with
...
\rlap{.}}
\]
For a functor $u \co \cal{I} \to \catE^\to$, we can to characterize uniform fibrations, which we introduced in \cref{def:I-fibration}, in terms of right $\LL$-maps.For this, we apply the general results in \cref{sec:ortf} using the cocylinder structure on the right adjoint $\exp(\interval, -)$ to $\interval \otimes (-)$. 
...
\begin{theorem} \label{thm:sset-cset-nwfs} The category $\cal{E}$ admits an algebraic weak factorization systems $(\mathsf{L}_\otimes, \mathsf{R}_\otimes)$ in which right maps are the uniform fibrations. 
\end{theorem}
