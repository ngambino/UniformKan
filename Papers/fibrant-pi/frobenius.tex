...
The main examples of $\cal{E}$ considered in this paper are the categories of simplicial sets and of cubical sets. 
...
call   the \emph{right orthogonal} of $u \co \cal{I} \to \catE^\to$. 
We these definitions in place, we can already define the notion of a uniform trivial fibration in a presheaf
category.

\begin{definition} \label{exa-triv-kan-fib}
Let $\catE$ be a presheaf category, $\cal{M}$ a full subcategory of $\catE_\cart^\to$ whose objects
are monomorphisms. A \emph{uniform trivial $\cal{M}$-fibration} is a  right $\cal{M}$-map. 
\end{definition}


This terminology is intended to suggest an analogy with the theory developed by Cisinski in~\cite{cisinski-asterisque}, where he studied extensively Quillen model structures in which the cofibrations are the monomorphisms. Note, however, that we do not require $\cal{M}$ to consist of all monomorphisms. To develop the theory more generally (and include some important examples), we will rather consider conditions on $\cal{M}$ (see~\cref{uniform-fib-presheaf-cat}). The compatibility condition for a uniform trivial $\cal{M}$-fibration $(f, \phi) \co X \to Y$ involves diagrams of the form
\[
\xymatrix{
  A
  \ar[r]^{h}
...
The underlying weak factorization system of $(\LL, \RR)$ has as left and right classes the maps that admit a lift to $\pcoalg{\LL}$ and $\palg{\RR}$.
