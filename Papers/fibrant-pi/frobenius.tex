...
A full subcategory $\cal{I}$ of $\catE_\cart^\to$  is said to be \emph{suitable} if the following conditions hold.
\begin{enumerate}[(i)]
\item For all $i$ in $\cal{I}$, any base change of $i$ is also in $\cal{I}$.
\item We have $\lcyl \hatotimes i \in \cal{I}$ and $\rcyl \hatotimes i \in \cal{I}$ for $i \in \cal{I}$.
\item We have $\bot_X \co 0_{\cal{E}} \to X$ in $\cal{I}$ for $X \in \cal{E}$.
...
Let us fix a suitable functor $u \co \cal{I} \to \catE^\to$, which we think of as the counterpart of a set of generating cofibrations.
In analogy with the work of Cisinski~\cite{cisinski-asterisque}, where trivial fibrations as the maps having the right lifting properties with respect to monomorphisms, we refer to a right $\cal{I}$-map as a \emph{uniform trivial $\cal{I}$-fibration} and define the category of uniform trivial $\cal{I}$-fibrations by letting
\[
  \TrivFib{\cal{I}} \defeq \liftr{\cal{I}} \rlap{.}
\]
We now want to combine $\cal{I}$ and the cylinder to define the generating trivial cofibrations.
...
\begin{definition} \label{def:I-fibration}
 A \emph{naive $\cal{I}$-fibration} is a right $\cal{I}_\otimes$-map.
\end{definition}
...
\]
One readily sees that the notion of a naive $\cal{I}$-fibration involves diagonal fillers for diagrams having on the left maps obtained by pushouts of the form
\begin{gather*}
\xymatrix@C=1.2cm{
  A_i
  \ar[r]^{u_i}
...
\rlap{.}}
\end{gather*}
where $i \in \cal{I}$ and $k \in \braces{0, 1}$. Below, we illustrate the notion of a naive (trivial) $\cal{I}$-fibration in the special cases of simplicial sets and cubical sets.
...
Also in the category $\CSet$ of cubical sets, we consider $\cal{M}$ to consist of all monomorphisms.
