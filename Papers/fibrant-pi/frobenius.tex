...
In suitable weak factorisation systems, the class $L$ consists often of all monomorphisms. In particular, given a 
Cisinski model structure ~\cite{cisinski-asterisque}, \ie cofibrantly generated model structure $(W, F, C)$ on a presheaf category 
in which the cofibrations are the monomorphisms, the weak factorisation system $(C, W \cap F)$ of cofibrations and trivial fibrations is suitable. 
For example, for every presheaf categories over an elegant Reedy category~\cite{bergner-rezk-elegant}, we have 
a suitable weak factorisation system cofibrantly generated by the boundary inclusions. 
...
\begin{proposition} \label{thm:wfstimes} There exists a weak factorisation system $(L_\otimes, R_\otimes)$ in $\cal{E}$ whose right 
...
As mentioned in the introduction, for a model structure~$(W, F, C)$ in which the cofibrations are the monomorphisms,
right properness, \ie that the pullback of a weak equivalence along a fibration is again a fibration, is equivalent to the Frobenius property of the weak factorisation system~$(W \cap C, F)$, \ie that the pullback of a trivial cofibrations along a fibration is again a trivial cofibration. In particular, the right properness of the model structure on $\SSet$ for Kan complexes is equivalent to the Frobenius property
...
a fibration if and only if it has the right lifting property with respect to the strong homotopy equivalences in $L$. The third and final step of the proof, in~\cref{thm:non-alg-frobenius},  consists in showing that the pullback
of a $(L \cap S)$-map along a fibration is again an $(L \cap S)$-map, which can be  easily seen to imply the Frobenius condition for $(L_\otimes, R_\otimes)$.
...
for $k = 0$ or $k= 1$. We begin by relating them with the maps in $L_\otimes$, as defined in~\eqref{thm:wfstimes}.

\begin{lemma}  \label{thm:main-sheretract}\hfill 
\begin{enumerate}[$(i)$] 
\item \label{lem:from-strong-hequiv}
$L \cap S \subseteq \overline{\mathsf{Cyl}}$.
\item \label{lem:to-strong-hequiv}
$\mathsf{Cyl} \subseteq L \cap S$.
\end{enumerate}
\end{lemma}

\begin{proof}
Part (i) follows from~\cref{strong-h-equiv-as-section-non-alg}, recalling that if $f \in  L$, then $\kcyl \hatotimes f \in \mathsf{Cyl}$. 
For part~(ii), since $L$ is closed under Leibniz product with~$\kcyl$, we have $\mathsf{Cyl}  \subseteq L$.
Then, observe that the assumption that the functorial cylinder has connections implies that, 
for every $f \co X \to Y$, the map $\kcyl \hatotimes f$ is a strong $k$-oriented homotopy equivalence. 
\end{proof}
...
A map is a fibration if and only if it has the right lifting property with respect to the maps in $L \cap S$.
\end{proposition}
...
\end{gathered}
\end{equation}
where $p \in R_\otimes$, if $i  \in \mathsf{Cyl}$ then $j \in L_\otimes$. So let us assume $i \in \mathsf{Cyl}$. By part~(ii) of~\cref{thm:main-sheretract}, we have $i \in L \cap S_k$, for some $k \in \braces{0, 1}$. We show that  $j \in L \cap S_k$, which implies $j \in L_\otimes$ by part (i) 
of~\cref{thm:main-sheretract}. Note that $i \in S_k$ implies that $\thetak \hatotimes i \co i \to \kcyl \hatotimes i$ has a retraction $\rho$, as follows:
\begin{align*}
\xymatrix@C+1em{
  i 
...
\rlap{.}}
\end{align*}
Also,  since $L$ is assumed to be closed under pullback, it suffices to show that $j \in S_k$.
