...
Algebraic weak factorisation systems  (by which we mean natural factorisation systems in the
sense of~\cite{grandis-tholen-nwfs} that satisfy the distributivity condition introduced 
in~\cite{garner:small-object-argument}) differ from standard weak factorisation systems in
 that they involve maps equipped with 
additional structure, consisting of an explicit choice of solutions for  left (or right) lifting problems satisfying a naturality condition, rather than maps that merely satisfy a left (or right) lifting property, as in the definition of a Kan fibration.
Algebraic weak factorisation systems are of interest in categorical homotopy theory since they improve on some aspects of standard weak factorisation systems~\cite{bousfield-wfs}, such as the unsatisfactory interaction between colimits and lifting problems
(see~\cite[Chapter~12]{riehl-cat-homotopy} for a discussion of this point). 
Recently, their theory has been developed significantly, thanks to the work of 
Bourke and Garner~\cite{bourke-garner-I,bourke-garner-II} and has found applications in Voevodsky's univalent foundations programme~\cite{voevodsky:uf}, thanks to the work of  Bezem, Coquand, and Huber on uniform Kan fibration in categories of cubical sets~\cite{coquand-cubical-sets},
...
let us point out here for the readers interested in constructive aspects that
 our development does not rely on the law of the excluded middle or the axiom of choice, apart  from~\cref{thm:sset-cset-nwfs} and those in~\cref{sec:non-alg}. For~\cref{thm:sset-cset-nwfs}, we argue separately that the result holds constructively for simplicial sets and
...
We will relate the uniform Frobenius condition introduced above with van den Berg and Garner's functorial Frobenius condition~\cite{garner:topological-simplicial} 
in~\cref{thm:frobenius-comparison} below. In order to do so, it is convenient 
...
We will now compare the uniform Frobenius condition with the functorial Frobenius condition of van den Berg and Garner~\cite{garner:topological-simplicial}.
