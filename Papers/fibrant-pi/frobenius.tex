...
\section{Fibrations and the Frobenius property}
\label{sec:fib-and-frob}

\subsection*{Preliminaries} 
Throughout the paper, we work with a category $\cal{E}$ that we assume to be locally presentable and locally cartesian closed. These
assumptions imply that $\catE$ is complete and cocomplete and that pullback preserves all limits and colimits since it has both a left 
...
for its right adjoint, which we call pushforward along $f$. 

In the following, we illustrate our definitions and results in two main examples of such a category. The first the is the category $\SSet$ of simplicial sets, defined as the category of 
...
\subsection*{The Frobenius property}  Let us recall that a weak factorisation system $(L, R)$ has the \emph{Frobenius property} if the pullback of an $L$-map along  an $R$-map is again an $L$-map. As mentioned in the introduction, a model structure~$(\mathsf{Weq}, \Fib, \Cof)$ in which the cofibrations are the monomorphisms,
is right proper, \ie the pullback of a weak equivalence along a fibration is again a weak equivalence, 
if and only if the weak factorisation system~$(\TrivCof, \Fib)$ of trivial cofibrations and fibrations has the Frobenius property, \ie that the pullback of a trivial cofibrations along a fibration is again a trivial cofibration. In particular, the right properness of the model structure on $\SSet$ for Kan complexes is equivalent to the Frobenius property
...
\subsection*{Fibrations} The notion of a fibration that we discuss below will be relative not only to $\cal{E}$ and its functorial cylinder,
...
weak factorisation system}, introduced in \cref{thm:suitable-wfs} below. In order to state that definition, we need to introduce some notation.  
For $k \in \{ 0, 1\}$ and a map $i \co A \to B$ in $\catE$, we define the \emph{Leibniz product} of~$\kcyl$ and~$i$ to be the map 
\[
...
\rlap{.}}
\end{gather*}
We shall use Leibniz products with endpoint inclusions throughout the paper. From the next definition onwards, we write
...
\begin{definition} \label{thm:suitable-wfs} A weak factorisation $(\Cof, \TrivFib)$ is said to be \emph{suitable} if the following hold:
\begin{enumerate}[(i)]
...
Let us now fix a suitable weak factorisation system $(\Cof,\TrivFib)$ in $\catE$, which we think of as consisting of cofibrations and trivial fibrations. 
...
\end{equation}
We now define the class $\Fib$ of \emph{fibrations} to consist of the maps with the right lifting property with respect to all cylinder inclusions.
And we define the class $\TrivCof$ of \emph{trivial cofibrations} is then defined the class of the maps with the left lifting property with respect to all fibrations, \ie 
\[
  \Fib = \liftr{(\Cof_\otimes)} \, , \quad \TrivCof = \liftl{(\Fib)} \, .
 \]
...
\subsection*{The Frobenius property for fibrations}
\label{sec:frobprop}
...
\begin{enumerate}[(i)] 
\item $\Cof \cap S \subseteq \overline{\Cof_\otimes}$,
\item $\Cof_\otimes \subseteq \Cof \cap S$. 
\end{enumerate}
\end{lemma}

\begin{proof} For part (i), let $f \in  \Cof \cap S$. Since $f \in S$, \cref{strong-h-equiv-as-section-non-alg} implies that $f$ is a retract of~$\kcyl \hatotimes f$, for some $k \in \braces{0 \, , 1 }$. But since $f \in \Cof$, we have $\kcyl \hatotimes f \in \Cof_\otimes$. For part~(ii), recall 
...
\[
  \Fib =  \liftr{(\Cof_\otimes)} = \liftr{(\overline{\Cof_\otimes})} \subseteq \liftr{(\Cof \cap S)} \, , \quad
...
\section{Remarks on the functorial Frobenius condition}
\label{sec:frobc}
...
weak factorisation system (\cref{thm:suitable-awfs}) is that the conditions in the former express the closure of the class of left maps under certain functions, while the conditions in the latter express the closure of the category of left maps under certain functors. The next result not only provides us with a wide class of examples of suitable algebraic weak factorisation systems, but also shows that there are situations in which object-wise assertions can be strengthened to functoriality properties. 

\begin{proposition}  \label{rem-lift-suitable} \label{thm:sset-cset-suitable}
Let $\cal{E}$ be a presheaf category and $\cal{M}$ be a full subcategory of $\cal{E}^\to_\cart$. Assume the following:
\begin{enumerate}[(i)] 
...
\end{enumerate} 
Then there exists a suitable algebraic weak factorisation system $(\CC, \TF)$ on $\cal{E}$ that is algebraically free on $\cal{M}$, \ie such that $\TrivFib = \liftr{\cal{M}}$.
\end{proposition}
...
\begin{remark} \label{identities-in-M}
In the context of \cref{rem-lift-suitable}, we have that $\id_X \in \cal{M}$ for all $X \in \cal{E}$.
This follows by inspection of the following diagram:
\[
\xymatrix{
  X
  \ar[r]
...
\\
  \Id \otimes X
  \ar[r]^{\delta^0 \hatotimes \bot_X}
&
  I \otimes X
\rlap{.}}
\]
By conditions~(iv) and~(vi), the right map is in $\cal{M}$.
By condition~(iii), the square is a pullback.
With condition~(v), the left map $\id_X$ is then in $\cal{M}$.
\end{remark}

Let us point out that the  reduction to the lifting problems against maps in $\cal{M}$ to those with a representable codomain in
the proof of \cref{rem-lift-suitable} exploits the good behaviour of the right orthogonality functor with respect to colimits as described in~\cref{awfs-on-arrows-into-representables}, which is not available in the setting of standard weak factorisation systems. 

\begin{remark} \label{partial-map-classifier}
Let $\cal{E}$ be a presheaf category on a category $\cat{C}$ and $\cal{M}$ be a full subcategory of $\cal{E}^\to_\cart$ such that the elements of $\cal{M}$ are closed under pullback along maps from representables.
Let $\cal{I}$ be the full subcategory of $\cal{M}$ spanned by maps with a representable presheaf as codomain. 
Then there exists a subobject
...
There is an evident inclusion $K \to \Omega$ giving rise to the pullback square~\eqref{partial-map-classifier:0} since identities are contained in $\cal{M}$ as per \cref{identities-in-M}.
...
Let $t \co Y \to K \times Y$ denote the classifier for $\cal{N}_{/Y}$.
Let $\pi \co K \times Y \to Y$ denote the map to the terminal object.
The \emph{partial object classifier} $P_Y$ relative to $Y$ associated to $\cal{N}$ is the polynomial endofunctor on $\cal{E}_{/Y}$. given by the data
\[
\xymatrix{
  Y
&
...
Let $u$ denote the inclusion of $\cal{I}$ into $\cal{E}^\to$.
...
Thus, a right $\cal{M}$-map structure on $f$ is given by a diagonal filler
\begin{equation*}
\begin{gathered}
\xymatrix{
  X
...
Note that the $\cal{N}$-classifier $1 \to K$ is not itself guaranteed to be in $\cal{M}$.
In fact, this assertion is equivalent to the statement that $M \to \cal{E}^\to_{\cart}$ lifts colimits.
If this holds, then also $\eta$ has components in $\cal{M}$ by~\eqref{partial-map-classifier:1}.
It follows that the underlying class of arrows of $\liftr{\cal{M}}$ coincides with the class of arrows having the non-algebraic right lifting property with respect to $\cal{M}$.
\end{remark}

\begin{remark} \label{elegant-reedy}
Let $\cal{E}$ be a presheaf category on an elegant Reedy category $\cat{C}$.
A map in $\cal{E}$ is called a \emph{trivial fibration} if it has the right lifting property with respect to boundary inclusions.
We may write by every monomorphism in $\SSet$ by skeletal decomposition as an $\omega$-composition of cobase changes of coproducts of boundary inclusions of simplices (this is a non-constructive argument).
This shows that trivial Kan fibrations are precisely the maps having the right lifting property with respect to monomorphisms.

Let $\cal{M}$ be the full subcategory of $\cal{E}^\to_\cart$ spanned by all monomorphisms.
Since $\cal{E}$ is a topos, the inclusion $\cal{M} \to \cal{E}^\to_\cart$ lifts colimits.
Using the last paragraph of \cref{partial-map-classifier}, it thus follows that every trivial fibration is a uniform trivial fibration.
\end{remark}

\begin{example} \label{rem:constructive-small-object} Obviously, \cref{thm:sset-cset-suitable} applies to both $\SSet$ and $\CSet$.
In particular, letting~$\cal{M}$ be the category of all monomorphisms and pullback squares, one obtains an algebraic weak factorisation
system whose right maps are \emph{uniform trivial Kan fibrations}. 
In these examples, \cref{thm:sset-cset-suitable} can be proved constructively, \ie without using the law of excluded middle or the axiom of choice, provided that 
one adds the assumption that elements of $\cal{M}$ are \emph{decidable} monomorphisms.
In order to see why this is the case, first observe that in these examples every subobject of a representable is finitely presentable and that the functorial cylinder $\interval \otimes (-) \co \cal{E} \to \cal{E}$ preserves finitely presentable objects.
For part~(i), by the assumption that subobjects of representables are finitely presentable, the values of the inclusion $u \co \cal{I} \to \cal{E}^\to$ are finitely presentable objects of $\cal{E}^\to$.
An inspection of the proof of \cite[Theorem~4.4]{garner:small-object-argument} shows that this suffices to construct the algebraically-free algebraic weak factorization system $(\CC, \TF)$ on $u \co \cal{I} \to \cal{E}^\to$, and in fact the sequence constructing the appropriate free monad converges after $\omega$ steps. For this argument, the assumption of working with decidable monomorphisms in a constructive setting is used to obtain the smallness of $\cal{I}$  without relying on the Power Set axiom.
We believe that this restriction is important also to treat constructively further aspects of the theory, such as the existence
...
\item A \emph{uniform fibration morphism} is a morphism of right $\Cof_\otimes$-maps.
\end{enumerate}
\end{definition}
...
\[
  \Fib  \defeq \liftr{(\Cof_\otimes)} \rlap{.}
\]
We now characterize uniform fibrations in terms of right $\Cof$-maps. For this, we use use the cocylinder structure on the right adjoint $\exp(\interval, -)$ to $\interval \otimes (-)$. We obtain a functor $\hatexp(\bar{\delta}^k, -)$, defined in terms of a pullback, dual to $\kcyl \hatotimes (-)$, which was defined in terms of a pushout, for~$k \in \braces{0, 1}$. These form an adjunction as follows:
\[
\xymatrix@C+4em{
  \catE^\to
...
\item $\hatexp(\bar{\delta}^0, p)$ and $\hatexp(\bar{\delta}^1, p)$ admit the structure of right $\Cof$-maps.
\end{enumerate}
\end{proposition}
...
\[
 \cal{S}_k(\Cof) \to \cal{E}^\to
\]
The next lemma is the analogue of~\cref{thm:main-sheretract}.

Define
\[
\cal{S}(\Cof) \defeq \cal{S}_0(\Cof) + \cal{S}_1(\Cof)
\]
...
\begin{proof} First, we show that $(  \Fib, \cal{S}(\Cof))$
 satisfies the uniform Frobenius condition. Recall  that~$\cal{S}_k(\Cof)$ is the pullback of~$\cal{S}_k  \to \cal{E}$ and~$\Cof \to \cal{E}$.
