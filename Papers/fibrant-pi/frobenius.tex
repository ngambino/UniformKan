...
finding a constructive counterpart of one of the key facts underpinning Voevodsky's simplicial model of univalent foundations, as well as giving a new proof of the right properness of
...
and is furthermore required to satisfy a naturality condition. 
Although algebraic weak factorisation systems were originally introduced to address issues in categorical homotopy theory (for example, to remedy the unsatisfactory interaction between 
colimits and lifting problems in standard weak factorisation systems~\cite[Chapter~12]{riehl-cat-homotopy}), 
 ...
 has shown that they have natural applications in Voevodsky's univalent foundations programme~\cite{voevodsky:uf}, which have 
 been developed  in~\cite{awodey-cubical,cohen-et-al:cubicaltt,pitts-cubical-nominal,swan-awfs}. 
...
and use it to obtain algebraic counterparts of some important results in Cisinski's theory of Quillen model structures on presheaf categories~\cite{cisinski-asterisque}. We then apply these results  to solve the open problem in univalent foundations, namely that of giving a constructive version of the
 ...
 the simplicial theory of bundles~\cite{joyal-tierney:simplicial-homotopy-theory}.

We will be particularly  interested in a property of weak factorisation systems that is known as the Frobenius condition~\cite[Definition~3.2.1]{garner:types-omega-groupoids}, since it is closely related to Lawvere's Frobenius reciprocity condition~\cite{lawvere-equality} (see \cite{clementino:frobenius} for an explanation of the connection). One of the reasons for the 
...
In the context of algebraic weak factorisation systems, it is natural to require not only a version of the Frobenius condition mentioned above, which is a property of maps, but 
...
since it is analogous to the well-known Beck-Chevalley conditions~\cite{benabou-descente,lawvere-equality}.  Hence, as part of our development, we will introduce a single
 single condition, which we call the uniform Frobenius condition, which is equivalent to combination of the the Frobenius and Beck-Chevalley conditions and that we consider the
 appropriate counterpart for algebraic weak factorisation systems of the usual Frobenius condition for weak factorisation systems. This condition is similar to the functorial Frobenius condition of~\cite{garner:topological-simplicial}, but formulated in a slightly more general setting.
...
This is proved using Garner's small object argument~\cite{garner:small-object-argument} after having shown that uniform (trivial) fibrations can be be defined equivalently by weak orthogonality with respect to a small category of maps (\cref{small-gen-triv-kan}).
...
Frobenius property, and therefore that pushforward along a uniform Kan fibration preserves uniform Kan fibrations. There are two separate reasons for the interest in this result. 
...
our results give also a new constructive 
...
We will also call $\liftr{\cal{I}}$ the category of \emph{uniform trivial $\cal{I}$-fibrations}, written $\TrivFib{\cal{I}}$. As explained in~\cref{exa-triv-kan-fib} below, 
the terminology is intended to suggest an analogy with the theory developed by Cisinski in~\cite{cisinski-asterisque}, where he studied extensively model structures in which the cofibrations are the monomorphisms.
...
factorisation systems of the Frobenius condition for weak algebraic factorisation 
systems~\cite{garner:types-omega-groupoids}. As already remarked in the introduction,
...
As such, it seems the most natural way to phrase the condition for algebraic weak factorisation
systems.
...
We conclude this section by remarking that an algebraic weak factorization system on a finitely complete category $\cal{E}$ algebraically free on discrete category $u \co \cal{I} \to \cal{E}^\to$ in the sense of~\cite{garner:small-object-argument} such that right adjoints to pullbacks along right $\cal{I}$-maps exist will satisfy the uniform Frobenius condition with respect to $\cal{I}$ precisely if the underlying cloven weak factorization system in the sense of~\cite{garner:topological-simplicial} is functorially Frobenius.
This follows by \cref{uniform-frob-even-nicer} and orthogonality.
...
Below, we fix a suitable category of monomorphisms $\cal{M}$. We write $u \co \cal{M} \hookrightarrow \catE^\to$ to denote the inclusion.
...
and a suitable category $\cal{M}$ of monomorphisms.
\begin{enumerate}[(i)]
...
\begin{remark} \cref{thm:sset-cset-nwfs} applies to both $\SSet$ and $\CSet$. In these cases, we explain why the result holds constructively, \ie without using 
...
This explains why we have to restrict the maps $d \co A_1 \to A_2$ to non-identity maps: without this restriction, regular trivial fibrations would have unique lifts against boundary inclusions, which is too restrictive.
...
the uniform Frobenius property. Hence, for  a uniform $\cal{M}$-fibration $p \co X \to Y$, pushforward along $p$ lifts to a functor 
\[
