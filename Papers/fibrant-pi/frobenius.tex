...
These follow \notec{`Follow from' or `are'? (cf referee comment)} from~\cite[Lemma~24]{bourke-garner-I}.
...
\notec{Say where is the hypothesis of morphism of Grothendieck fibrations used?}
...
\begin{proof} 
For (i), apply \cref{lift-of-adjunction} \notec{Referee comment not yet addressed} to the adjunction $v_j^* \dashv (v_j)_*$ with $u = u_{/D_j}$ and $v = {\liftr{w}}_{/C_j}$, using \cref{pitchfork-slicing-grothendieck} to permute slicing and orthogonality functors.
...
\begin{proof}
Using \cref{retract-closure} and the adjunction~\eqref{garner-adjunction} \notec{Why also the adjunction?}, we have $\overline{\liftr{\cal{I}}} \leftrightarrow \liftr{\cal{I}}$, hence $\pAlg{\RR} \leftrightarrow \overline{\pAlg{\RR}}$.
...
We now give the algebraic analogue of \cref{thm:char-fib-nonalg}, \ie an equivalent characterization of uniform fibrations which does not refer to generating uniform cofibrations. This will show that the category $\Fib$ is independent from the choice of the cofibrant generator $\cal{I}$ of $(\CC, \TF)$.
...
\begin{proof} \notec{To be adapted}
We begin by illustrating how the adjunction~\eqref{equ:leibniz-prod-exp} between the functors $\delta^k \hatotimes (-)$ and $\hatexp(\bar{\delta}^k, -)$ for $k \in \braces{0\,,1}$ fits into the framework of \cref{pitchfork-leibniz-most-general-example}.
...
The statement then follows from \cref{lift-of-leibniz-adjunction-pullback-form} above.
\end{proof}
