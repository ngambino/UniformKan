...
\begin{proposition} \label{thm:frobenius-comparison} Let $\cal{E}$ be a finitely complete category.
\begin{enumerate}[(i)] 
\item An algebraic weak factorisation system $(\mathsf{L}, \mathsf{R})$ satisfies the functorial Frobenius condition if and only if 
$\mathsf{R}$-$\mathsf{alg}$  has the uniform Frobenius condition with respect to  $\mathsf{L}$-$\mathsf{coalg}$.
\item An algebraic weak factorisation system $(\mathsf{L}, \mathsf{R})$ algebraically free on $\cal{I}$ has the functorial Frobenius condition if and only if $\liftr{\cal{I}}$ has the uniform Frobenius condition with respect to~$\cal{I}$. 
\end{enumerate}
  \end{proposition} 

\begin{proof} Part (i) follows by \cref{uniform-frob-even-nicer} and orthogonality.
\end{proof} 
