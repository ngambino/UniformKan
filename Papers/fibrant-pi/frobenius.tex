...
In the category of simplicial sets, this amounts to the preservation of Kan fibrations by pushforward along Kan fibrations, a fact that plays an important role in the simplicial model of univalent foundations~\cite{voevodsky-simplicial-model}.
...
We then construct wfs's that have fibrations as the right maps we show show that satisfy the Frobenius condition.
...
Since the proof of these results for simplicial sets and cubical sets is constructive, \ie avoids the use of the law of excluded middle and the axiom of choice, we solve the open problem of obtaining a constructive counterpart of the fact that pushforward along a Kan fibration preserves Kan fibrations, which is unprovable constructively without modifying the notion of a Kan fibration~\cite{coquand-non-constructivity-kan}.
This problem was motivated by the goal of proving that the extension of Martin-L\"of type theory with Voevodsky's univalence axiom is consistent relatively to Martin-L\"of type theory.
Indeed, one strategy to do this is to develop the simplicial model of univalent foundations working within a constructive theory.
But, in order to interpret correctly the rules for $\Pi$-types of Martin-L\"of type theory, one needs to show that pushforward along Kan fibrations preserves Kan 
fibrations~\cite[Lemma 2.3.1]{voevodsky-simplicial-model}. 
...
Our results show that the key obstacle to a constructive treatment of the result on pushforward along Kan fibrations is not the simplicial setting, but 
non-algebraic nature of the definition of Kan fibration.

Furthermore, since our results apply also in the category of cubical sets of~\cite{cohen-et-al:cubicaltt,huber-thesis}, we give a proof of the fact that pushforward along a uniform Kan fibration preserves uniform Kan fibrations which avoids entirely complex calculations with cubical sets, unlike the existing ones.
...
However, let us point out here for the interested readers that our development does not rely on the law of the excluded middle or the axiom of choice, apart 
from~\cref{thm:wfstimes}, \cref{thm:sset-cset-nwfs} and \cref{rem-lift-suitable}. 
...
As mentioned in the introduction, a Quillen model structure~$(\mathsf{Weq}, \Fib, \Cof)$ in which the cofibrations are the monomorphisms (such as the Quillen
model structure for Kan complexes~\cite{quillen-homotopical}) is right proper, \ie the pullback of a weak equivalence along a fibration is again a weak equivalence, if and only if the wfs~$(\TrivCof, \Fib)$ of trivial cofibrations and fibrations has the Frobenius property.
...
By a result of Cisinski~\cite[Proposition~1.2.27]{cisinski-asterisque}, every presheaf category admits a suitable wfs whose cofibrations are the 
mononomorphisms. When $\cal{E}$ is a category of presheaves over an elegant Reedy category~\cite{bergner-rezk-elegant}, 
a map has the right lifting property with respect to monomorphisms if and only if it has it with respect to boundary inclusions, since
...
This section starts the second part of the paper, in which we are interested in generalizations for awfs's of the results obtained earlier. In this section and the next we will need to review and establish some facts about the algebraic setting that are useful for our development. Importantly, in the algebraic setting we do not consider just classes of arrows in $\catE$, but rather categories~$\cal{I}$, to be thought of as indexing categories, and functors~$u \co \cal{I} \to \catE^\to$.
...
This concludes our work on orthogonality functors and the functorial Frobenius condition in general. In \cref{sec:alg-fib} and \cref{sec:alg-frob}, we develop the algebraic countepart
...
Finally, the general proof of \cref{uniform-fibrations-uniform-frobenius} and \cref{uniform-fibrations-frobenius-pushforward}, establishing the functorial Frobenius condition for $(\TC, \FF)$ and its pushforward analogue, are constructive in general.
...
For $\SSet$, this result is a constructive counterpart of the fact that the pushforward along a Kan fibration preserves Kan fibrations, which cannot be proved constructively
...
The rest of this section is devoted to give a characterization of uniform trivial fibrations in terms of the partial map classifier, a result suggested to us by Thierry Coquand and Andr\'e Joyal, and then use this characterization to relate non-algebraic and algebraic notions of (trivial) fibration. We begin with a simple lemma.
...
the non-algebraic setting, at least for $\SSet$ or~$\CSet$. The key obstacle to this is that, while the special case of the algebraic 
...
the suitable (algebraic) wfs's on which the definition of a uniform Kan fibration and the proof of the functorial Frobenius property are based.
\end{remark} 
