...
additional structure, consisting of a choice of diagonal fillers for a class of lifting problems, which is then required to satisfy a naturality condition. 
Although algebraic weak factorisation systems were originally introduced to address issues in categorical homotopy theory, such as  the 
...
and use it to obtain algebraic counterparts of some important results in Cisinski's theory of Quillen model structures on presheaf categories~\cite{cisinski-asterisque}. We then apply these results  to solve the open problem in univalent foundations, namely that of giving a constructive version of the
 fact that pushforward along Kan fibrations preserves Kan fibrations~\cite{coquand-non-constructivity-kan,voevodsky-simplicial-model}.
...
since it is analogous to the well-known Beck-Chevalley conditions~\cite{benabou-descente,lawvere-equality}.  Hence, as part of our development, we will introduce a single
 single condition, which we call the \emph{uniform Frobenius condition}, which is equivalent to combination of the the Frobenius and Beck-Chevalley conditions and that we consider the
...
\section{Remarks on right properness} 
