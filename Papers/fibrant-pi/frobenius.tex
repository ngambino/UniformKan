...
\begin{proof}
This follows from the more general \cref{thm:kcylf-is-she-alg}, which is proven abstractly.
To help with intuition, we sketch an explicit unfolding of the case $k = 0$.
...
A standard diagram-chasing argument using diagrams~\eqref{contractions}, \eqref{connections:0}, \eqref{connections:1} and naturality verifies that $\phi$ and $\psi$ have the correct endpoints and that the $0$-oriented homotopy equivalence $(f, g, \phi, \psi)$ is strong using diagrams.
%
...
Then $\alpha \defeq (\delta^k, \bar{\delta}^k)$ is a morphism in $\Adj(\cal{E}, \cal{E})$ from $\Id \dashv \Id$ to $I \otimes (-) \dashv \exp(I, -)$.
