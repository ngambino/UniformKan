...
Let $(L, R)$ be a weak factorisation system and let $I \subseteq L$ be a class such that~$I^\pitchfork = R$. Then the following are equivalent:
\begin{enumerate}[(i)] 
\item $(L,R)$ has the Frobenius property. 
\item For every $p \co Y \to X$, the pushforward $p^* \co \cal{E}/Y \to \cal{E}/X$ preserves $R$-maps.
\item The pullback of an $I$-map along 
an $R$-map is an $L$-map, \ie for every pullback diagram of the form
\[
\xymatrix{
 Y \ar[d]_j \ar[r]^q & X \ar[d]^i \\ 
B \ar[r]_p & A }
\]
where $p \in R$, if $i  \in I$ then $j \in L$.
\end{enumerate}
\end{proposition} 
...
for $k = 0$ or $k= 1$. We begin by relating them to the cylinder inclusions, as defined in~\eqref{equ:cylinc}.
...
\begin{proof} For part (i), let $f \in  L \cap S$. Since $f \in S$, \cref{strong-h-equiv-as-section-non-alg} implies that $f$ is a retract of~$\kcyl \hatotimes f$. But since $f \in L$, we have $\kcyl \hatotimes f \in \mathsf{Cyl}$. For part~(ii), observe that since $L$ is closed under Leibniz product with~$\kcyl$, we have $L_\otimes  \subseteq L$.
...
\end{gathered}
\end{equation}
where $i  \in S_k$ and $p \in R_\otimes$. We wish to show that $j \in  S_k$. Since $i \in S_k$, $\thetak \hatotimes i \co i \to \kcyl \hatotimes i$ has a retraction $\rho$, as follows:
\begin{align*}
\xymatrix@C+1em{
  i 
