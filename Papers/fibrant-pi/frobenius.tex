...
For an arrow $f \in \cal{E}^\to$, we have $f \in \cal{S}_k$ if and only if there is a retraction $\rho$ of the map $\thetak \hatotimes f$, as below:
\[
\xymatrix@C+2em{
  f
...
Let $\cal{I}$ be a class of maps in $\cal{E}$.
We write $\cal{S}_k(\cal{I})$ for the subclass of $\cal{I}$ consisting of strong $k$-oriented homotopy equivalences.
