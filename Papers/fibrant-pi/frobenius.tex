...
We will only consider cofibrantly generated awfs $(L, R)$, for which there are always functors back and forth between $\pAlg{R}$ and $\palg{R}$.
...
We further write $\TrivCof$ for the category of uniform trivial cofibrations, defined as left $\Fib$-maps.
In formulae, this means:
\[
\begin{aligned}
\Fib = \liftr{(\cal{I}_\otimes)}
,&&
\TrivCof = \liftl{\Fib}
.\end{aligned}
\]
...
This shows that the categories $\Fib$ and $\TrivCof$ are in fact independent of the choice of the cofibrant generator $\cal{I}$ of $(\CC, \TF)$.
