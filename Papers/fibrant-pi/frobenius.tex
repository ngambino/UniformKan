...
\section{The non-algebraic case: preliminaries}

\subsection*{Weak factorization systems}

Let $\cal{E}$ be a locally-presentable category.
Let $\cal{I} \subseteq \cal{E}^\to$ be a set of maps, called the \emph{generating cofibrations}.
It generates a weak factorization system $(\cal{C}, \cal{TF})$, whose left and right classes we respectively call \emph{cofibrations} and \emph{trivial fibrations} even though we will not be working in the context of a model category.
...
\notec{We define a second set of maps $\cal{I}_\otimes \defeq (\lcyl \hatotimes \cal{I}) \cup (\rcyl \hatotimes \cal{I})$ in $\cal{E}$, called the \emph{generating trivial cofibrations.} It generates a weak factorization system $(\cal{TC}, \cal{F})$, whose left and right classes we respectively call \emph{trivial cofibrations} and \emph{fibrations}. \\ Add assumptions on $\cal{C}$ --- every object cofibrant, closure under pushout product with endpoint inclusions, etc.}
...
\section{The non-algebraic case: the tools}
...
Given $f \in \cal{S}_k(\cal{I})$, then $f$ is a retract of a map in $\cal{I}$.
\end{lemma}

\begin{proof}
By \cref{strong-h-equiv-as-section}.
\end{proof}
...
Assume that the class $\cal{I}$ is closed under Leibniz tensor with $\lcyl$ and $\rcyl$.
Assume further that the functorial cylinder has effective connections.
Then we have $\cal{I}_\otimes \subseteq \cal{S}_k(\cal{I})$.
\end{lemma}

\begin{proof}
By the first assumption, we have $\cal{I}_\otimes \subseteq \cal{I}$.
By the second assumption, every map in $\cal{I}_\otimes$ is a (left or right) strong homotopy equivalence.
\end{proof}

\begin{proposition} \label{thm:twodir}
Under the assumptions of~\cref{lem:to-strong-hequiv}, the sets of maps $\cal{I}_\otimes$ and $\cal{S}_k(\cal{I})$ generate the same weak factorization system.
\end{proposition}
...
\section{The non-algebraic case: main result}

Given $k \in \braces{0, 1}$, we call a map in $\cal{E}$ a $k$-Hurewitz fibration if it has the right lifting property with respect to $\kcyl \otimes X$ for all $X \in \cal{E}$.
Any fibration is in particular a $k$-Hurewitz fibration for $k \in \braces{0, 1}$ since $\bot_X \co 0 \to X$ is in $\cal{I}$ for all $X \in \cal{E}$.
...
\section{Algebraic weak factorisation systems}
\label{sec:ortf}
...
\subsection{Restriction to representable codomains} 
