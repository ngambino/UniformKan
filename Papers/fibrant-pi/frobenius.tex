...
For a category $\catE$, we write $\cal{E}^\to$ for the category of arrows and commutative squares in $\cal{E}$, and $\cal{E}^\to_{\cart}$ for the subcategory of $\cal{E}^\to$ with pullback squares as maps. For our purposes, it will be convenient to work with a modified version of the standard definition of a functorial cylinder~\cite{kamps-porter:homotopy}, obtained by requiring also preservation of finite colimits.

\begin{definition} \hfill  \label{def:connections}
\begin{enumerate}[(i)]
\item A \emph{functorial cylinder} in $\calE$ is an endofunctor $\interval \otimes (-) \co \catE \to \catE$ preserving finite colimits equipped with natural transformations 
\[
\lcyl \otimes (-) \, ,  \rcyl \otimes (-) \co \Id_\catE \to \interval \otimes (-)
\] 
called the \emph{left} and \emph{right endpoint inclusions}, respectively.
...
\rlap{.}}
\]
\item A functorial cylinder $(\interval \otimes (-), \lcyl \otimes (-), \rcyl \otimes (-))$ with contractions $\ccyl \otimes (-)$ has \emph{connections} if, for $k \in \braces{0, 1}$, there is a natural transformations $c^k \otimes (-) \co \interval \otimes \interval \otimes (-) \to \interval \otimes (-)$ such that the diagrams
...
When working with a functorial cylinder, we adopt the convention of associating the tensor product notation to the right in order to avoid excessive bracketing.
The notation $\interval \otimes (-)$ adopted here is deliberately suggestive of the fact that a functorial cylinder is 
often defined using a monoidal structure and an interval object, as in the examples below,
so it is convenient to make some remarks about them in general.
% \begin{example} \label{exa:cyl-via-int} \label{exa:connections-for-interval}
...
\end{aligned}
\end{align}
The maps $c^0$ and $c^1$ can be seen as analogous to the minimum and maximum operations for the real line interval, or as conjunction and disjunction in a bounded distributive lattice. Tensoring with an interval object with connections induces a functorial cylinder with connections.
% \end{example}
...
Then, taking the Cartesian product with $\Delta^1$ provides a functorial cylinder with contractions.
This interval object has uniquely determined connections $c^k \co \Delta^1 \times \Delta^1 \to \Delta^1$ with $k \in \braces{0, 1}$ given on points by $c^0(x, y) = \min(x, y)$ and $c^1(x, y) = \max(x, y)$.
...
We stress that the only feature of the cube category $\Box$ relevant for the main results on the  Frobenius condition are connections and symmetries.
...
Then, tensoring with $\Box^1$ provides a functorial cylinder with contractions.
...
\subsection*{Fibrations} We assume that $\catE$ is equipped with a functorial cylinder $ \interval\otimes (-) \co \catE \to \catE$
having a right adjoint and such that the naturality squares of $\lcyl \otimes (-)$ and $\rcyl \otimes (-)$ are pullbacks. % \footnote{The latter assumption allows us to transport statements about uniform trivial fibrations to uniform fibrations via \cref{prod-exp-general}.} In this setting, the analogy with the theory developed by Cisinski in~\cite{cisinski-asterisque}, it is natural to consider $\cal{I}$ to be a category of monomorphisms. 
We isolate the appropriate assumptions on our class of generating cofibrations in the next definition.



\begin{definition} \label{uniform-fib-presheaf-cat}
A class of monomorphisms $\cal{I}$ in $\catE$  is said to be \emph{suitable} if the following conditions hold.
\begin{enumerate}[(i)]
\item $\cal{I}$ is closed under pullbacks, \ie for every pullback square
\[
\xymatrix{
C \ar[d]_j \ar[r] & A \ar[d]^i \\
D \ar[r] & B }
\]
if $i \in \cal{I}$ then $j \in \cal{I}$. 
\item $\cal{I}$ is closed under Leibniz product with the endopoint inclusions, \ie for every $i \in \cal{I}$ we have 
 $\lcyl \hatotimes i \in \cal{I}$ and $\rcyl \hatotimes i \in \cal{I}$.
\item For every $X \in \cal{E}$, the unique map $\bot_X \co 0_{\cal{E}} \to X$ is in $\cal{I}$.
\item The functor $I \otimes (-)$ preserves pullbacks for squares of the form $i \to j$ with $i, j \in \cal{I}$. \notec{WHERE IS THIS USED?}
\end{enumerate}
\end{definition}
...
If one takes $\cal{I}$ to consist of all monomorphisms, the conditions above are inherited from~$\Set$.
...
Let us fix a suitable class of monomorphisms $\cal{I}$, which we think of as the counterpart of a set of generating cofibrations.
In analogy with the work of Cisinski~\cite{cisinski-asterisque}, we define a \emph{trivial $\cal{I}$-fibration} to be a map with the 
right lifting property with respect to $\cal{I}$, and write $\TrivFib{\cal{I}}$ for the class of  trivial $\cal{I}$-fibrations, 
\[
  \TrivFib{\cal{I}} \defeq \liftr{\cal{I}} \rlap{.}
\]
We now want to combine $\cal{I}$ and the cylinder to define the generating trivial cofibrations. We define $\cal{I}_\otimes$ to consist
of all the maps of the form $\kcyl \hatotimes i$, where $k \in \{ 0, 1 \}$ and  $i \in \cal{I}$. Explicitly, these maps are obtained via the
...
\rlap{.}}
\end{gather*}
Since $\cal{I}$ is  closed under Leibniz products with the endpoint inclusions, we have that $\cal{I}_\otimes
\subseteq \cal{I}$. We define an \emph{$\cal{I}$-fibration} to be a map with the right lifting property with respect to $\cal{I}_\otimes$
and write~$\Fib{\cal{I}}$ for the class of  $\cal{I}$-fibrations, 
\[
  \Fib{\cal{I}} \defeq \liftr{(\cal{I}_\otimes)} \rlap{.}
\]
We  illustrate the notion of a (trivial) $\cal{I}$-fibration in simplicial sets and cubical sets.

\begin{example}[Fibrations in simplicial sets]
In the category $\SSet$ of simplicial sets, we consider~$\cal{I}$ to consist of all monomorphisms. Then, a (trivial) $\cal{I}$-fibration
is the same thing as a (trivial) Kan fibration in the usual sense. 
Indeed, an $\cal{I}$- fibration $p \co X \to Y$ is a map for which there exist diagonal fillers for diagrams of the form
\[
\xymatrix{
...
\begin{example}[Fibrations in cubical sets]
Also in the category $\CSet$ of cubical sets, we consider~$\cal{I}$ to consist of all monomorphisms.
A  (trivial) $\cal{I}$-fibration in $\CSet$ will be called a \emph{(trivial) Kan fibration}.
...
The fibrant cubical types defined in~\cite{cohen-et-al:cubicaltt} arise by considering the  $\cal{I}'$-fibrations where~$\cal{I}'$ is the full subcategory of $\cal{I}$ spanned by (decidable) monomorphisms classified by their face lattice~$\mathbb{F}$, \ie those arising as a base change of $\braces{\top} \to \mathbb{F}$, excluding for example diagonals in cubes.
...
Note that  $\cal{I}'$ satisfies the properties~(i) to~(iv) of \cref{uniform-fib-presheaf-cat} and is thus covered by our development.
\end{example}
...
The following notion is very important in our development.

\begin{definition} \label{def:strhe}
A map $f \co X \to Y$ is called a \emph{strong} right  homotopy equivalence if there exist $g \co Y \to X$ and homotopies $\phi \co \id_X \sim g \cc f$ and $\psi \co \id_Y \sim f \cc g$ such that the diagram
\[
\xymatrix{
  \interval \otimes X
...
Examples of strong homotopy equivalences are given by   strong deformation retracts, i.e. homotopy equivalences for which the homotopy~$\phi$ is trivial, \ie $\phi = \varepsilon_X$, where $\varepsilon \co I \otimes (-) \to \Id_\catE$ is the contraction of the functorial cylinder. For $k \in \braces{0, 1}$, we write $\cal{S}_k$ for the class of strong $k$-oriented homotopy equivalences.
...
A map $f \co X \to Y$ is a string $k$-oriented equivalence 
if and only if there is a retraction $\rho$ of the map $\thetak \hatotimes f  \co f \to \kcyl \hatotimes f$, as below:
\[
\xymatrix@C+2em{
  f
...
Let $\cal{I}$ be a class of maps in $\cal{E}$ and define $\cal{S}_k(\cal{I}) \defeq \cal{I} \cap \cal{S}_k$, \ie 
the class of maps in $\cal{I}$ that are strong $k$-oriented homotopy equivalences.
...
Every $f \co A \to B$ that is in $\cal{I}$ , then $f$ is a retract of a map in $\cal{I}_\otimes$.
\end{lemma}

\begin{proof}
By \cref{strong-h-equiv-as-section-non-alg}.
\end{proof}


\begin{corollary} \label{effective-connections-leibniz}
For every $f \in \catE^\to$, the map $\kcyl \hatotimes f$ is a strong $k$-oriented homotopy equivalence,  
for $k \in \braces{0, 1}$.\qed
\end{corollary}

\begin{lemma} \label{lem:to-strong-hequiv}

Then we have $\cal{I}_\otimes \subseteq \cal{S}_k(\cal{I})$.
\end{lemma}

\begin{proof}
Since $\cal{I}$ is closed under Leibniz tensor with $\lcyl$ and $\rcyl$, we have $\cal{I}_\otimes \subseteq \cal{I}$.
By~\cref{effective-connections-leibniz}, every map in $\cal{I}_\otimes$ is a (left or right) strong homotopy equivalence.
\end{proof}

\begin{proposition} \label{thm:twodir}
A map is a fibration if and only if it has the right lifting property with respect to  $\cal{S}_k(\cal{I})$.
Hence, the sets of maps $\cal{I}_\otimes$ and   $\cal{S}_k(\cal{I})$ generate the same weak factorization system.
\end{proposition}

\begin{proof}
Combine \cref{lem:from-strong-hequiv,lem:to-strong-hequiv}.
\end{proof}



Given $k \in \braces{0, 1}$, we call a map in $\cal{E}$ a \emph{$k$-Hurewitz fibration} if it has the right lifting property with respect to $\kcyl \otimes X$ for all $X \in \cal{E}$.


\begin{lemma} Any fibration is in particular a $k$-Hurewitz fibration for $k \in \braces{0, 1}$ 
\end{lemma}

\begin{proof} The claim follows since $\bot_X \co 0 \to X$ is in $\cal{I}$ for all $X \in \cal{E}$.
\end{proof}

\begin{proposition}
Let $k \in \braces{0, 1}$.
The class $\cal{S}_k$ is stable under pullback along $(1-k)$-Hurewicz fibrations.
\end{proposition}

\begin{proof}
Let $p \co X \to Y$ be a $(1-k)$-Hurewicz fibration and $f \co B \to Y$ be a $k$-strong homotopy equivalence.
This means we have a retraction $\rho$ of $\thetak \hatotimes f$ as follows:
\begin{align*}
\xymatrix@C+1em{
  f
  \ar[r]^-{\thetak \hatotimes f}
...
\rlap{.}}
\end{align*}
We construct the pullback $\sigma \co \bar{f} \to f$ of $f$ along $p$:
...
\end{gathered}
\end{equation}
We want to make $\bar{f}$ into a strong $k$-oriented homotopy equivalence.
This means to find a retraction $\bar{\rho}$ of $\thetak \hatotimes \bar{f}$ as follows:
\[
\xymatrix@C+1em{
  \bar{f}
  \ar[r]^-{\thetak \hatotimes \bar{f}}
  \ar@{=}[dr]
&
  \kcyl \hatotimes \bar{f}
  \ar@{.>}[d]^{\bar{\rho}}
\\&
  \bar{f}
\rlap{.}}
...
\rlap{.}}
\]
But we have such a filler since $p$ was assumed a $(1-k)$-Hurewicz fibration.
\end{proof}

\begin{theorem}
Trivial cofibrations are stable under pullback along fibrations.
\end{theorem}
