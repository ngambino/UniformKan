...
\begin{proof} This is a diagram-chasing argument, which we present (in outline) it since this gives an unfolding of the abstract proof of~\cref{thm:kcylf-is-she-alg},
...
\begin{proof}
In \cref{lift-of-leibniz-adjunction}, rewrite the first diagram using the adjunction~\eqref{garner-adjunction} and compare it to the second diagram, using naturality in $\cal{J}$.
\end{proof}

\begin{remark} \label{pitchfork-leibniz-most-general-example}
We will apply~\cref{lift-of-leibniz-adjunction,lift-of-leibniz-adjunction-pullback-form} in \cref{sec:alg-fib} as follows.
...
In order to define uniform fibrations, we introduce some notation.
We write $(-) \otimes (-) \co [\catE, \catE] \times \catE \to \catE$ as an infix shorthand for functor application,
so that we can write $(-) \hatotimes (-) \co [\catE, \catE]^\to \times \catE^\to \to \catE^\to$ for the functor
obtained by applying the Leibniz construction to it, which is defined in terms of pushouts.
...
\,.\]
Explictly, $\kcyl \hatotimes u_i$ is the Leibniz product of $\kcyl$ and $u_i \co A_i \to B_i$, as in~\eqref{equ:leib-prod}.
We shall use similar conventions for other natural transformations (or maps of such) written using the tensor notation.
...
The statement then follows from \cref{lift-of-leibniz-adjunction-pullback-form} applied to the adjunction~\eqref{equ:leibniz-prod-exp} \notec{(actually, not directly to this adjunction, but rather the underlying adjunction in the unhatted form it comes from using the remark in the section on orthogonal categories that was deleted)} and the category of arrows $\cal{I}$ for $k \in \braces{0\,,1}$.
\end{proof}
...
Using the adjunction~\eqref{equ:leibniz-prod-exp} \notec{(actually, not directly to this adjunction, but rather the underlying adjunction in the unhatted form it comes from using the remark in the section on orthogonal categories that was deleted)} and \cref{lift-of-leibniz-adjunction}, the goal follows from the map $\Fib \to \cal{D}_k$ of \cref{prod-exp-general}.
\end{proof}
