...
\begin{proposition}  \label{rem-lift-suitable} \label{thm:sset-cset-suitable}
 Let $\cal{E}$ be a presheaf category and $\cal{M}$ be a subcategory of $\cal{E}^\to_\cart$. Assume the following:
\begin{enumerate}[(i)] 
\item $\interval \otimes (-) \co \cal{E} \to \cal{E}$ preserves pullback squares,
\item the natural transformations $\kcyl \co \id_\cal{E} \to \interval \otimes (-)$, for $k \in \braces{ 0, 1}$, are cartesian,
\item the maps in $\cal{M}$ are monomorphisms.
\item for every $X \in \cal{E}$, the unique map $\bot_X \co 0 \to X$ is in $\cal{M}$, 
 \item $\cal{M}$ is closed under pullbacks, 
\item $\cal{M}$ is closed under Leibniz product with the endpoint inclusions.
\end{enumerate} 
Then there exists a suitable algebraic weak factorisation system $(\CC, \TF)$ on $\cal{E}$ that is algebraically free on $\cal{M}$, \ie 
such that $\TrivFib = \liftr{\cal{M}}$.
\end{proposition}

\begin{proof} Let $\cal{I}$ be the full subcategory of $\cal{M}$ spanned by maps with a representable presheaf as codomain. 
Since $\cal{I}$ is small, Garner's small object argument~\cite{garner:small-object-argument} it generates a
...
These follow by the assumptions (iii)-(v) on $\cal{M}$ via  standard diagram-chasing argument, which we omit. For condition~(iv), the
crucial property being used in that proof is that elements of $\cal{M}$, being monomorphisms in a topos,
are adhesive maps~\cite{garner-lack:adhesive}. 
\end{proof} 
...
\begin{remark} SOMETHING ON THE PARTIAL MAP CLASSIFIER.
\end{remark}
...
of universes~\cite{cohen-et-al:cubicaltt}.
\end{example} 
