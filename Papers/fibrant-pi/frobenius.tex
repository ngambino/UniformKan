...
Over the past few years, there has been increasing interest in the study of algebraic  weak factorisation system~\cite{bouke-garner-I,bourke-garner-II,grandis-tholen-nwfs,garner:small-object-argument} and algebraic model categories~\cite{riehl-algmodel-cat,riehl-cat-homotopy}. While standard weak factorisation systems and Quillen model categories involve classes of
maps satisfying appropriate weak orthogonality properties, algebraic weak factorisation systems and algebraic model categories  involve classes of maps that are equipped with additional structure which specifies diagonal fillers for appropriate diagrams, which is furthermore required to satisfy appropriate uniformity conditions. In this context, we generally
refer as uniform (trivial) fibrations for algebraic counterparts of the standard notions of (trivial) fibrations. Although the initial
motivation for this developments derives from homotopy theory, this research has recently found applications in relation to Voevodsky's univalence foundations programme~\cite{voevodsky:uf}, beginning with the work by  Bezem, Coquand, and Huber~\cite{coquand-cubical-sets} on uniform Kan fibration in categories of cubical sets.


Our aim here is to contribute to the development of the theory of algebraic weak factorisation
systems by introducing the uniform Frobenius condition,  the general notion of uniform fibration in a presheaf category equipped with a functorial cylinder, and establishing that, under mild assumptions, uniform fibrations are the right maps of algebraic weak factorisation 
systems that satisfy the uniform Frobenius condition. As a special case of our results, we prove that, in the category of simplicial sets,  pushforward along a uniform Kan fibration preserves uniform Kan fibrations. This result provides a solution to
the  open problem of giving a constructive counterpart of the fact that pushforward along a Kan fibration preserves Kan fibrations, a well-known result of classical 
simplicial homotopy theory that is fundamental for the definition of simplicial model of univalent foundations~\cite[Lemma~2.3.1]{voevodsky-simplicial-model} but cannot be proved constructively~\cite{coquand-non-constructivity-kan}. When instanciated to a category of cubical sets with connections, instead, our results  give back the results obtained by Coquand and his collaborators in~\cite{cohen-et-al:cubicaltt}. Indeed, one of the initial motivations for this work was to explore whether Coquand's ideas could be developed at a greater level of generality, so as to make them applicable also to simplicial sets. 


We devote particular attention to uniform (trivial) fibrations in presheaf categories equipped with a functorial cylinder.
In this setting, our definitions of uniform trivial fibrations and uniform fibrations can be seen
as algebraic counterparts of those introduced by Cisinski in~\cite{cisinski-asterisque}: 
...
As a special case of this result, we obtain algebraic weak factorization systems in simplicial and cubical sets that have uniform Kan fibrations as the right maps, which are of interest for the interpretation of Martin-L\"of's rules for identity types~\cite{awodey-warren:homotopy-idtype,gambino-garner:idtypewfs,warren:thesis,garner:topological-simplicial,shulman:inverse-diagrams}.
These algebraic weak factorization systems are obtained using Garner's small object argument~\cite{garner:small-object-argument}.
...
Working with uniform fibrations is essential to prove this fact since it allows us to write an arbitrary lifting problem as a colimit of a functorial family of lifting problems. This may be
considered a further example of the good interaction between colimits and lifting problems in the theory of algebraic weak factorization systems that was already noted in~\cite{riehl-cat-homotopy}.
...
In this setting, we introduce a version in our setting of the Frobenius 
condition for weak factorization systems~\cite{garner:types-omega-groupoids},
which is related to Lawvere's Frobenius reciprocity condition~\cite{lawvere-equality}
(see ~\cite{clementino:frobenius} for details). This condition 
...
For $k \in \braces{0, 1}$, the category $\cal{S}_k$ of strong $k$-oriented homotopy equivalences in $\catE$ can isomorphically be described as the category of arrows $f \in \catE^{\to}$ with a retraction $\rho$ to $\thetak \hatotimes f$.
In detail,
\begin{enumerate}[(i)]
...
The assumption of $u \otimes_{\cal{D}} (-) \co \cal{D} \to \catE^\to$ lifting through $\cal{S}_k \to \catE^\to$ is equivalent to having a retraction to~$\thetak \hatotimes_\catE (u \otimes_{\cal{D}} (-))$.
...
\subsection*{Connections} Although parts of our development are carried out assuming only a functorial cylinder, some of our results assume also the presence of connections. We recall
the definition.
...
\subsection*{Relating uniform fibrations and strong homotopy equivalences}
Let $u \co \cal{I} \to \catE^\to$ be a functor.
...
\end{proof} 
 ...
 In order to obtain a functor in the other direction,
we make use of the additional assumption of the presence of 
