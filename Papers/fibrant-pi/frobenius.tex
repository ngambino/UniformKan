...
\begin{proof}
For (i), the claim follows from~\cref{uniform-fibrations-uniform-frobenius}. For (ii), the claim follows from \cref{uniform-fibrations-uniform-frobenius} and \cref{lift-pushforward-BC} (see also \cref{beck-chevalley-iso})
\end{proof}
...
Let $\catE$ be a locally presentable, locally cartesian closed category equipped with a functorial cylinder $I \otimes (-) \co \catE \to \catE$ with 
contractions and connections.
Let $(\LL, \RR)$ be a suitable category of monomorphisms in $\catE$.
Then the algebraic weak factorization system whose right maps are the uniform $\LL$-fibrations satisfies the functorial Frobenius property.
\end{theorem}
