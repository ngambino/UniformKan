...
This condition, called the functorial Frobenius condition, was stated for cloven a wfs in~\cite{garner:topological-simplicial}, which is slightly a more general notion than an awfs.
...
We observe that both (i) and (ii) are equivalent to $(\pcoalg{\LL}, \palg{\RR}, \pcoalg{\LL})$ satisfying the generalized uniform Frobenius condition. For the equivalence with (i), use \cref{generalized-uniform-frob-nicer}. For the equivalence with (ii), use \cref{generalized-uniform-frob-functorial}, noting that 
...
\begin{proof}
We consider the lift $\widetilde{Q}$ of $Q$ in the generalized uniform Frobenius condition~\eqref{generalized-uniform-frob:lift} for $(\cal{I}, \cal{J}, \liftl{(\liftr{\cal{K}})})$.
Since the functor $\liftl{(\liftr{w})} \co \liftl{(\liftr{\cal{W}})} \to \cal{E}^\to$ is faithful, the lift $\widetilde{Q}$ consists just of a lift of the action of $Q$ on objects that is coherent with respect to the action of $Q$ on morphisms, in the sense that the action of $Q$ on morphisms determines uniquely the action of $\widetilde{Q}$ on morphisms.
...
We now give the desired characterization of the functorial Frobenius condition for an algebra{-}ically-free awfs.
...
Observe that $\overline{\pAlg{\RR}} \leftrightarrow \palg{\RR}$ (this is the case for (co)pointed endofunctor and (co)monad algebras over any (co)monad).
