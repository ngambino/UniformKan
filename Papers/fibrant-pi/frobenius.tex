...
\footnote{When $\cal{E}$ is a category of presheaves over an elegant Reedy category~\cite{bergner-rezk-elegant}, we can be more specific: the boundary inclusions (latching object inclusions for representables) form a set of generators.}
...
To illustrate the relationship with the standard Kan filling condition for cubes, let $i^n \co \partial \Box^n \to \Box^n$, for $n \in \mathbb{N}$, denote the boundary inclusion for of the $n$-cube $\Box^n$, which is given by the $n$-fold Leibniz product of~$[\lcyl, \rcyl] \co \braces{0\,, 1} \to \Box^1$.
...
\]
are generating trivial cofibrations.
...
\, .\]
Recall the chosen set of generators $\cal{I}$ of $(\Cof, \TrivFib)$, defining our fibrations via right orthogonality to $\cal{I}_\otimes$.
