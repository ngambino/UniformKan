...
Fix a  locally presentable category $\catE$. Recall from~\cite{kamps-porter:homotopy} that a \emph{functorial cylinder} in $\calE$ is an endofunctor $\interval \otimes (-) \co \catE \to \catE$  equipped with natural transformations 
\[
...
Our main examples functorial cylinders with  contractions and connections, to be discussed below, arise from monoidal categories $(\catE, \otimes, \top)$ 
equipped with interval object, \ie an object~$\interval \in \catE$ equipped with maps~$\lcyl, \rcyl \co \top \to \interval$, called the left 
...
We write $\SSet$ for the category of simplicial sets, defined as the category of presheaves over the simplex category $\Delta$.
...
by special cases of horn inclusions, $\kcyl = h^1_n$, for $k = 0, 1$. We have contractions, since the monoidal structure is cartesian,
...
Categories of cubical sets are defined as presheaves over a cube category $\Box$ that encodes the specific variant of cubical sets under consideration.
...
The category $\Box$ has a symmetric monoidal structure, which makes cubical sets into a symmetric monoidal closed category.\footnote{The presence of symmetries precludes $\Box$ from being a Reedy category.
...
We stress that the only feature of the cube category $\Box$ relevant for the main results  are connections and symmetries.
...
The category of cubical sets $\CSet$ as studied in \cite{cohen-et-al:cubicaltt} has a monoidal structure in which the unit is a terminal object.
An interval object with contractions is given by $\Box^1$ with endpoint inclusions $\kcyl \co \braces{k} \to \Box^1$  given by the maps from a singleton set to the free de Morgan algebra 
...
Let us now assume that $\catE$ is equipped with a functorial cylinder $\interval\otimes (-) \co \catE \to \catE$ with contractions and connections, and that the functor $\interval\otimes (-) \co \catE \to \catE$ has a right adjoint, which we write~$\exp(I, -) \co \catE \to \catE$.  As a consequence, $\interval\otimes (-) \co \catE \to \catE$ preserves colimits. 
In order to introduce the notion of a suitable weak factorisation system in~\cref{uniform-fib-presheaf-cat} below, we need to introduce some notation.  For a map $i \co A \to B$ in $\catE$, we define 
\[
\kcyl \hatotimes i \co (\interval \otimes A) +_A B \to \interval \otimes B
\] 
to be given by the following pushout diagram:
\begin{gather*}
\xymatrix@C=1.2cm{
  A
  \ar[r]^{i}
...
\rlap{.}}
\end{gather*}
We call $\kcyl \hatotimes i$ the \emph{Leibniz product} of $\kcyl$ and $i$.
...
\item the maps in $L$ are monomorphisms, 
...
\item $L$ is closed under Leibniz product with the endpoint inclusions, \ie for every $i \in L$ we have 
 $\kcyl \hatotimes i \in L$.
\end{enumerate}
\end{definition}

When working with a suitable weak factorisation system, we refer to maps in $\cal{L}$ as \emph{cofibrations} and to maps in $\cal{R}$ as \emph{trivial fibrations} in order to help the intuition.
Indeed, a wide class of examples of suitable weak factorisation systems arises from Cisinski model structures~\cite{cisinski-asterisque}, \ie cofibrantly generated Quillen model structures on presheaf categories in which the cofibrations are the monomorphisms. In particular, for presheaf categories over an elegant Reedy category~\cite{bergner-rezk-elegant}, such a weak factorisation
system is cofibrantly generated by the boundary inclusions. In particular, the right lifting property with respect to boundary inclusions implies the right lifting property with respect to all monomorphisms (see also~\cref{sec:non-alg}). We are now concerned with the issue of defining a new weak factorisation system that can be thought of as consisting of trivial cofibrations and fibrations. For this, we begin by defining the class of maps that serve the purpose of testing whether a certain map is a fibration, imitating the development in simplicial sets. Such a class is given by what we call the
\emph{cylinder inclusions}, 
\[
\mathsf{Cyl}  \defeq \{ \kcyl \hatotimes i \ | \ k \in \{ 0, 1 \} \, ,  i \in L \}
\]
We then define $R_\otimes$ to be the class of maps with the right lifting property with respect to all cylinder inclusions, \ie 
\[
R_\otimes \defeq \mathsf{Cyl}^\pitchfork
\]


\begin{proposition} \label{thm:wfstimes} There exists a weak factorisation system $(L_\otimes, R_\otimes)$ such that $R_\otimes$ are exactly the maps with the right lifting property with respect to
the cylinder inclusions.
\end{proposition}

\begin{proof} Let $I$ be a generating set of maps for $(L, R)$ and define $(L_\otimes, R_\otimes)$ to be the weak factorisation system cofibrantly generated by the set 
\[
J \defeq \{ \delta^k \hatotimes i \ | \ i \in I \}
\]
It remains to check that $R_\otimes$ is exactly the set of the maps with the right lifting property with respect to the cylinder inclusions.
\end{proof}

We refer to the maps in $L_\otimes$ and $R_\otimes$ as \emph{trivial cofibrations} and \emph{fibrations}, respectively. 
Note that, since $L$ is  closed under Leibniz products with the endpoint inclusions, we have that $L_\otimes
\subseteq L$, which could be rephrased as saying that every trivial cofibration is a cofibration. 
We  illustrate these notions in simplicial sets and cubical sets.

\begin{example}[Kan fibrations]
In $\SSet$, we let~$(\cal{L}, \cal{R})$ to be the weak factorisation system of monomorphisms and trivial Kan fibrations. Then, an $\cal{L}$-fibration
is the same thing as a Kan fibration in the usual sense. 
Indeed, an $\cal{L}$-fibration $p \co X \to Y$ is a map for which there exist diagonal fillers for diagrams of the form
\[
\xymatrix{
...
Even if higher-dimensional horns are not involved explicitly, they are included indirectly since they are retracts of the Leibniz product of a one-dimensional horn inclusion with themselves~\cite[Chap.~IV, Sec.~2]{gabriel-zisman:calculus-of-fractions}.
\end{example}

\begin{example}[Coquand fibrations]
Also in the category $\CSet$ of cubical sets, we consider~$\cal{L}$ to consist of all monomorphisms.
A  (trivial) $\cal{L}$-fibration in $\CSet$ will be called a \emph{(trivial) Kan fibration}.
...
Since our cube category has symmetries, a Kan fibration in $\CSet$ is has the left lifting property also with respect to open box inclusions, which are counterparts in cubical sets of the horn inclusions in simplicial sets.%
...
The fibrant cubical types defined in~\cite{cohen-et-al:cubicaltt} arise by considering the  $\cal{L}'$-fibrations where~$\cal{L}'$ is the full subcategory of $\cal{L}$ spanned by (decidable) monomorphisms classified by their face lattice~$\mathbb{F}$, \ie those arising as a pullback of $\braces{\top} \to \mathbb{F}$, excluding for example diagonals in cubes.
...
Note that  $\cal{L}'$ satisfies the properties of a suitable weak factorisation system and is thus covered by our development.
\end{example}

\begin{remark} \label{cisinski-remark}
We also relate the notion of an $\cal{I}$-fibration introduced here with the notion of a naive fibration introduced by Cisinski~\cite{cisinski-asterisque}.
...
Let us begin by recalling a definition.

\begin{definition} We say that a weak factorisation system $(\cal{L}, \cal{R})$ has the \emph{Frobenius property} if pullback of $\cal{L}$-maps along $\cal{R}$-maps are again $\cal{L}$-maps, \ie in every pullback diagram of the form
\[
\xymatrix{
 Y \ar[d]_j \ar[r]^q & X \ar[d]^i \\ 
B \ar[r]_p & A }
\]
where $p \in \cal{R}$, if $i  \in \cal{L}$ then $j \in \cal{L}$.
\end{definition} 

One of the reasons for the interest in the Frobenius conditition is that it is closely related to the right properness of model structures. Recall that the right properness of a model 
structure~$(\cal{W}, \cal{F}, \cal{C})$ means that the pullback of a weak equivalence along a fibration is again a fibration. It is easy  to see that, when the cofibrations are the monomorphisms, the right properness of the model structure is equivalent to the Frobenius property of the weak factorisation system~$(\cal{W} \cap \cal{C}, \cal{F}$), \ie that the pullback of a trivial cofibrations along a fibration is again a trivial cofibration. Our aim in this section is to show that the weak factorisation system $(\cal{L}_\otimes, \cal{R}_\otimes)$ of  \cref{thm:wfstimes} satisfies the Frobenius property. 
The strategy of the proof is as follows. We will begin by defining a class of maps, called \emph{strong homotopy equivalences}. Then, the proof will consist of three main steps. 
First, we will give a characterization of strong homotopy equivalences as retracts of certain canonical maps. Secondly, we show that the maps in $\cal{L}$ which are strong homotopy
equivalences are retracts of maps in $\cal{L}_\otimes$ and that every map in $\cal{L}_\otimes$ is both in $\cal{L}$ and a strong homotopy equivalence. This implies that a map is
a $\cal{L}$-fibration if and only if it has the right lifting property with respect to $\cal{S} \cap \cal{L}$. The third and final step of the proof will consist in showing that the pullback
of a $(\cal{S} \cap \cal{L})$-map along a fibration is again an $(\cal{S} \cap \cal{L})$-map, which can be seen to imply the Frobenius condition for $(\cal{L}_\otimes, \cal{R}_\otimes)$.
...
Examples of strong $k$-oriented homotopy equivalences are given by strong deformation retracts, i.e. homotopy equivalences for which the homotopy~$\phi$ is trivial, \ie $\phi = \varepsilon_X$, where $\varepsilon \co I \otimes (-) \to \Id_\catE$ is the contraction of the functorial cylinder. For $k \in \braces{0, 1}$, we write $\mathsf{SHeq}_k$ for the class of strong $k$-oriented homotopy equivalences.
...
\]
for $k \in \braces{0, 1}$. This diagram gives us an arrow $\thetak \hatotimes f \co f \to \kcyl \hatotimes f$ in the category of arrows in $\catE$, which we now use
to provide the following characterization of strong homotopy equivalences as retractions. 

\begin{lemma}[Strong homotopy equivalences as sections] \label{strong-h-equiv-as-section-non-alg}
...
\begin{remark}
\cref{strong-h-equiv-as-section} implies that strong $k$-oriented homotopy equivalences are closed under retracts since functors, in this case $\delta^k \hatotimes (-)$, preserve retracts and sections are closed under retracts.
\end{remark}


For our second step, we focus on the class $\cal{S} \cap \cal{L}$, \ie the class of maps in $\cal{L}$ that are strong $k$-oriented homotopy equivalences.
We begin by relating them with the maps in $\cal{L}_\otimes$, as defined in~\eqref{thm:wfstimes}.

\begin{lemma}  \label{thm:main-sheretract}\hfill 
\begin{enumerate}[$(i)$] 
\item \label{lem:from-strong-hequiv}
 $S \cap L \subseteq  \overline{L_\otimes}$.
\item \label{lem:to-strong-hequiv}
$L_\otimes \subseteq S \cap L$.
\end{enumerate}
\end{lemma}

\begin{proof}
Part (i) follows by \cref{strong-h-equiv-as-section-non-alg}. For part (ii), since $L$ is closed under Leibniz tensor with~$\kcyl$, we have $L_\otimes \subseteq L$.
Then, observe that for every $f \co X \to Y$, the map $\kcyl \hatotimes f$ is a strong $k$-oriented homotopy equivalence. Hence, 
every map in $L_\otimes$ is a (left or right) strong homotopy equivalence.
\end{proof}
...
A map is a fibration if and only if it has the right lifting property with respect to  $L \cap S$.
Hence, the saturation of $L_\otimes$ and $L \cap S$ coincide.
\end{proposition}

\begin{proof}
Combine parts (i) and (ii) of \cref{thm:main-sheretract}.
\end{proof}


\begin{theorem} The weak factorisation system $(L_\otimes, R_\otimes)$ has the Frobenius property. 
\end{theorem}

\begin{proof} We wish to show that, in a pullback diagram of the form
\begin{equation}
...
\end{gathered}
\end{equation}
where $p \in R_\otimes$, if $i  \in L_\otimes$ then $j \in L_\otimes$. For this, it suffices to show that
if $i \in L \cap S_k$, then $j \in L \cap S_k$. In fact, since $L$ is assumed to be closed under pullback, we need to show
only that $j \in S_k$.
This means that $\thetak \hatotimes i \co i \to \kcyl \hatotimes i$ has a retraction $\rho$, as follows:
\begin{align*}
\xymatrix@C+1em{
  i 
  \ar[r]^-{\thetak \hatotimes i}
  \ar@{=}[dr]
&
  \kcyl \hatotimes i \ar[d]^{\rho}
\\&
  i
\rlap{.}}
\end{align*}
We want to show that $j$ is a strong $k$-oriented homotopy equivalence.
This means to show that $\thetak \hatotimes j \co j \to \kcyl \hatotimes j$ has a retraction, as follows:
\[
\xymatrix@C+1em{
  j
...
\rlap{.}}
\]
We will define the retraction $\tau$ such that the below diagram, where $\sigma \co j \to i$ is 
the pullback in~\eqref{non-alg-strong-h-equiv-uniform-base-change:0}, 
commutes and its two horizontal composites are identities:
\[
\xymatrix@C+2em{
j 
  \ar[r]^-{\thetak \hatotimes j}
...
\rlap{.}}
\]
But we have such a filler since $\kcylinv \otimes B \in L_\otimes$ and $p \in R_\otimes$. 
\end{proof}


\begin{remark}[Right properness of the model structure for Kan complexes] \label{rem:rightpropernessnew}
...
Recall also that $(\LL, \RR)$ is said to be \emph{algebraically-free} on a category $\cal{I}$ of arrows if $\pAlg{\R} = \liftr{\cal{I}}$.
...
\begin{proof}
Using the notation introduced above, for $(f, \phi) \in \liftr{\cal{J}}$, we have $\liftr{F}(f, \phi) = (f, \phi_F)$ and~$\liftr{G}(f, \phi) = (f, \phi_G)$.
...
\subsection*{Restriction and extensions} We now consider some results in the case $\catE$ is a presheaf category. 
...
\begin{remark}
Recall that the Frobenius condition for a weak factorization system states that pullback along a right map preserves left maps~\cite{garner:types-omega-groupoids,gambino-garner:idtypewfs}.
...
\section{Uniform fibrations with the Frobenius property}


\subsection*{Existence of the algebraic weak factorisation systems}
...
Let $\catE$ be a presheaf category equipped with a functorial cylinder $I \otimes (-) \co \catE \to \catE$ that has a right adjoint and a suitable category $\cal{M}$ of monomorphisms (in the sense of~\cref{uniform-fib-presheaf-cat}).
\begin{enumerate}[(i)]
...
\begin{proof}
For (i), one applies Garner's small object argument, using part~(i) of~\cref{small-gen-triv-kan} and smallness of $\cal{N}$.
...
This section leads to the proof that the algebraic weak factorization system on presheaf categories with uniform fibrations as right maps satisfies the functorial Frobenius condition.
...
For the remainder of this section, we fix $k \in \braces{0, 1}$.
...
\begin{proposition} \label{thm:onedir}
There is a functor
\[
\xymatrix@!C@C-2.5em{
...
\begin{proof}
Combine the cases $k = 0$ and $k = 1$ of~\cref{lem:from-strong-hequiv} and compose with a component of the unit of the monad $\liftl{(\liftr{(-)})}$.
...
\begin{proof}
Combine the cases $k = 0$ and $k = 1$ of~\cref{lem:to-strong-hequiv}.
\end{proof}
...
\begin{proof}
We begin by proving (i) and then show that the other claims hold by a linear sequence of implications.
