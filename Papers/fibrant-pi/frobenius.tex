...
\subsection*{Remarks} For brevity, we do not state the results for standard 
weak factorisation systems that follow  from our results on algebraic
weak factorisation system. These can be readily obtained by replacing 
the requirements of existence of structure with that of satisfaction of properties,
replacing functors between categories with inclusions between
classes, and so on. The idea is illustrated in a few examples at the beginning
of the paper and then highlighted only when appropriate.

In the hope of making the paper more widely accessible, we have also
confined comments on constructivity issues to a series of remarks and footnotes running through the paper. However,
...
cubical sets in~\cref{rem:constructive-small-object}.\footnote{A constructive proof 
