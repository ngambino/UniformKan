...
Algebraic weak factorization systems~\cite{garner:small-object-argument,grandis-tholen-nwfs} are a variant of standard weak factorization systems~\cite{bousfield-wfs}.  They involve maps  equipped with additional structure, given by an explicit choice of solutions for lifting problems which, crucially, satisfies a naturality condition, rather than maps satisfying a lifting property (as in the definition of a Kan fibration). 
...
In order to do so, we introduce what we call the uniform Frobenius condition, which provides a convenient 
way to phrase our analysis.
...
When instantiated in the category of cubical sets with connections, instead, \cref{frobenius-uniform-presheaf} gives  a new constructive 
...
\rlap{,}}
\]
where $i$ and $j$ are in $\cal{M}$ and the square on the left is a pullback. Uniform fibrations are defined as
the right $\cal{M}_\otimes$-maps (by instantiating~\cref{def:I-fibration}) and the category of uniform  $\cal{M}$-fibrations, 
 is given by letting
\[
...
For this, we use our characterisation of the functorial Frobenius property in algebraically
