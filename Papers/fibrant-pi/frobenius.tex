...
In particular, we show that in a wide class of presheaf categories, including simplicial sets and cubical sets with connections, uniform fibrations are the right class of a natural weak factorization system and satisfy the Frobenius condition.
...
Let us also mention that, just as we introduce a version of the Frobenius condition for a map, we introduce also a version of the Beck-Chevalley conditions for morphisms of maps, \ie commutative squares.
...
Although large parts of our development are carried out assuming only a functorial cylinder, some of our results, including that of the uniform Frobenius condition for uniform fibrations (\cref{uniform-fibrations-uniform-frobenius}), assume also the presence of connections.
...
\begin{remark}
\cref{strong-h-equiv-as-section} implies that strong left or right homotopy equivalences are closed under retracts since the functors $\delta^0 \hatotimes -$ and $\delta^1 \hatotimes -$ preserve retracts and sections are closed under retracts.
\end{remark}
...
In order to guarantee that the diagram commutes, we send $i \in \cal{I}$ to a pair of the form $(\kcyl \hatotimes i, \rho)$, using the assumption~\eqref{to-strong-hequiv:0} that $\kcyl \hatotimes (-)$ lifts to $\cal{I}$.
...
Given a functor $\cal{K} \to \cal{E}^\to$, we write $\cal{K}_{/-}^{\to}$ for the opposite Grothendieck construction of the functor sending $X \in \cal{E}$ to the slice category $\cal{K}_{/X}^{\to}$.
...
Coherence in morphisms in $\cal{I}_{/-} \times_{\cal{E}} \cal{J}$ separates into two parts:
\begin{enumerate}[(i)]
...
\xymatrix{
&
  {\cal{I}_2}_{/-} \times_{\cal{E}} \cal{J}_2
  \ar[rr]^{Q_1}
  \ar[dd]^(0.25){{u_1}_{/-} \times_{\cal{E}} v_1}
&&
  \liftl{(\liftr{\cal{I}_2})}_{/-}
  \ar[dr]
  \ar[dd]_(0.25){\liftl{(\liftr{u_1})}_{/-}}
\\
  {\cal{I}_1}_{/-} \times_{\cal{E}} \cal{J}_1
  \ar[ur]
  \ar[dr]_{{u_2}_{/-} \times_{\cal{E}} v_2}
  \ar[rrrr]^{Q_2}
&&&&
  \liftl{(\liftr{\cal{I}_1})}_{/-}
  \ar[dl]^{\liftl{(\liftr{u_2})}_{/-}}
\\&
  \cal{E}^\to_{/-} \times_{\cal{E}} \cal{E}^\to
  \ar[rr]_{P}
&&
...
By the close relationship between uniform $\cal{I}$-fibrations and the right maps for $\cal{S}_0(\cal{I}) + \cal{S}_1(\cal{I})$ (\cf \cref{relating-strong-hequiv-and-uniform-fib}) this will enable us to establish the uniform Frobenius condition for uniform $\cal{I}$-fibrations with respect to $\cal{I}_\otimes$, as desired.

Let $u' \co \cal{I}' \to \cal{E}^\to$ be the functor with domain $\cal{E}$ that sends $X$ to $\bot_X$.
The central technical tool of our development is the following statement.

\begin{lemma} \label{technical}
Let $k \in \braces{0, 1}$.
Right $(\delta^{1-k} \hatotimes u')$-maps satisfy the uniform Frobenius condition with respect to strong $k$-sided homotopy equivalences.
\end{lemma}
...
Using \cref{uniform-frobenius-product-u}, we may combine the uniform Frobenius conditions for uniform $\cal{I}$-fibrations with respect to $\cal{S}_0 + \cal{S}_1$, provided by (ii), and with respect to $\cal{I}$, provided by assumption (2).

For the step from (iii) to (iv), apply functoriality of uniform Frobenius in the form of \cref{uniform-frobenius-functorial} with $F$ the map $\cal{S}_0(\cal{I}) + \cal{S}_1(\cal{I}) \to \liftl{(\liftr{\cal{I}_\otimes})}$ induced by \cref{lem:from-strong-hequiv} and $G$ the map $\cal{I}_\otimes \to \cal{S}_0(\cal{I}) + \cal{S}_1(\cal{I})$ induced by \cref{lem:to-strong-hequiv} (\cf \cref{relating-strong-hequiv-and-uniform-fib}).
Note that the latter map uses assumption (3).
\end{proof}
...
Under the assumptions (1) to (3) stated above, we have that $\Fib{\cal{I}} $ satisfies the uniform Frobenius condition with respect to $\cal{I}_{\otimes}$.
\qed
\end{theorem}
...
Assumption (1) is guaranteed by condition (v).
...
Let $k \in \braces{0, 1}$.
Right $(\delta^{1-k} \hatotimes u')$-maps have the uniform Frobenius condition with respect to strong $k$-sided homotopy equivalences.
\end{replemma}

\begin{proof}
We use the characterization of the uniform Frobenius condition given by \cref{uniform-frob-even-nicer}.
The required lift is provided by the lift $Q'$ in \cref{strong-h-equiv-uniform-base-change} below after postcomposing it with the unit of monad $\liftl{(\liftr{-})}$.
Note that $\delta^{1-k} \hatotimes \bot_X = \delta^{1-k} \otimes \id_X$ for $X \in \cal{E}$.
\end{proof}

Let $P' \co \cal{E}^\to \times_{\cal{E}} \cal{E}^\to \to \cal{E}^\to$ the pullback functor, sending a cospan $(g, h)$ to $h^* g$.
