...
Over the past few years, there has been increasing interest in algebraic  weak factorisation systems~\cite{bourke-garner-I,bourke-garner-II,garner:small-object-argument,grandis-tholen-nwfs}. Algebraic weak factorisation systems
differ from standard weak factorisation systems~\cite{bousfield-wfs} 
in that they do not involve maps that merely satisfy a left (or right) lifting property,
as in the definition of a Kan fibration, but rather maps that are equipped with 
additional structure, which provides explicit solutions for  left (or right) lifting problems
and is required to satisfy a naturality condition. 
Although algebraic weak factorisation systems were originally introduced to address issues in categorical homotopy theory (for example, the unsatisfactory interaction between 
colimits and lifting problems in standard weak factorisation systems, discussed in~\cite[Chapter~12]{riehl-cat-homotopy}), 
 the seminal work of  Bezem, Coquand, and Huber on uniform Kan fibration in categories of cubical sets~\cite{coquand-cubical-sets} 
 has shown that they have natural applications in Voevodsky's univalent foundations programme~\cite{voevodsky:uf}, developed further in~\cite{awodey-cubical,cohen-et-al:cubicaltt,pitts-cubical-nominal,swan-awfs}. 
 
 Our aim here is to develop further the theory of algebraic weak factorisation systems. In particular, we introduce the 
 notion of uniform (trivial) fibration in  a category with a functorial cylinder
and use it to obtain algebraic counterparts of some important results in Cisinski's theory of Quillen model structures on presheaf categories~\cite{cisinski-asterisque}. We  apply these results  to solve the open problem in univalent foundations, namely that of proving a constructive counterpart of the
 ...
 which avoids the use of topological realization~\cite{hovey-model-categories} and
...
equivalences~\cite[Definition 13.1.1]{hirschhorn-model-localizations}. Indeed, for 
...
is equivalent to the preservation of right maps by pushforward along a right map. For example, the Frobenius property for 
the weak factorisation system of acyclic cofibrations and Kan fibrations
in the category of simplicial sets is equivalent to the preservation of Kan fibrations by pushforward along Kan fibrations,
...
In the context of algebraic weak factorisation systems, it is natural to require not only a version of the Frobenius condition, which is a property of maps, but 
also a counterpart of it for morphisms of maps (\ie commutative squares), which we call the Beck-Chevalley condition,
since it is analogous to the well-known Beck-Chevalley conditions~\cite{benabou-descente,lawvere-equality}.  Hence, as part of our development, we will introduce a single
 single condition, which we call the uniform Frobenius condition, which is equivalent to a combination of the Frobenius and Beck-Chevalley conditions and that we consider the
 appropriate counterpart for algebraic weak factorisation systems of the usual condition. The uniform Frobenius condition introduced here is similar to the functorial Frobenius condition of~\cite{garner:topological-simplicial}, but formulated in a slightly different setting.

The main results in this paper establish the existence of algebraic weak factorisation systems with the uniform Frobenius condition on presheaf categories
...
Uniform trivial fibrations are defined by weak uniform orthogonality with respect to a category~$\mathcal{M}$ of monomorphisms and pullback squares, while uniform fibrations are defined by weak uniform orthogonality with respect to a category obtained by applying a Leibniz construction, in the sense of~\cite{riehl-verity:reedy}, to the endpoint inclusions of the functorial cylinder and~$\cal{M}$.
...
This is proved using Garner's small object argument~\cite{garner:small-object-argument} after having shown that uniform (trivial) fibrations can be be defined equivalently by weak uniform orthogonality with respect to a small category of maps (\cref{small-gen-triv-kan}).
Working with algebraic notions of fibrations is essential to prove this result, since it allows us to rephrase an general lifting problem as a colimit of a functorial family of lifting problems, providing
...
When instanciated to simplicial sets, our results imply that 
...
We are interested in algebraic counterparts of the weak orthogonality properties that are used in the definition of a weak factorization system~\cite{bousfield-wfs}.
Instead of starting from a mere class of arrows in a category $\catE$ and defining its left or right orthogonal class, we consider a category~$\cal{I}$, to be thought of as an indexing category (but not assumed to be small), and a functor~$u \co \cal{I} \to \catE^\to$.
...
Let $\catE$ be a presheaf category and $\cal{M}$ be some full subcategory of $\catE_\cart^\to$ of monomorphisms (we will start to impose certain conditions on $\cal{M}$ in \cref{uniform-fib-presheaf-cat}). In this context, a right $\cal{M}$-map will be called a \emph{uniform trivial $\cal{M}$-fibration}. This terminology is intended to suggest an analogy with the theory developed by Cisinski in~\cite{cisinski-asterisque}, where he studied extensively model structures in which the cofibrations are the monomorphisms. The compatibility condition for a uniform trivial $\cal{M}$-fibration $(f, \phi) \co X \to Y$ involves diagrams of the form
\[
\xymatrix{
  A
  \ar[r]^{h}
...
A uniform trivial $\cal{M}$-fibration in $\SSet$ or $\CSet$ with $\cal{M}$ consisting of all monomorphisms will be called a \emph{uniform trivial Kan fibration}.\footnote{When working constructively, it is appropriate to restrict the attention to decidable  monomorphisms, \ie natural transformations whose components are injective functions with decidable image. See~\cref{rem:constructive-small-object} for further comments on this issue.}
\end{example}
...
In the standard setting of weak factorization systems, it is well-known that the right orthogonality operation commutes with slicing, while the left orthogonality operation commutes with coslicing.
In order to prove a counterpart of this fact in the setting of algebraic weak factorisation
systems, we need some auxiliary definitions.
...
\rlap{.}}
\]
In the standard setting of weak factorization systems, it is well known that if we have classes of maps $\cal{I} \subseteq \catE^\to$ and~$\cal{J} \subseteq \cal{F}^\to$, then $F(\cal{I}) \subseteq \liftl{\cal{J}}$ if and only if $\liftr{\cal{I}} \subseteq G(\cal{J})$.
...
\]
natural in $i \in \cal{I}$ and $j \in \cal{J}$.
Since $F$ is left adjoint to $G$, these situations are equivalent.
\end{proof}

We will use the example discussed in~\cref{exa:composition-pullback-lift} below
in~\cref{sec:frobc} when introducing the Beck-Chevalley conditions.  
...
A morphism from $U_1 \dashv V_1$ to $U_2 \dashv V_2$ consists of natural transformations $u \co U_1 \to U_2$ and $v \co V_2 \to V_1$ forming mates, in the sense of~\cite{kelly-street}.
...
We now establish some general facts about the interaction between orthogonality functors and Kan extensions along fully faithful functors. These will be applied in \cref{sec:unifpc} to show that uniform (trivial) Kan fibrations are the right maps of a natural weak factorization system.
...
We finish by unifying the Frobenius and Beck-Chevalley conditions, calling it the uniform Frobenius condition, that we consider the appropriate counterpart for algebraic weak
factorisation systems of the usual Frobenius condition~\cite{garner:types-omega-groupoids,gambino-garner:idtypewfs}. As already remarked in the introduction,
the uniform Frobenius condition is similar to the functorial Frobenius condition of~\cite{garner:topological-simplicial}, but formulated in a slightly different setting.
...
We will relate the uniform Frobenius condition introduced above with van den Berg and Garner's functorial Frobenius condition~\cite{garner:topological-simplicial} in~\cref{thm:frobenius-comparison} below. In order to do so, it is convenient 
...
\begin{remark} In applications, we might only need functoriality of the uniform Frobenius condition of $\cal{J}$ with respect to $\cal{I}$ in one of the parameters.
...
The underlying weak factorization system of $(\LL, \RR)$ has as left and right classes maps that admit a lift to $\pcoalg{\LL}$ and $\palg{\RR}$.
...
An algebraic weak factorization system is called functorially Frobenius if $\pcoalg{\LL}$ is stable under pullback along $\palg{\RR}$ in the sense of a lift $Q'$ as follows:
\[
\xymatrix@C+2em{
...
\begin{proposition} \label{thm:frobenius-comparison}
...
\item
Assume that $\cal{E}$ is locally presentable and locally Cartesian closed and that $(\LL, \RR)$ is algebraically free on a category $\cal{I}$ of arrows.
...
Applying $\liftr{(-)}$ and using \cref{retract-closure}, it follows that there are maps in $\CAT/\cal{E}^\to$ back and forth between $\liftr{\cal{I}}$ and $\palg{\RR}$.
It also follows that that there are maps back and forth between $\liftl{(\liftr{\cal{I}})}$ and $\pcoalg{\LL}$.
With this, the functorial Frobenius condition for $(\LL, \RR)$ takes the form of a lift
\[
\xymatrix@C+2em{
  \pcoalg{\LL} \times_{\cal{E}} \liftr{\cal{I}}
  \ar@{.>}[r]
  \ar[d]
&
  \liftl{(\liftr{\cal{I}})}
...
\]
emulating the form of uniform Frobenius of \cref{uniform-frob-even-nicer}, with a subtle difference: instead of a map coming from $\cal{I}$, we are pulling back a pointed endofunctor coalgebra for $\LL$.

Since there is a map $\cal{I} \to \pcoalg{\LL}$, we have that functorial Frobenius implies uniform Frobenius.
For the other direction, note that there is a map $\pcoalg{\LL} \to \overline{\cal{K}}$ where $\cal{K} \defeq \cal{E}^\to$ is regarded as a category of arrows via $\LL$: the pointed endofunctor coalgebra structure exhibits the underlying map as a retract of a free coalgebra for the comonad $\LL$.
Since pullback preserves retracts, it remains to find a lift of $P'$ to $\cal{K} \times_{\cal{E}} \liftr{\cal{I}} \to \liftl{(\liftr{\cal{I}})}$.
By inspecting the algebraic small object argument~\cite{garner:small-object-argument} [\ldots].
\end{proof}
}
...
\subsection*{Uniform fibrations in presheaf categories} The aim of this section is to study 
in more detail uniform $\cal{I}$-fibrations in presheaf categories. In this setting, the analogy
with the theory developed by Cisinski in~\cite{cisinski-asterisque} it is 
natural to consider $\cal{I}$ to be a category $\cal{M}$ of monomorphisms. Rather than taking all monomorphisms, we work making only some assumotions on $\cal{M}$, so as to recover
...
We will show that every (trivial) fibration  can be equipped with the structure of a uniform (trivial) fibration. We begin by proving that every trivial fibration  can be equipped with the structure of a uniform trivial fibration. This will shown in two steps: first,  we show that 
...
it is possible to reduce Kan filling to what is called there composition.
