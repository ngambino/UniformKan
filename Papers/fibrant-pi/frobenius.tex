...
Over the past few years, there has been increasing interest in algebraic  weak factorisation system~\cite{bourke-garner-I,bourke-garner-II,garner:small-object-argument,grandis-tholen-nwfs}, which
...
colimits and lifting problems in standard weak factorisation systems, see~\cite[Chapter~12]{riehl-cat-homotopy} for a discussion), 
 ...
 has shown that they have natural applications in Voevodsky's univalence foundations programme~\cite{voevodsky:uf}, as developed 
 in~\cite{awodey-cubical,cohen-et-al:cubicaltt,pitts-cubical-nominal,swan-awfs}. 
 
 Our aim here is to develop further the theory of algebraic weak factorisation
systems. In particular, we obtain algebraic counterparts of some important results in Cisinski's theory of Quillen model structures on presheaf categories~\cite{cisinski-asterisque} and apply them  to solve an open problem related to the simplicial model of univalent foundations~\cite{coquand-non-constructivity-kan,voevodsky-simplicial-model}.

We will be  interested in the property of weak factorisation systems called the Frobenius condition~\cite[Definition~3.2.1]{garner:types-omega-groupoids}, since it is closely related to Lawvere's Frobenius reciprocity condition~\cite{lawvere-equality} (see \cite{clementino:frobenius} for an explanation of the connection). One of the reasons for the 
...
a Quillen model structure in which the cofibrations are the monomorphisms, such as Cisinski model structures~\cite{cisinski-asterisque},   
right properness for the model structure is equivalent to the Frobenius condition for the  weak factorisation system of acyclic cofibrations and fibrations. 
The Frobenius condition is also intimately related to the homotopy-theoretic aspects of Martin-L\"of rules for identity types~\cite{gambino-garner:idtypewfs}.
...
implies  that pushforward along a right map preserves right maps. For the weak factorisation system of acyclic cofibrations and Kan fibrations
in the category of simplicial sets, this fact (namely that pushforward along Kan fibrations preserves Kan fibrations) plays an important role in the definition of the
simplicial model of univalent foundations~\cite[Lemma~2.3.1]{voevodsky-simplicial-model}.
As we will see, in the context of algebraic weak factorisation systems, it is natural to require not only a version of the Frobenius condition mentioned above, which is a property of maps, but 
also a counterpart of it for morphisms of maps (\ie commutative squares), which we call the Beck-Chevalley condition,
since it is analogous to the well-known Beck-Chevalley conditions~\cite{lawvere-equality}.  We will then show that it is possible to formulate a single condition, which we call the uniform Frobenius condition, which is equivalent to combination of the the Frobenius and Beck-Chevalley conditions.

We then turn our attention to establishing the existence of algebraic weak factorisation systems with the uniform Frobenius condition on presheaf categories.
Assuming the existence of a functorial cylinder,  it is possible to define general notions of uniform (trivial) fibration
...
a further example of the good interaction between colimits and lifting problems which motivated the introduction of algebraic weak factorisation systems
in~\cite{grandis-tholen-nwfs}.
...
As an application of these results, we obtain that in the category of simplicial sets
...
in the presence of the axiom of choice, we consider this result as a constructive counterpart of the well-known result of standard simplicial homotopy 
